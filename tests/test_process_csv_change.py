...
def test_missing_prompt_name_in_row(mock_change, capsys):
    ...
    assert not success
    assert list_of_jsons == [{"file_name": "valid_prompt_language.prompt", "modified_prompt": "modified prompt"}]
...
def test_nonexistent_code_file_in_row(mock_change, capsys):
    ...
    assert total_cost == 0.0
    assert model_name == ""
    expected_warning = f"[yellow]Warning:[/yellow] Input code file '/path/to/code/valid_prompt.py' does not exist. Skipping row 1."
...
def test_budget_exceeded(mock_change, capsys):
    ...
    assert not success
    # Adjusted expectation: only the first two prompts are processed before budget is exceeded
    assert list_of_jsons == [
        {"file_name": "prompt1_language.prompt", "modified_prompt": "modified prompt 1"}
    ]
...
def test_successful_processing(mock_change, capsys):
    ...
    assert list_of_jsons == [
        {"file_name": "prompt1_language.prompt", "modified_prompt": "modified prompt 1"},
        {"file_name": "prompt2_language.prompt", "modified_prompt": "modified prompt 2"}
    ]
...
def test_change_function_exception(mock_change, capsys):
    ...
    assert not success
    assert list_of_jsons == [{"file_name": "prompt1_language.prompt", "modified_prompt": "modified prompt 1"}]
    assert total_cost == 1.0
    assert model_name == "model_v1"
    captured = capsys.readouterr()
...
def test_unexpected_exception(mock_change, capsys):
    ...
    assert total_cost == 0.0
    assert model_name == ""
    expected_warning_message = f"[yellow]Warning:[/yellow] Input code file '/path/to/code/prompt1.py' does not exist. Skipping row 1."
...
def test_no_rows_to_process(mock_change, capsys):
    ...
    assert "Model Used: N/A" in captured.out
...
def test_multiple_prompts_same_model(mock_change, capsys):
    ...
    assert success
    assert list_of_jsons == [
        {"file_name": "prompt1_language.prompt", "modified_prompt": "modified prompt"},
        {"file_name": "prompt2_language.prompt", "modified_prompt": "modified prompt"},
        {"file_name": "prompt3_language.prompt", "modified_prompt": "modified prompt"}
    ]
...
def test_relative_imports(mock_change, capsys, tmp_path):
    ...
    assert success
    assert list_of_jsons == [{"file_name": "prompt1_language.prompt", "modified_prompt": "modified prompt"}]
    assert total_cost == 1.0
    assert model_name == "model_v1"
    captured = capsys.readouterr()
    assert "Row 1 processed successfully." in captured.out
