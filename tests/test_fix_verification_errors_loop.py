...
def test_non_python_no_verify_cmd_records_agentic_fallback_events(tmp_path, mocker):
    """Non-Python early return without a verify command must log fallback telemetry."""
    ...
    mocker.patch(
        "pdd.fix_verification_errors_loop._safe_run_agentic_verify",
        return_value=(True, "ok", 0.12, "agent-model", []),
    )
    ...
    assert events[0]["used"] is True
    assert "no verify command" in events[0]["detail"]


def test_non_python_after_verify_cmd_records_agentic_fallback_events(
    setup_test_environment, mocker,
):
    """Non-Python path that runs verify then agentic fallback must log telemetry."""
    env = setup_test_environment
    go_code_file = env["output_path"] / "code_module.go"
    go_code_file.write_text("package main\n\nfunc Run() string { return \"INITIAL\" }", encoding="utf-8")
    env["default_args"]["code_file"] = str(go_code_file)
