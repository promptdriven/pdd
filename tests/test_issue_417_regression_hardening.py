"""Regression hardening for PDD tag brace escaping in the preprocess pipeline.

Covers issue #417: ``preprocess(..., double_curly_brackets=True)`` followed by
``.format()`` must not crash, and PDD interface JSON must remain parseable
across includes, web/shell directives, and pathological brace content.
"""

# ``parse_prompt_tags`` returns ``Dict[str, Any]`` but pylint/astroid narrow the
# ``"interface"`` key to ``None`` from its initial assignment in the body. Tests
# legitimately subscript the runtime value (a dict), so silence the false
# positive once at module scope instead of decorating every site.
...
def _pdd_interface_payloads(text: str) -> list[dict]:
    """Return parsed JSON payloads for every ``<pdd-interface>`` tag in ``text``."""
...
def test_backtick_include_with_pdd_tags_is_format_safe(tmp_path):
    """Backtick-style includes carrying PDD tags survive ``.format()``."""
...
def test_nested_xml_includes_with_pdd_tags_are_format_safe(tmp_path):
    """Recursively expanded XML includes preserve nested PDD interface JSON."""
    leaf = tmp_path / "leaf.prompt"
    leaf.write_text(
        '<pdd-interface>{"type":"module",'
        '"module":{"functions":[{"name":"nested"}]}}</pdd-interface>',
        encoding="utf-8",
    )
...
def test_non_recursive_nested_xml_includes_with_pdd_tags_are_format_safe(tmp_path):
    """Non-recursive includes still emit format-safe PDD interface JSON."""
    leaf = tmp_path / "leaf.prompt"
...
def test_already_escaped_braces_in_pdd_content_keep_existing_preprocess_contract():
    """Already-escaped braces inside PDD content must not be double-escaped."""
...
def test_empty_or_whitespace_only_pdd_interface_tags_are_format_safe(prompt):
    """Empty or whitespace-only PDD interface tags round-trip through ``.format()``."""
    processed = preprocess(prompt, recursive=False, double_curly_brackets=True)
...
def test_pdd_tags_at_included_file_boundaries_are_format_safe(
    ...
    """PDD tags adjacent to include boundaries remain parseable after format."""
...
def test_unicode_in_pdd_interface_is_format_safe():
    """Unicode content and literal braces inside PDD JSON survive format."""
...
def test_real_file_include_with_pdd_tags_integration(tmp_path):
    """End-to-end integration with a real on-disk include."""
...
def test_full_pipeline_preprocess_then_parse_then_format(tmp_path):
    """Explicit preprocess -> parse_prompt_tags -> ``.format()`` ordering holds."""
...
def test_malformed_brace_sequences_inside_pdd_json_do_not_break_format(raw_value):
    """Malformed brace sequences inside JSON strings do not break ``.format()``."""
...
def test_pdd_tags_with_shell_and_web_tags_are_format_safe(monkeypatch):
    """Mixed ``<shell>`` and cached ``<web>`` directives co-exist with PDD tags."""
    class FakeCache:
        """Stub firecrawl cache: serves a fixed payload, refuses to write."""

        def get(self, url):
            """Return the canned PDD interface payload for the test URL."""
            assert url == "https://example.test/pdd"
            return '<pdd-interface>{"source":"web","payload":{"cached":true}}</pdd-interface>'

        def set(self, url, content):
            """Fail loudly if production tries to write through the cache stub."""
            raise AssertionError("cached web content should not be written")

    monkeypatch.setattr("pdd.preprocess.get_firecrawl_cache", FakeCache)
    prompt = (
