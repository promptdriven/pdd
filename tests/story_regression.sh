...
echo "[story-regression] repo: $REPO_ROOT"
echo "[story-regression] log: $LOG_FILE"
...
echo "[story-regression] Running: pytest -m story (deterministic/offline)"

# Run single-process (no xdist) so the per-story summary in conftest.py
# aggregates across the whole session.
set +e
python -m pytest tests/ -m story --no-header -p no:cacheprovider "$@"
status=$?
set -e
...
if [ "$status" -eq 5 ]; then
  echo "[story-regression] No story-marked tests collected yet; lane is green (nothing to run)."
  exit 0
fi

if [ "$status" -ne 0 ]; then
  echo "[story-regression] ERROR: story regression lane failed (exit $status)" >&2
  exit "$status"
fi

echo "[story-regression] Story regression lane completed successfully"
