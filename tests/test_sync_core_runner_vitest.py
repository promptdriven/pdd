...
def test_real_vitest_workflow_uses_checked_in_locked_toolchain() -> None:
    ...
    provision_start = workflow.index("- name: Provision identity-bound Vitest toolchain")
    provision_end = workflow.index(
        "- name: Provision identity-bound Playwright Chromium toolchain",
        provision_start,
    )
    vitest_provision = workflow[provision_start:provision_end]
...
def _preload_off_direct_fd_probe() -> str:
    ...
    return (
        ...
        "  let observed;\n"
        "  try { observed = fs.fstatSync(198, {bigint: true}); }\n"
        "  catch (error) {\n"
        "    if (['EBADF', 'ENOENT'].includes(error?.code)) return;\n"
        "    throw new Error('preload authority direct-fd inspect failed');\n"
        "  }\n"
        "  const isExpectedPipe = (observed.mode & 0o170000n) === 0o010000n &&\n"
        "    observed.dev === expected.device && observed.ino === expected.inode;\n"
        "  if (!isExpectedPipe) {\n"
        "    throw new Error('preload authority direct-fd unexpected');\n"
        "  }\n"
