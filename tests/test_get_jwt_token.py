...
# =============================================================================
# Issue #404: Keyring operations hang indefinitely in CI/CD/SSH environments
# =============================================================================
...
# Single hang duration shared by all timeout tests. Significantly longer than
# any reasonable timeout (5s macOS / 10s elsewhere) so that a passing test
# proves the timeout wrapper kicked in. Short enough that a buggy run still
# fails within ~30s rather than hanging "forever" in CI.
_HANG_SECONDS = 30

# Upper bound the test allows for any single keyring op. Must be larger than
# the largest expected timeout (10s on Linux/Windows) plus scheduling slack,
# but well under _HANG_SECONDS so a buggy run is unambiguously caught.
_TIMEOUT_BUDGET = 15
...
class TestKeyringTimeoutProtection:
    """
    Tests for Issue #404: Keyring operations must not hang indefinitely.
    ...
    returns within ~5-10s and yields False/None.
    ...
    def test_set_password_hangs_indefinitely_in_ci_environment(self, mock_keyring):
        ...
        Reproduces the line 360 hang inside `_store_refresh_token`. The
    ...
    def test_get_password_hangs_indefinitely_when_retrieving_token(self, mock_keyring):
        ...
        Reproduces the line 401 hang inside `_get_stored_refresh_token`.
    ...
    def test_delete_password_hangs_indefinitely_during_cleanup(self, mock_keyring):
        ...
        Reproduces the line 418 hang inside `_delete_stored_refresh_token`.
