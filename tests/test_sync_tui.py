...
def test_steering_logic_safety_z3():
    ...
    """
    from z3 import Solver, Or, And, Not, Implies, String, Bool, EnumSort, Const, unsat
...
def test_z3_maybe_steer_filtering_logic():
    ...
    """
    from z3 import Solver, Or, And, Not, Implies, Bool, EnumSort, Const, unsat
    ...
    assert solver.check() == unsat, "Found a case where skip_tests failed to prevent switching to test/fix"
    solver.pop()
    ...
    assert solver.check() == unsat, "Found a case where skip_verify failed to prevent switching to verify"
    solver.pop()
