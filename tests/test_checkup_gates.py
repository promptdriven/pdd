...
class TestDocContractCheck:
    ...
    def test_diff_parser_includes_lines_starting_with_plus_plus(self, tmp_path: Path) -> None:
        """Regression: added lines whose content starts with '++' must NOT be filtered out
        by the diff parser. (Issue #1422 fix — guard was not line.startswith('+++') without
        the trailing space, incorrectly dropping lines like '+++foo'.)"""
        from pdd.checkup_gates import run_doc_contract_check

        _git_init(tmp_path)
        (tmp_path / "README.md").write_text("# README\n--plus-plus-opt option\n", encoding="utf-8")
        (tmp_path / "cli.py").write_text("import click\n@click.command()\n", encoding="utf-8")

        subprocess.run(["git", "add", "."], cwd=tmp_path, check=True)
        subprocess.run(["git", "-c", "user.name=t", "-c", "user.email=t@x",
                        "commit", "-m", "init", "-q"], cwd=tmp_path, check=True)

        # Add a Click option whose name starts with '--' so the diff line starts with '++--'
        (tmp_path / "cli.py").write_text(
            "import click\n@click.command()\n@click.option('--plus-plus-opt')\n",
            encoding="utf-8",
        )
        # README documents the option — gate should pass (line correctly parsed)
        result = run_doc_contract_check(tmp_path, base_ref="HEAD~1")
        assert result == 0, (
            "Doc-contract gate failed even though the option is documented — "
            "likely the '++' diff line was incorrectly filtered out"
        )

    def test_diff_parser_handles_filename_with_spaces(self, tmp_path: Path) -> None:
        """Regression: diff --git header with a filename containing spaces must capture the
        full filename, not truncate at the first space. (Issue #1422 fix)"""
        import re

        # Standard git diff --git header (unquoted, space-separated a/ and b/ paths)
        diff_text = (
            "diff --git a/pdd/my module.py b/pdd/my module.py\n"
            "--- a/pdd/my module.py\n"
            "+++ b/pdd/my module.py\n"
            "@@ -1 +1 @@\n"
            "+def hello(): pass\n"
        ...
        # The full filename (with space) must be the key
        assert "pdd/my module.py" in added_lines_by_file, (
            f"Filename with space was truncated. Keys found: {list(added_lines_by_file)}"
        )
        assert added_lines_by_file["pdd/my module.py"] == ["def hello(): pass"], (
            "Added lines were not correctly attributed to the spaced filename"
        )
