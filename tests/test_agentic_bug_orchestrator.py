...
# ============================================================================
# Additional coverage: helper functions not yet directly tested
# ============================================================================
...
class TestParsePatternSearch:
    """Tests for _parse_pattern_search helper."""

    def test_returns_none_when_no_marker(self):
        from pdd.agentic_bug_orchestrator import _parse_pattern_search
        assert _parse_pattern_search("no marker here") is None

    def test_extracts_simple_pattern(self):
        from pdd.agentic_bug_orchestrator import _parse_pattern_search
        assert _parse_pattern_search("PATTERN_SEARCH: foo\\.bar") == "foo\\.bar"
    ...
    def test_strips_trailing_comment(self):
        from pdd.agentic_bug_orchestrator import _parse_pattern_search
        result = _parse_pattern_search("PATTERN_SEARCH: \\.glob\\( # broad pattern")
        assert result == "\\.glob\\("

    def test_preserves_hash_in_character_class(self):
        from pdd.agentic_bug_orchestrator import _parse_pattern_search
        # Hash without surrounding spaces must be preserved
        result = _parse_pattern_search("PATTERN_SEARCH: [a-z#]+")
        assert result == "[a-z#]+"

    def test_returns_none_for_empty_pattern(self):
        from pdd.agentic_bug_orchestrator import _parse_pattern_search
        assert _parse_pattern_search("PATTERN_SEARCH: ") is None


class TestSanitizeGrepPattern:
    """Tests for _sanitize_grep_pattern helper."""

    def test_accepts_valid_pattern(self):
        from pdd.agentic_bug_orchestrator import _sanitize_grep_pattern
        assert _sanitize_grep_pattern("foo\\.bar") == "foo\\.bar"

    def test_rejects_too_short(self):
        from pdd.agentic_bug_orchestrator import _sanitize_grep_pattern
        assert _sanitize_grep_pattern("ab") is None

    def test_rejects_pure_wildcards(self):
        from pdd.agentic_bug_orchestrator import _sanitize_grep_pattern
    ...
    def test_rejects_shell_metacharacters(self):
        from pdd.agentic_bug_orchestrator import _sanitize_grep_pattern
        assert _sanitize_grep_pattern("foo;rm -rf") is None
        assert _sanitize_grep_pattern("foo`whoami`") is None
        assert _sanitize_grep_pattern("foo$bar") is None

    def test_allows_pipe_alternation(self):
        from pdd.agentic_bug_orchestrator import _sanitize_grep_pattern
        assert _sanitize_grep_pattern("foo|bar") == "foo|bar"
...
class TestParseE2eNeededMarker:
    """Tests for _parse_e2e_needed_marker helper."""

    def test_returns_none_when_absent(self):
        from pdd.agentic_bug_orchestrator import _parse_e2e_needed_marker
        assert _parse_e2e_needed_marker("no marker") is None

    def test_parses_yes(self):
        from pdd.agentic_bug_orchestrator import _parse_e2e_needed_marker
        assert _parse_e2e_needed_marker("E2E_NEEDED: yes") == "yes"

    def test_parses_no(self):
        from pdd.agentic_bug_orchestrator import _parse_e2e_needed_marker
        assert _parse_e2e_needed_marker("E2E_NEEDED: no") == "no"

    def test_ignores_marker_inside_step_report(self):
        from pdd.agentic_bug_orchestrator import _parse_e2e_needed_marker
        output = "<step_report>E2E_NEEDED: yes</step_report>"
        assert _parse_e2e_needed_marker(output) is None

    def test_last_marker_outside_wins(self):
        from pdd.agentic_bug_orchestrator import _parse_e2e_needed_marker
        output = "E2E_NEEDED: yes\n<step_report>x</step_report>\nE2E_NEEDED: no"
        assert _parse_e2e_needed_marker(output) == "no"


class TestStateSafeStepOutput:
    """Tests for _state_safe_step_output helper (secret redaction)."""

    def test_passes_through_normal_output(self):
        from pdd.agentic_bug_orchestrator import _state_safe_step_output
        result = _state_safe_step_output("normal output")
        assert "normal output" in result

    def test_handles_non_string(self):
        from pdd.agentic_bug_orchestrator import _state_safe_step_output
        # Non-strings returned unchanged
        assert _state_safe_step_output(None) is None


class TestExtractMatchContext:
    """Tests for _extract_match_context helper."""

    def test_extracts_context_window(self, tmp_path):
        from pdd.agentic_bug_orchestrator import _extract_match_context
        f = tmp_path / "code.py"
        lines = [f"line {i}" for i in range(50)]
        f.write_text("\n".join(lines))
        result = _extract_match_context(
            [("code.py", 25, "line 24")], tmp_path,
            context_before=3, context_after=2,
        )
        assert "code.py" in result
        # Window should cover lines around 25
        assert "line 24" in result["code.py"]
    ...
    def test_merges_overlapping_windows(self, tmp_path):
        ...
        result = _extract_match_context(
            [("code.py", 10, "x"), ("code.py", 12, "y")],
            tmp_path, context_before=3, context_after=3,
        )
        # Should merge into single section (no "..." separator)
        assert "..." not in result["code.py"]

    def test_handles_missing_file(self, tmp_path):
        from pdd.agentic_bug_orchestrator import _extract_match_context
        result = _extract_match_context(
            [("missing.py", 1, "x")], tmp_path,
        )
        assert result["missing.py"] == ""


class TestParseClassificationEvidence:
    """Tests for _parse_classification_evidence helper."""

    def test_parses_needs_fix_and_safe(self):
        from pdd.agentic_bug_orchestrator import _parse_classification_evidence
        output = (
            "NEEDS_FIX: pdd/a.py | shares bug\n"
            "SAFE_EVIDENCE: pdd/b.py | 42 | guarded\n"
        )
        needs_fix, safe = _parse_classification_evidence(output)
        assert needs_fix == ["pdd/a.py"]
        assert safe == ["pdd/b.py"]

    def test_deduplicates_across_markers(self):
        from pdd.agentic_bug_orchestrator import _parse_classification_evidence
        output = (
            "NEEDS_FIX: pdd/a.py\n"
            "NEEDS_FIX: pdd/a.py\n"
    ...
    def test_returns_empty_lists_when_absent(self):
        from pdd.agentic_bug_orchestrator import _parse_classification_evidence
        needs_fix, safe = _parse_classification_evidence("nothing here")
        assert needs_fix == []
        assert safe == []


class TestMergeFixLocations:
    """Tests for _merge_fix_locations helper."""

    def test_preserves_originals(self):
        from pdd.agentic_bug_orchestrator import _merge_fix_locations
        result = _merge_fix_locations(["pdd/a.py"], [], ["pdd/a.py"], [])
        assert "pdd/a.py" in result

    def test_adds_needs_fix(self):
        from pdd.agentic_bug_orchestrator import _merge_fix_locations
        result = _merge_fix_locations(["pdd/a.py"], ["pdd/b.py"], [], ["pdd/b.py"])
        assert "pdd/a.py" in result
        assert "pdd/b.py" in result

    def test_unclassified_defaults_to_needs_fix(self):
        from pdd.agentic_bug_orchestrator import _merge_fix_locations
        result = _merge_fix_locations(["pdd/a.py"], [], [], ["pdd/c.py"])
        assert "pdd/c.py" in result

    def test_excludes_value_level_siblings(self):
        from pdd.agentic_bug_orchestrator import _merge_fix_locations
        # Value-level siblings like extension:.htm must not enter FIX_LOCATIONS
        result = _merge_fix_locations(
            ["pdd/a.py"], ["extension:.htm"], [], [],
        )
        assert "extension:.htm" not in result

    def test_safe_evidence_excluded_when_not_in_originals(self):
        from pdd.agentic_bug_orchestrator import _merge_fix_locations
        # File classified SAFE and not in originals is excluded
        result = _merge_fix_locations(
            ["pdd/a.py"], [], ["pdd/safe.py"], ["pdd/safe.py"],
        )
        assert "pdd/safe.py" not in result


class TestIsFixLocationPath:
    """Tests for _is_fix_location_path helper."""

    def test_accepts_source_path(self):
        from pdd.agentic_bug_orchestrator import _is_fix_location_path
        assert _is_fix_location_path("pdd/foo.py") is True

    def test_accepts_bare_filename(self):
        from pdd.agentic_bug_orchestrator import _is_fix_location_path
        assert _is_fix_location_path("foo.py") is True

    def test_rejects_value_level_id(self):
        from pdd.agentic_bug_orchestrator import _is_fix_location_path
        assert _is_fix_location_path("extension:.htm") is False
        assert _is_fix_location_path("command:resume") is False

    def test_rejects_empty(self):
        from pdd.agentic_bug_orchestrator import _is_fix_location_path
        assert _is_fix_location_path("") is False
        assert _is_fix_location_path("  ") is False


class TestIsLlmClassified:
    """Tests for _is_llm_classified helper."""

    def test_exact_path_in_needs_fix(self):
        from pdd.agentic_bug_orchestrator import _is_llm_classified
        assert _is_llm_classified("pdd/a.py", ["pdd/a.py"], [], ["pdd/a.py"]) is True

    def test_exact_path_in_safe(self):
        from pdd.agentic_bug_orchestrator import _is_llm_classified
        assert _is_llm_classified("pdd/a.py", [], ["pdd/a.py"], ["pdd/a.py"]) is True

    def test_unclassified_returns_false(self):
        from pdd.agentic_bug_orchestrator import _is_llm_classified
        assert _is_llm_classified("pdd/c.py", ["pdd/a.py"], ["pdd/b.py"], ["pdd/c.py"]) is False

    def test_bare_basename_unambiguous(self):
        from pdd.agentic_bug_orchestrator import _is_llm_classified
        # Bare "foo.py" classification covers single grep file pdd/foo.py
        assert _is_llm_classified("pdd/foo.py", [], ["foo.py"], ["pdd/foo.py"]) is True

    def test_bare_basename_ambiguous_not_classified(self):
        from pdd.agentic_bug_orchestrator import _is_llm_classified
        # Bare "foo.py" with multiple grep matches → unclassified
        assert _is_llm_classified(
            "pdd/foo.py", [], ["foo.py"], ["pdd/foo.py", "tests/foo.py"],
        ) is False

# ============================================================================
# Additional helper coverage (appended)
# ============================================================================
...
class TestGetCodeExtensions:
    def test_returns_glob_patterns(self):
        exts = _get_code_extensions()
        assert isinstance(exts, list)
        assert len(exts) > 0
        # Every entry should be a glob pattern
        assert all(e.startswith("*.") for e in exts)

    def test_excludes_data_extensions(self):
        exts = _get_code_extensions()
        # Data/config extensions should not appear
...
class TestVerifyPatternCompleteness:
    def test_rejects_invalid_pattern(self, tmp_path):
        matches, summary = _verify_pattern_completeness(".*", ["pdd/a.py"], tmp_path)
        assert matches == []
        assert "rejected" in summary.lower() or "invalid" in summary.lower()

    def test_finds_unclassified_matches(self, tmp_path):
        # Create two python files; one in fix_locations, one not
        (tmp_path / "pdd").mkdir()
        (tmp_path / "pdd" / "a.py").write_text("def foo_unique_pattern_xyz(): pass\n")
        (tmp_path / "pdd" / "b.py").write_text("def foo_unique_pattern_xyz(): pass\n")

        matches, summary = _verify_pattern_completeness(
            "foo_unique_pattern_xyz", ["pdd/a.py"], tmp_path
        )
        # b.py should be unclassified
        unclassified_files = {m[0] for m in matches}
        assert any("b.py" in f for f in unclassified_files)
        assert not any(f == "pdd/a.py" for f in unclassified_files)

    def test_no_matches_returns_empty(self, tmp_path):
        (tmp_path / "x.py").write_text("def something(): pass\n")
        matches, summary = _verify_pattern_completeness(
            "no_such_pattern_zzz_qqq", [], tmp_path
        )
        assert matches == []
        assert "0" in summary or "found" in summary.lower()


class TestMaybePostStepComment:
    def test_success_posts_and_marks_state(self, tmp_path):
        state = {"step_outputs": {}, "step_comments": {}}
        with patch(
            ...
            _maybe_post_step_comment(
                ...
                step_output="<step_report>found nothing</step_report>",
                state=state,
                state_dir=tmp_path,
                cwd=tmp_path,
                issue_number=1,
        ...
        assert state["step_comments"]["1"]["posted"] is True

    def test_already_posted_skips(self, tmp_path):
        state = {
            "step_outputs": {},
        ...
        with patch(
            ...
            _maybe_post_step_comment(
                ...
                step_num=1,
                description="Duplicate check",
                step_output="<step_report>x</step_report>",
                state=state,
                state_dir=tmp_path,
                cwd=tmp_path,
                issue_number=1,
    ...
    def test_failure_posts_fallback_without_setting_posted(self, tmp_path):
        state = {"step_outputs": {}, "step_comments": {}}
        with patch(
            "pdd.agentic_bug_orchestrator.post_step_comment", return_value=True
        ) as mock_post, patch(
        ...
        assert mock_post.called
        entry = state["step_comments"]["2"]
        # Failure path should NOT set 'posted' flag — allows retry to repost
        assert entry.get("posted") is not True
        assert "failed_posted" in entry

    def test_corrupted_step_comments_normalized(self, tmp_path):
        # Non-dict step_comments must be normalized to dict
        state = {"step_outputs": {}, "step_comments": ["bogus", "list"]}
        with patch(
            "pdd.agentic_bug_orchestrator.post_step_comment", return_value=True
        ), patch(
            "pdd.agentic_bug_orchestrator.save_workflow_state", return_value=None
            ...
            _maybe_post_step_comment(
                ...
                step_num=1,
                description="x",
                step_output="<step_report>ok</step_report>",
                state=state,
                state_dir=tmp_path,
                cwd=tmp_path,
                issue_number=1,
        ...
        assert isinstance(state["step_comments"], dict)


class TestScanStep9FileForNewPatterns:
    def test_no_pre_snapshot_full_scan(self, tmp_path):
        f = tmp_path / "test_new.py"
        f.write_text("def test_x():\n    assert hasattr(m, 'f')\n")
        violations = _scan_step9_file_for_new_patterns(f, pre_snapshot=None)
        assert len(violations) > 0
    ...
    def test_unchanged_file_returns_empty(self, tmp_path):
        ...
        violations = _scan_step9_file_for_new_patterns(f, pre_snapshot=content)
        assert violations == []

    def test_pure_append_scans_only_new_lines(self, tmp_path):
        pre = "def test_existing():\n    assert hasattr(m, 'f')\n"
        appended = pre + "\ndef test_new():\n    assert True\n"
        f = tmp_path / "t.py"
        f.write_text(appended)
        violations = _scan_step9_file_for_new_patterns(f, pre_snapshot=pre)
        # Pre-existing hasattr violation should NOT be reported
        assert violations == []

    def test_rewrite_subtracts_preexisting(self, tmp_path):
        # Pre had one violation; rewrite has the same violation but reorganized
        pre = "def test_a():\n    assert hasattr(m, 'f')\n"
        rewritten = (
            "import x\n"
            "def test_a():\n"
        ...
        violations = _scan_step9_file_for_new_patterns(f, pre_snapshot=pre)
        # The preexisting hasattr should be filtered out
        assert violations == []

    def test_rewrite_with_new_violation(self, tmp_path):
        pre = "def test_a():\n    assert True\n"
        rewritten = "def test_b():\n    assert hasattr(m, 'f')\n"
        f = tmp_path / "t.py"
        f.write_text(rewritten)
        violations = _scan_step9_file_for_new_patterns(f, pre_snapshot=pre)
        assert len(violations) > 0
    ...
    def test_missing_file_returns_empty(self, tmp_path):
        f = tmp_path / "ghost.py"
        violations = _scan_step9_file_for_new_patterns(f, pre_snapshot="something")
        assert violations == []

# ============================================================================
# Additional targeted coverage (appended)
# ============================================================================
...
from pdd.agentic_bug_orchestrator import (
    ...
    _parse_scope_classification,
    _extract_click_command_names,
    _cli_test_covers_command_module,
    _verify_fix_location_coverage,
    detect_structural_test_patterns,
)


class TestMergeFixLocationsBasenameSemantics:
    def test_full_path_not_covered_by_different_dir_same_basename(self):
        """pdd/foo.py in merged should NOT cover tests/foo.py (different dirs)."""
        result = _merge_fix_locations(
            ["pdd/foo.py"], ["tests/foo.py"], [], ["tests/foo.py"]
        )
        assert "pdd/foo.py" in result
        assert "tests/foo.py" in result

    def test_bare_name_in_merged_covers_full_path(self):
        """Bare 'foo.py' in originals covers a needs_fix entry 'pdd/foo.py'."""
        result = _merge_fix_locations(
            ["foo.py"], ["pdd/foo.py"], [], []
        )
        # bare name already covers
        assert "foo.py" in result


class TestParseNeedsFixEdgeCases:
    def test_skips_empty_lines(self):
        assert _parse_needs_fix("NEEDS_FIX: \nNEEDS_FIX:   ") == []

    def test_strips_backticks(self):
        assert _parse_needs_fix("NEEDS_FIX: `pdd/x.py` | reason") == [("pdd/x.py", "reason")]


class TestMergeNeedsFixIntoExpansion:
    def test_returns_none_for_empty_inputs(self):
        assert _merge_needs_fix_into_expansion("none", []) == "none"
        assert _merge_needs_fix_into_expansion("", []) == "none"

    def test_appends_new_sibling(self):
        result = _merge_needs_fix_into_expansion("none", [("pdd/a.py", "shares helper")])
        assert "pdd/a.py" in result
        assert "shares helper" in result


class TestApplyStep6ScopeMarkersCaseInsensitive:
    def test_lowercase_classification_normalized(self):
        ctx = {"step6_expansion_items": "none"}
        result = _apply_step6_scope_markers(
            "SCOPE_CLASSIFICATION: cross_cutting", ctx
        )
        assert result == "CROSS_CUTTING"


class TestParseExpansionItemsLastMarkerWins:
    def test_last_marker_overrides(self):
        output = "EXPANSION_ITEMS: a\nEXPANSION_ITEMS: b, c"
        result = _parse_expansion_items(output)
        assert result == "b, c"

    def test_explicit_none_returns_none(self):
        assert _parse_expansion_items("EXPANSION_ITEMS: none") == "none"


class TestExtractClickCommandNames:
    def test_extracts_positional_name(self, tmp_path):
        f = tmp_path / "cmd.py"
        f.write_text('import click\n@click.command("example")\ndef cmd(): pass\n')
        assert _extract_click_command_names(f) == ["example"]

    def test_extracts_name_kwarg(self, tmp_path):
        f = tmp_path / "cmd.py"
        f.write_text('import click\n@click.command(name="gen")\ndef cmd(): pass\n')
        assert _extract_click_command_names(f) == ["gen"]

    def test_missing_file_returns_empty(self, tmp_path):
        assert _extract_click_command_names(tmp_path / "nope.py") == []


class TestCliTestCoversCommandModule:
    def test_returns_false_for_non_command_module(self, tmp_path):
        assert _cli_test_covers_command_module("pdd/foo.py", "anything", tmp_path) is False

    def test_returns_false_without_clirunner(self, tmp_path):
        cmd = tmp_path / "pdd" / "commands" / "gen.py"
        cmd.parent.mkdir(parents=True)
        cmd.write_text('import click\n@click.command("gen")\ndef gen(): pass\n')
        assert _cli_test_covers_command_module(
            "pdd/commands/gen.py", "from pdd import cli", tmp_path
        ) is False


class TestVerifyFixLocationCoverageSingleFile:
    def test_single_location_returns_empty(self, tmp_path):
        assert _verify_fix_location_coverage(["pdd/only.py"], [], tmp_path) == []


class TestDetectStructuralPatternsStartLine:
    def test_start_line_past_end_suppresses_all(self, tmp_path):
        f = tmp_path / "t.py"
        f.write_text("def test_x():\n    assert hasattr(m, 'f')\n")
        # start_line past EOF must suppress (issue #1064)
        violations = detect_structural_test_patterns(str(f), start_line=999)
        assert violations == []

    def test_inspect_signature_detected(self, tmp_path):
        f = tmp_path / "t.py"
    ...
    def test_comments_not_flagged(self, tmp_path):
        f = tmp_path / "t.py"
        f.write_text("# inspect.getsource is fine in a comment\ndef test_x():\n    assert True\n")
        assert detect_structural_test_patterns(str(f)) == []