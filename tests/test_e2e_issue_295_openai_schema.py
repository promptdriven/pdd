...
def _isolate_pdd_model_default(monkeypatch):
    ...
    These tests mock ``_load_model_data`` to return an OpenAI-only fixture. If
    the surrounding environment exports a prefixed default like
    ``PDD_MODEL_DEFAULT=vertex_ai/gemini-3-flash-preview`` (Cloud Batch CI, dev
    shells), the provider-lock added by #1113 engages at selection time and
    raises ``ValueError`` before the test's behaviour-under-test fires —
    because the mocked CSV has no Vertex rows. Clearing both the env var and
    the module-level ``DEFAULT_BASE_MODEL`` (captured at import time) keeps
    the tests hermetic against external env, mirroring the same fixture in
