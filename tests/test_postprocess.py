...
def mock_dependencies(monkeypatch):
    ...
    monkeypatch.setattr("pdd.postprocess.llm_selector", lambda x, y: (None, lambda z: len(z), 0.01, 0.02, "mock_model"))
...
def test_postprocess_normal_execution(mock_dependencies, mock_console):
    ...
    mock_chain.invoke.return_value = {"extracted_code": "def test():\n    pass"}
    with patch("builtins.open", mock_open(read_data="Mock prompt template")):
        with patch("pdd.postprocess.chain", mock_chain):
...
def test_postprocess_remove_backticks(mock_dependencies, mock_console):
    mock_chain = MagicMock()
    mock_chain.invoke.return_value = {"extracted_code": "```python\ndef test():\n    pass\n```"}
    with patch("builtins.open", mock_open(read_data="Mock prompt template")):
        with patch("pdd.postprocess.chain", mock_chain):
            result, cost = postprocess("Test input", "python")
