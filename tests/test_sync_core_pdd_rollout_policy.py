...
CI_DETECT_REQUIREMENT_ROTATION = {
    ...
    "head_policy_sha256": (
        "89bf95f75b56fc4c85130c759692183de2925844b814997e06fd64a5166fac95"
    ),
    "base_prompt_sha256": (
...
def test_pr1790_rotations_equal_exact_dormant_bootstrap_authority() -> None:
    """PR 1790 rules exactly match code trust roots and remain future-only."""
    policy = json.loads(ROTATION_FILE.read_text(encoding="utf-8"))
    ...
    policy_rows = {
        (row["prompt_path"], row["language_id"]): row for row in pr1790_rows
    }
    expected_rows = {
        identity: row
        for identity, row in bootstrap_rows.items()
        if row["head_policy_sha256"]
        == "e451dc7b076388f184e8c9f5f4f89c93a027bcf1d666f5c96b3767f76cb22af5"
    }
    ...
    for row in pr1790_rows:
        ...
        assert profile_rows[(row["prompt_path"], row["language_id"])][
...
def test_pr1790_bootstrap_transition_bindings_fail_closed(
    ...
    rows = json.loads(ROTATION_FILE.read_text(encoding="utf-8"))[
        "requirement_rotations"
    ]
    row = next(
        item
        for item in rows
        if item["head_policy_sha256"]
        == "e451dc7b076388f184e8c9f5f4f89c93a027bcf1d666f5c96b3767f76cb22af5"
    )
