...
def test_check_cost_limits_enforces_ctx_items_when_max_cost_none(tmp_path: Path) -> None:
    """Regression: _check_cost_limits must check nondeterministic_context_items even when
    max_cost_usd is None. (Issue #1422 fix)"""
    from unittest.mock import MagicMock
    from pdd.gate_main import _check_cost_limits
    from pdd.gate_policy import GatePolicy, GateLimits

    manifest = MagicMock()
    manifest.basename = "test_unit"
    manifest.generation = {"nondeterministic_context_items": 50, "cost_usd": 0.0}

    policy = GatePolicy(
        require={},
        allow={},
        limits=GateLimits(max_cost_usd=None, max_nondeterministic_context_items=10),
    )

    failures = _check_cost_limits(manifest, policy)
    codes = [f.code for f in failures]
    assert "nondeterministic_context_limit" in codes, (
        "_check_cost_limits skipped context-items check when max_cost_usd=None"
    )


def test_missing_evidence_produces_single_failure(tmp_path: Path) -> None:
    """Regression: gate should emit exactly one missing_evidence failure for an absent
    manifest — not redundant policy-violation failures from evaluate_manifest on the stub.
    (Issue #1422 fix)"""
    project = tmp_path
    # resolve_prompt_path looks under project_root/prompts/ for <basename>_*.prompt
    (project / "prompts").mkdir(parents=True)
    (project / "prompts" / "myunit_python.prompt").write_text("# prompt\n", encoding="utf-8")

    result = run_gate_policy(project, target="myunit")
    codes = [f.code for f in result.failures]
    assert "missing_evidence" in codes, f"Expected a missing_evidence failure, got {codes}"
    extra = [c for c in codes if c.endswith("_missing")]
    assert not extra, (
        f"evaluate_manifest was called on the stub and produced redundant failures: {extra}"
    )
