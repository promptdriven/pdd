...
# This reachable ancestor preserves the exact profile/prompt bytes bound by the
# estimate rollout.  The checked-out profile has since advanced through
# independent runner rotations, so it is not the estimate transition's base.
ESTIMATE_HISTORICAL_BASE_REVISION = "e50f0213b10362c528b96276961870e89e488452"
EstimateState = tuple[bytes, dict[str, bytes]]


def _historical_bytes(path: str) -> bytes:
    """Load one exact blob from the estimate rollout's historical base tree."""

    return subprocess.check_output(
        ["git", "show", f"{ESTIMATE_HISTORICAL_BASE_REVISION}:{path}"], cwd=ROOT
    )
