...
ESTIMATE_REQUIREMENT_ROTATIONS = (
    {
        ...
        "head_policy_sha256": (
            "795e9d84d5ca82e586796cb6688962fa7eb4caeab972d16bcd087d0733f14d07"
        ),
        "base_prompt_sha256": (
...
def test_estimate_contract_rotations_are_exact_and_dormant(monkeypatch) -> None:
    ...
    assert (
        hashlib.sha256(PROFILE_FILE.read_bytes()).hexdigest()
        == (ESTIMATE_REQUIREMENT_ROTATIONS[0]["base_policy_sha256"])
    )
    assert (
        hashlib.sha256(target_profile).hexdigest()
