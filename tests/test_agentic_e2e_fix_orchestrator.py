...
class TestPromptNotABugCategory:
    ...
    def test_orchestrator_prompt_documents_direct_edit_guard(self):
        ...
        Without this, a future regeneration could drop the direct-edit check
        and re-introduce the false "not a bug" exit for non-PDD-managed
        module fixes.
        """
        prompt_path = Path(__file__).parent.parent / "prompts" / "agentic_e2e_fix_orchestrator_python.prompt"
        if not prompt_path.exists():
            pytest.skip("Prompt file not available in public repo")
        template = prompt_path.read_text()
        assert "_detect_meaningful_changes" in template, (
            "Orchestrator prompt must reference `_detect_meaningful_changes` "
            "so the NOT_A_BUG direct-edit guard can't be silently dropped."
        )
        assert "has_direct_edits" in template, (
            "Orchestrator prompt must describe `has_direct_edits` as the "
            "NOT_A_BUG suppression signal for non-PDD-managed module edits."
        )

    def test_orchestrator_prompt_documents_noise_filter(self):
        """Issue #1206: orchestrator prompt must document the noise filter so
        build/cache artifacts never silently suppress a legitimate NOT_A_BUG
        exit (e.g. a stray `__pycache__/*.pyc` write during the workflow).
        """
        prompt_path = Path(__file__).parent.parent / "prompts" / "agentic_e2e_fix_orchestrator_python.prompt"
        if not prompt_path.exists():
            pytest.skip("Prompt file not available in public repo")
        template = prompt_path.read_text()
        assert "_is_noise_path" in template, (
            "Orchestrator prompt must reference `_is_noise_path` so the "
            "noise filter can't be silently dropped from future regenerations."
        )
        for token in ("__pycache__", ".pytest_cache", "node_modules"):
            assert token in template, (
                f"Orchestrator prompt must list `{token}` in the noise filter "
                "so build/cache artifacts cannot silently suppress NOT_A_BUG."
            )

    def test_orchestrator_prompt_documents_cycle_waste_breaker(self):
        """Issue #1206: orchestrator prompt must document the cycle-waste
        breaker so a prior-cycle direct edit plus no current progress exits
        cleanly instead of looping to max_cycles.
        """
        prompt_path = Path(__file__).parent.parent / "prompts" / "agentic_e2e_fix_orchestrator_python.prompt"
        if not prompt_path.exists():
            pytest.skip("Prompt file not available in public repo")
        template = prompt_path.read_text()
        assert "cycle_start_hashes" in template, (
            "Orchestrator prompt must reference `cycle_start_hashes` so the "
            "cycle-waste breaker's cycle-local progress check is preserved."
        )
...
class TestIssue467BlindResume:
    ...
    def mock_deps(self, tmp_path):
        ...
        with patch("pdd.agentic_e2e_fix_orchestrator.run_agentic_task") as mock_run, \
             ...
             patch("pdd.agentic_e2e_fix_orchestrator._get_file_hashes") as mock_hashes, \
             patch("pdd.agentic_e2e_fix_orchestrator._commit_and_push") as mock_commit, \
             patch("pdd.agentic_e2e_fix_orchestrator.classify_step_output") as mock_classify:
            mock_run.return_value = (True, "Step Output", 0.1, "gpt-4")
...
class TestStep11CodeCleanup:
    ...
    def test_fix_flow_does_not_invoke_agentic_checkup(
        self, e2e_fix_mock_dependencies, e2e_fix_default_args
    ):
        """pdd fix's final gate must NOT invoke the agentic pdd
        checkup (no run_agentic_checkup, no review-loop). Verification is the fix
        workflow's own steps (Step 7/9) plus the lean deterministic pre-checkup
        gate; `pdd checkup` stays a separate command. The run succeeds on the
        gate alone — a transient checkup verdict can no longer fail an
...
class TestNotABugGuardNoiseFilter:
    ...
    def test_is_noise_path_detects_extended_artifacts(self):
        from pdd.agentic_e2e_fix_orchestrator import _is_noise_path

        # Coverage in parallel mode and XML report
        assert _is_noise_path(".coverage.host.123.456789")
        assert _is_noise_path("coverage.xml")
        assert _is_noise_path("nested/coverage.xml")
        # Python build artifacts
        assert _is_noise_path("mypackage.egg-info/PKG-INFO")
        assert _is_noise_path(".eggs/some-pkg.egg/x.py")
        # JS/web tooling caches
        assert _is_noise_path(".next/static/chunk.js")
        assert _is_noise_path(".turbo/cache/x")
        assert _is_noise_path(".parcel-cache/data")
        # Real coverage-named source files should NOT be filtered
        assert not _is_noise_path("coverage_helper.py")
        assert not _is_noise_path("src/.coverage_utils.py")
...
class TestNotABugGuardResumeSnapshot:
    """The NOT_A_BUG direct-edit guard must keep working across workflow
    resumes. The workflow-start file snapshot has to survive interruption,
    otherwise the original false-failure exit recurs on resume.
    """

    def test_resume_restores_initial_file_hashes(
        self, e2e_fix_mock_dependencies, e2e_fix_default_args, monkeypatch
    ):
        """On resume, the saved initial_file_hashes must be used so direct
        edits made by prior cycles still register.
        """
        mock_run, _, _ = e2e_fix_mock_dependencies
        e2e_fix_default_args["resume"] = True
        e2e_fix_default_args["max_cycles"] = 3

        saved_snapshot = {"src/before.py": "abc123"}
        loaded_state = {
            "current_cycle": 2,
            ...
            "total_cost": 0.05,
            "model_used": "gpt-4",
            "changed_files": ["src/direct_fix.py"],
            "dev_unit_states": {},
            "skipped_steps": {},
            "initial_file_hashes": saved_snapshot,
            "initial_sha": "deadbeef",
        }
        monkeypatch.setattr(
            "pdd.agentic_e2e_fix_orchestrator.load_workflow_state",
            lambda *a, **kw: (loaded_state, None),
        )

        # _detect_changed_files returns prior-cycle edits ONLY when called with
        # the saved snapshot. If the workflow recaptures fresh (the bug), it
        # would be called with {} and return [], and the original failure recurs.
        def detect_with_snapshot_check(cwd, hashes):
            if hashes == saved_snapshot:
                return ["src/direct_fix.py"]
            return []

        monkeypatch.setattr(
            "pdd.agentic_e2e_fix_orchestrator._detect_changed_files",
            detect_with_snapshot_check,
        )
        ...
        # Without the resume fix, has_direct_edits would be False and the workflow
        # would exit with success=False, msg="Issue determined to be not a bug."
        assert msg != "Issue determined to be not a bug.", (
            f"Resume must restore initial_file_hashes so direct-edit detection "
            f"survives interruption. Got msg={msg!r}"
        )

    def test_state_data_persists_initial_snapshot(
        self, e2e_fix_mock_dependencies, e2e_fix_default_args
    ):
        """save_workflow_state must receive initial_file_hashes + initial_sha
        so a future resume can restore them.
        """
        mock_run, _, _ = e2e_fix_mock_dependencies
        ...
        with patch("pdd.agentic_e2e_fix_orchestrator.save_workflow_state") as save:
            save.return_value = None
            run_agentic_e2e_fix_orchestrator(**e2e_fix_default_args)

            assert save.called, "save_workflow_state should have been called"
            saved_state = None
            for call in save.call_args_list:
                args = call.args
                kwargs = call.kwargs
                state_arg = args[3] if len(args) > 3 else kwargs.get("state_data")
                if isinstance(state_arg, dict):
                    saved_state = state_arg
            assert saved_state is not None, "Expected dict state_data in save call"
            assert "initial_file_hashes" in saved_state, (
                "state_data must include initial_file_hashes for resume to restore it"
            )
            assert "initial_sha" in saved_state, (
...
class TestNotABugSuppressedOnResume:
    ...
    def test_resume_legacy_state_inline_cycle_waste_breaker_blocked_when_rerun_returns_not_a_bug(
        ...
        """External reviewer P1 regression: legacy state file (no
        cycle_start_hashes saved) where the resume helper correctly demotes
        the cached step_outputs["3"] = NOT_A_BUG via the missing-snapshot
        branch and validate_cached_state rewinds last_completed_step. The
        outer loop then captures a FRESH post-edit cycle_start_hashes
        snapshot at cycle-entry. When Step 3 reruns and returns NOT_A_BUG
        again, the INLINE cycle-waste-breaker computed
        ``has_cycle_progress = bool(_detect_meaningful_changes(cwd, fresh))``
        as False (the fresh snapshot was taken after edits), and with
        ``has_direct_edits and current_cycle > 1`` would terminal-success
        on a baseline it cannot verify.

        Fix: ``cycle_baseline_unverified`` flag set in the resume logic
        when ``resumed_cycle_start_hashes is None and last_completed_step >
        0 and current_cycle > 1``; the inline cycle-waste-breaker AND-s
        ``not cycle_baseline_unverified`` into the terminal-success
        condition. The cycle falls through to "NOT_A_BUG ignored" and
        Step 9 takes loop control normally; the flag is cleared at the
        next legitimate cycle rollover.

        Without the fix: final_message becomes
        "Direct-edit fix applied in a prior cycle; Step 3 classifies
        remaining state as not a bug." (false terminal-success).
        With the fix: final_message is NOT the terminal-success string;
        Step 9 is reached and decides loop control.
        """
        mock_run, _, _ = e2e_fix_mock_dependencies
        e2e_fix_default_args["resume"] = True
        e2e_fix_default_args["max_cycles"] = 2
        ...
        # Legacy state — last_completed_step=3 (mid-cycle, past Step 3),
        # current_cycle=2, NO cycle_start_hashes key.
        resumed_state = {
            "current_cycle": 2,
        ...
        # _detect_meaningful_changes returns a non-empty list against
        # initial_file_hashes (direct edits present) AND against the
        # fresh cycle_start_hashes snapshot (because the fresh snapshot
        # equals the current tree, BUT the function is patched to always
        # return ["src/module.py"], emulating the worst case where the
        # function's filter incorrectly reports "has changes").
        # Actually for the bug repro we want the cycle-progress check to
        # return EMPTY (the bug condition); pass a side_effect that
        # distinguishes initial_file_hashes from cycle_start_hashes.
        initial_hashes = resumed_state["initial_file_hashes"]

        def detect_side_effect(cwd, hashes):
            # Direct edits vs initial: True (file changed since workflow start)
            if hashes is initial_hashes or (
                isinstance(hashes, dict) and hashes == initial_hashes
            ):
                return ["src/module.py"]
            # Cycle progress vs fresh snapshot: empty (the bug condition —
            # fresh snapshot was taken AFTER edits so diff vs current = empty)
            return []
        ...
        with patch(
            ...
            def side_effect(*args, **kwargs):
                label = kwargs.get("label", "")
                # Step 3 rerun ALSO returns NOT_A_BUG — this is the key
                # difference from the prior test. Without the flag, this
                # triggers the inline cycle-waste-breaker terminal-success.
                if "step3" in label:
                    return (True, "Root cause: NOT_A_BUG — expected behavior.", 0.1, "gpt-4")
                if "step9" in label:
                    return (True, "Some tests still failing. CONTINUE_CYCLE", 0.1, "gpt-4")
        ...
        assert final_message != (
            ...
            "Inline cycle-waste-breaker terminal-successed on a legacy "
            "resumed cycle with no verifiable baseline — external "
...
class TestTrustedStepCommentPosting:
    ...
    def test_step10_post_persists_step_comments_before_gate_failure_return(
        self, e2e_fix_mock_dependencies, e2e_fix_default_args
    ):
        """Regression for Greg's PR review (round 3 — idempotency).
        ...
        Otherwise, when a later failure-return fires (with the agentic checkup
        removed, the next failure-return after the Step 10 post is
        the pre-checkup gate failing — or its drift-sync push failing), a later
        resume re-posts the Step 10 comment (and likely Step 11 too).

        Assertion: at least one ``save_workflow_state`` call between
        Step 10 post and the gate-failure return must carry the Step 10
        ...
        assert success is False, "Pre-checkup gate failure must propagate"
        ...
        assert step10_persisted, (
            ...
            "save_workflow_state before any gate-failure return so "
    ...
    def test_step10_persists_on_success_fall_through_resume_path(
        ...
        (``10010``) to saved state before any gate-failure return (with the
        agentic checkup removed, the gate is now the last failure-return
        after the Step 10 post).
        """
        mock_run, _, _ = e2e_fix_mock_dependencies
...
class TestVerifierOutputDetail:
    ...
    def test_python_failure_surfaces_exact_command_and_stderr(self, tmp_path):
            ...
            "test_results": [
                {
                    ...
                    "standard_error": "ERROR collecting: ImportError: no widget",
                }
            ],
        }
        with patch(
        ...
        assert passed is False
        assert exact_cmd in output
        assert "assert 1 == 2" in output
        assert "ImportError: no widget" in output
    ...
    def test_non_python_timeout_surfaces_command_and_partial_output(self, tmp_path):
        ...
        assert passed is False
        assert "npx playwright test tests/widget.spec.ts" in output
        assert "timeout" in output.lower()
        assert "partial stdout here" in output


class TestStep9VerifierRejectionVisibleComment:
    """Step 9 verifier rejections must be visible at rejection time."""
    ...
    def test_step9_rejection_posts_visible_comment_per_cycle(
        ...
        def side_effect(*args, **kwargs):
            ...
            if "step9" in label:
                return (
                    True,
                    "All targeted tests pass now. ALL_TESTS_PASS",
                    0.1,
                    "gpt-4",
                )
            return (True, f"Output for {label}", 0.1, "gpt-4")
    ...
    def test_rejection_comment_posted_even_when_later_cycle_succeeds(
        ...
        def verify_side_effect(*_args, **_kwargs):
            verify_calls["n"] += 1
            if verify_calls["n"] == 1:
    ...
    def test_resume_reverify_rejection_posts_visible_comment(
        ...
        assert success is False
        rejections = self._rejection_calls(mock_post_once)
        assert rejections, msg
        assert rejections[0].kwargs["step_num"] // 10000 == 1
        assert "tests/test_widget.py" in rejections[0].kwargs["body"]
    ...
    def test_rejection_comment_body_is_bounded(
        ...
        body = self._rejection_calls(mock_post_once)[0].kwargs["body"]
