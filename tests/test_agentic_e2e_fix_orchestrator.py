...
class TestFinalCheckupForwardsCwd:
    ...
    ``_run_final_checkup_on_pr`` must forward its ``cwd`` argument to
    ``run_agentic_checkup``. Otherwise, the final-gate checkup would resolve
    project context against the process CWD instead of the e2e orchestrator's
    worktree, which can be a sibling directory.
    """
    ...
    def test_run_final_checkup_passes_cwd_to_run_agentic_checkup(self, tmp_path):
        ...
        assert checkup_mock.called, "run_agentic_checkup should have been invoked"
        kwargs = checkup_mock.call_args.kwargs
        assert "cwd" in kwargs, (
            "_run_final_checkup_on_pr must forward `cwd` to run_agentic_checkup"
        )
    ...
    def test_no_pr_found_skips_checkup_without_invoking(self, tmp_path):
        """If no open PR is found, the helper short-circuits — no checkup call."""
        from unittest.mock import patch
