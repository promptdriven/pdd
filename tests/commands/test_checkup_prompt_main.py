...
class TestRunCheckupPrompt:
    def test_check_only_returns_stage1_pass(self) -> None:
        success, message, cost, model, exit_code = run_checkup_prompt(
            str(PAYMENT_PROMPT),
            explain=False,
            interactive=False,
            apply=False,
    ...
    def test_explain_non_fatal_on_llm_failure(self) -> None:
        ...
        with patch(
            ...
            with patch(
                ...
                success, _, _, _, _ = run_checkup_prompt(
                    str(PAYMENT_PROMPT),
                    explain=True,
                    interactive=False,
                    apply=False,
    ...
    def test_explain_does_not_change_exit_outcome(self) -> None:
        with patch("pdd.checkup_prompt_main._run_explain_advisory") as explain:
            ...
            fail_success, _, _, _, _ = run_checkup_prompt(
                str(PAYMENT_PROMPT),
                explain=True,
                quiet=True,
                project_root=REPO_ROOT,
            )
        without_explain, _, _, _, _ = run_checkup_prompt(
            str(PAYMENT_PROMPT),
            explain=False,
            quiet=True,
            project_root=REPO_ROOT,
...
class TestCheckupDispatch:
    ...
    def test_checkup_prompt_dispatch_success_exits_zero(self) -> None:
        ...
        with patch(
            ...
            return_value=(True, "Prompt source-set check passed.", 0.0, "test-model", 0),
        ):
            result = runner.invoke(
                checkup,
                ["prompt", str(PAYMENT_PROMPT)],
    ...
    def test_checkup_prompt_strict_forwarded(self) -> None:
        ...
        with patch("pdd.commands.checkup_prompt.run_checkup_prompt") as run_prompt:
            run_prompt.return_value = (True, "ok", 0.0, "", 0)
            runner.invoke(
                checkup,
