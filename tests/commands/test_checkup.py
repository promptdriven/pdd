...
def test_checkup_agentic_review_loop_forwards_mirror_options() -> None:
    runner = CliRunner()

    with patch("pdd.commands.checkup.run_agentic_checkup") as run_checkup:
        run_checkup.return_value = (True, '{"status":"ok"}', 0.25, "codex")
    ...
    assert kwargs["review_loop"] is False
    assert kwargs["final_gate"] is True
    assert kwargs["as_json"] is True
    assert kwargs["reviewers"] == "codex:/review,claude:/code-review"
    assert kwargs["adversarial_prompt"] == "mirror canonical checkup only"
    assert kwargs["fresh_final_review_role"] == "codex"


def test_checkup_agentic_review_loop_rejects_review_loop_combo() -> None:
    runner = CliRunner()
    ...
    assert "--agentic-review-loop owns its mirror review pass" in result.output


def test_checkup_agentic_scoped_flags_require_agentic_mode() -> None:
    runner = CliRunner()
    ...
    assert result.exit_code == 2
    assert "--adversarial-prompt is only valid with --agentic-review-loop" in result.output
    ...
    assert result.exit_code == 2
    assert "--fresh-final-review is only valid with --agentic-review-loop" in result.output
