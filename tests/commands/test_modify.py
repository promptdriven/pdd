...
def test_change_agentic_uses_real_issue_derived_story_generation(
    runner,
    mock_run_agentic_change,
):
    """Regression guard: pdd change must keep the PR #1360 story pipeline.

    This intentionally does not mock pdd.commands.modify.generate_user_story.
    It exercises the real issue resolution, LLM-authoring wrapper, story write,
    and pdd-story-prompts metadata stitching through the pdd change command.
    """
    issue_url = "https://github.com/x/y/issues/1454"
    story_markdown = (
        "# User Story: Prompt Story Workflow\n\n"
        "## Covers\n"
        "- ISSUE-1454: prompt-story workflow\n\n"
        "## Story\n"
        "As a maintainer, I can generate a linked story from an issue.\n\n"
        "## Context\n"
        "pdd change modified a prompt.\n\n"
        "## Acceptance Criteria\n"
        "- A story file is created.\n"
        "- The changed prompt is linked in metadata.\n\n"
        "## Oracle\n"
        "Issue-derived behavior is preserved.\n\n"
        "## Non-Oracle\n"
        "Exact prose is not important.\n\n"
        "## Negative Cases\n"
        "- Do not author from prompt content.\n\n"
        "## Non-Goals\n"
        "No live provider call in this test.\n\n"
        "## Notes\n"
        "Deterministic test story.\n"
    )
    issue_body = (
        "# Issue: Prompt-story workflow\n\n"
        "Create or refresh a story from issue context when pdd change touches "
        "a prompt."
    )
    mock_run_agentic_change.return_value = (
        True,
    ...
    with runner.isolated_filesystem():
        os.makedirs("prompts")
        with open("prompts/upload_python.prompt", "w", encoding="utf-8") as fh:
            fh.write("PROMPT_ONLY_MARKER must not reach story author")

        captured_author = {}

        def fake_author(**kwargs):
            captured_author.update(kwargs)
            assert "Prompt-story workflow" in kwargs["issue_text"]
            assert "PROMPT_ONLY_MARKER" not in kwargs["issue_text"]
            return story_markdown, 0.25, "story-model"

        with patch("pdd.user_story_tests._fetch_issue_via_gh") as mock_fetch, \
             patch(
                 "pdd.user_story_tests._llm_generate_story_markdown",
                 side_effect=fake_author,
             ):
            mock_fetch.return_value = ("Prompt-story workflow", issue_body)

            result = runner.invoke(change, [issue_url])

        story_path = "user_stories/story__upload.md"
        with open(story_path, encoding="utf-8") as fh:
            generated_story = fh.read()

    assert result.exit_code == 0, result.output
    assert "Generated user story:" in result.output
    assert mock_fetch.call_args.args == ("x/y", "1454")
    assert captured_author["issue_ref"] == "x/y#1454"
    assert "<!-- pdd-story-prompts: upload_python.prompt -->" in generated_story
    assert "## Acceptance Criteria" in generated_story
    assert "## Oracle" in generated_story
    assert "## Negative Cases" in generated_story
