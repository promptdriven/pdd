...
CODEGEN_PROMPT = DEMO / "prompts" / "foo_codegen_python.prompt"
CODEGEN_STORY = DEMO / "user_stories" / "story__foo_codegen.md"

HAND_AUTHORED_PROMPTS = sorted([VAGUE_PROMPT.name, FORMALIZED_PROMPT.name, CODEGEN_PROMPT.name])
HAND_AUTHORED_STORIES = sorted([VAGUE_STORY.name, FORMALIZED_STORY.name, CODEGEN_STORY.name])
...
def test_deterministic_demo_persists_vague_formalized_artifacts_and_diff() -> None:
    """The demo always keeps stable copies of vague/formalized + their diff."""
    proc = _run_demo()
    assert proc.returncode == 0, proc.stdout + proc.stderr

    vague_artifact = REPORTS / "artifacts" / "prompt_vague.prompt"
    formalized_artifact = REPORTS / "artifacts" / "prompt_formalized.prompt"
    diff = REPORTS / "diffs" / "prompt_vague_vs_formalized.diff"

    assert vague_artifact.is_file(), "missing reports/artifacts/prompt_vague.prompt"
    assert formalized_artifact.is_file(), "missing reports/artifacts/prompt_formalized.prompt"
    assert diff.is_file(), "missing reports/diffs/prompt_vague_vs_formalized.diff"

    # The persisted copies must exactly match the hand-authored inputs.
    assert vague_artifact.read_text(encoding="utf-8") == VAGUE_PROMPT.read_text(encoding="utf-8")
    assert formalized_artifact.read_text(encoding="utf-8") == FORMALIZED_PROMPT.read_text(encoding="utf-8")
    ...
    # The migration must add a <vocabulary> block.
    assert any(
