...
@pytest.mark.e2e
class TestStep9NoneTokenHandling:
    """
    Tests for Step 9 outputs that don't produce a parseable loop-control token.

    Case 1: Step 9 fails (step_success=False) and _classify_step_output returns None.
            We should start the next cycle so Step 9 can run again.

    Case 2: Step 9 succeeds (step_success=True) but _classify_step_output returns None
            even after a retry. We should stop with a clear terminal message and NOT
            start another cycle.
    """

    def test_step9_failure_with_no_token_starts_next_cycle(
        self,
        mock_git_repo,
        orchestrator_io_mocks,
    ):
        """Failed Step 9 with no token should start the next cycle."""
        mock_run = orchestrator_io_mocks["run"]
        ...
        def side_effect(*args, **kwargs):
            label = kwargs.get("label", "")
            match = re.search(r"cycle(\d+)", label)
            if match:
                cycles_seen.add(int(match.group(1)))

            # All steps except Step 9 succeed normally
            if "step9" not in label:
                return (True, f"Output for {label}", 0.1, "test-model")

            # Step 9: simulate provider failure with no control token
            # (matches the "providers failed / timeout" style error)
            return (
                False,
                "All agent providers failed: anthropic: Timeout expired",
                0.0,
                "test-model",
            )
        ...
        success, msg, cost, model, files = run_agentic_e2e_fix_orchestrator(
            ...
            issue_content="Step 9 provider failure with no token",
            repo_owner="test",
            repo_name="repo",
            issue_number=999,
            issue_author="test-user",
            issue_title="Step 9 provider failure",
            cwd=mock_git_repo,
            verbose=False,
            quiet=True,
            max_cycles=2,
        ...
        # We expect at least cycles 1 and 2 to be started since Step 9 failure
        # with no token should now be treated as a transient issue.
        assert 1 in cycles_seen
        assert 2 in cycles_seen

    def test_step9_success_no_token_after_retry_stops_workflow(
        self,
        mock_git_repo,
        orchestrator_io_mocks,
    ):
        """
        Successful Step 9 that never emits a loop-control token (even after a
        retry) should stop the workflow with a clear message and not start a
        second cycle.
        """
        mock_run = orchestrator_io_mocks["run"]
        ...
        def side_effect(*args, **kwargs):
            ...
            # All non-step9 labels succeed normally
            if "step9" not in label:
                return (True, f"Output for {label}", 0.1, "test-model")

            # Step 9 (and its retry): succeed but never emit a control token
            return (
                True,
                "Final verification complete. Tests checked, but no explicit status.",
                0.2,
                "test-model",
            )
        ...
        success, msg, cost, model, files = run_agentic_e2e_fix_orchestrator(
            issue_url="https://github.com/test/repo/issues/1000",
            issue_content="Step 9 success with no token even after retry",
            repo_owner="test",
            repo_name="repo",
            issue_number=1000,
            issue_author="test-user",
            issue_title="Step 9 no-token retry terminal",
            cwd=mock_git_repo,
            verbose=False,
            quiet=True,
            max_cycles=3,
        ...
        # Workflow should terminate after Step 9 retry; there should be no Cycle 2.
        assert success is False
        assert "did not emit a loop control token after retry" in msg
        assert not any("cycle2_" in label for label in labels_seen), (
            "Expected workflow to stop after Step 9 retry without starting Cycle 2 "
            f"when no loop control token is emitted. Labels seen: {labels_seen}"
        )
