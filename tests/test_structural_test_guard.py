...
class TestScanStep9HelperRewriteWithPreexistingViolation:
    """Whole-file rewrites must not retry only because old violations moved."""

    def test_rewrite_preserving_preexisting_violation_reports_zero(
        self, tmp_path: Path
    ) -> None:
        f = tmp_path / "test_x.py"
        snap = (
            "import inspect\n\n"
            "def test_existing_structural():\n"
            "    sig = inspect.signature(target)\n"
            "    assert 'token' in sig.parameters\n"
        )
        current = (
            "# regenerated by step 9\n"
            + snap
            + "\n\ndef test_new_behavioral():\n    assert run() == 'ok'\n"
        )
        ...
        assert violations == [], (
            "A non-prefix rewrite that keeps an old structural test must not "
            f"be treated as a new Step 9 violation. Got: {violations}"
        )

    def test_rewrite_preserving_old_violation_still_flags_new_violation(
        self, tmp_path: Path
    ) -> None:
        f = tmp_path / "test_x.py"
        snap = (
        ...
        assert any("inspect.getsource" in v for v in violations)
        assert not any("inspect.signature" in v for v in violations)
