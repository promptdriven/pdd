...
def test_finding3_no_override_still_reads_disk(tmp_path: Path) -> None:
    """When ``prompt_content`` is omitted, the helper must still fall back to
    reading includes from ``prompt_path`` — the original API contract."""
    prompts = tmp_path / "prompts"
    prompts.mkdir(parents=True)
    on_disk = (
        "<pdd-reason>r</pdd-reason>\n\n"
