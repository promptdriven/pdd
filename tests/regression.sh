...
# --- Common CI Setup (always runs, even for individual cases) ---
...
# Seed prerequisite files for individual case runs (CI parallel mode).
# When running "all", earlier cases generate these files. For individual cases,
# copy from fixtures so they don't depend on earlier cases having run first.
if [ "$TARGET_TEST" != "all" ]; then
    # simple_math.py — generated by case_1 (generate), needed by cases 2,4,5,6,8,10,14,18
    ...
    # simple_math_example.py — generated by case_2 (example), needed by cases 6,10
    ...
    # updated_simple_math_python.prompt — generated by case_4 (update), needed by case_5
    if [ ! -f "$UPDATED_MATH_PROMPT" ] && [ -f "$FIXTURES_PATH/updated_simple_math_python.prompt" ]; then
        log "Seeding prerequisite: $UPDATED_MATH_PROMPT from fixture"
        cp "$FIXTURES_PATH/updated_simple_math_python.prompt" "$UPDATED_MATH_PROMPT"
    fi
    # sub_simple_math.prompt — generated by case_10 (split), needed by case_11
    if [ ! -f "$SPLIT_SUB_PROMPT" ] && [ -f "$FIXTURES_PATH/sub_simple_math.prompt" ]; then
        log "Seeding prerequisite: $SPLIT_SUB_PROMPT from fixture"
        cp "$FIXTURES_PATH/sub_simple_math.prompt" "$SPLIT_SUB_PROMPT"
    fi
fi
