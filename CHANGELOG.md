...
- **checkup**: allow `pdd checkup --pr <pr-url> --issue <issue-url>` to run the full standard fix/verify loop on the existing PR, push generated fixes back to the same PR branch, and let `pdd-issue` use that PR-mode checkup as its final gate (#1104).
...
- **checkup**: Step 7 verdict parser now selects the LAST top-level JSON object in the output (via `json.JSONDecoder.raw_decode`), matching the prompt contract that the verdict be last. Earlier intermediate-reasoning blocks — fenced or unfenced — can no longer mask a final `issue_aligned=false` (round-4 Finding 1).
- **checkup**: PR mode posts the canonical final report to the PR and issue threads on every terminal path (gate pass after push, gate fail, max-iteration exhaustion, registry/prompt-source guard refusals, push failure, post-push reverify abort, `--no-fix` pass/fail). Previously the report was only posted after a successful fix-mode push, so `--no-fix` runs and any failure path left consumers with no public report (round-4 Finding 2).
- **checkup**: PR mode now reloads `architecture.json` and `.pddrc` from the PR worktree after checkout so downstream prompts audit the PR's project state, not the pre-PR parent checkout's (round-4 Finding 3).
- **checkup**: PR-mode comment-post failures from `post_pr_comment` / `post_step_comment` are no longer silently discarded. The rendered body is saved to `.pdd/checkup-pr-<n>/final-report.md`, a human-readable status is persisted as `step_outputs["pr_post_status"]`, and the orchestrator's returned message gets a suffix pointing to the artifact. The gate outcome itself is NOT flipped — gh/network flakiness must remain decoupled from code-verification truth (round-4 Finding 4).
