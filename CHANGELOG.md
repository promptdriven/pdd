...
- **agentic_sync**: add `PDD_CHANGED_MODULES` env-var fast path for module identification (#1883). When set to a comma-separated list of basenames (e.g. `agentic_sync,sync_main`), `pdd sync <issue-url>` bypasses the identify-modules LLM call when branch-diff detection returns no modules and uses the listed modules directly (branch-diff detection is still attempted first; this env var activates when branch-diff returns empty). Parsing via `_parse_changed_modules_env` splits on commas, strips whitespace, discards empty entries, and deduplicates preserving first-occurrence order; path-qualified names (e.g. `commands/maintenance`) are preserved intact. The parsed list enters the same normalization, validation, and scheduling pipeline as LLM-identified modules. When any parsed entry is ultimately unresolvable, an error diagnostic names `PDD_CHANGED_MODULES` as the source instead of silently syncing only the resolvable subset. Unset or empty values fall through to the existing LLM fallback unchanged.
- **sync**: classify prose/planning LLM output as `ProseOutputError` before the architecture conformance gate; a provider that returns a planning sentence instead of a code block now raises a distinct typed error with output-shape retry directive ("return the complete source file only, inside a single code block"), limits prose retries to 1 additional attempt, and emits a structured `=== generation output extraction failure ===` hard-failure block with provider/model, prompt, output path, extractor result, and raw-output excerpt on repeated failure. The target file is never overwritten. Escape hatch: `PDD_ALLOW_EMPTY_GENERATION=1`. Affects `code_generator_main.py` and `agentic_sync_runner.py` (#1649).
...
- **sync**: preserve safe `architecture.json.filepath` precedence across nested or
  custom `.pddrc` prompt roots, honor exact configured example/test templates,
  reject architecture filenames/outputs that escape their roots, and prevent
  cross-context or differently named prompt modules from aliasing each other (#1976).
  Additionally: a stale sibling-context entry whose prompt was deleted can no longer
  be borrowed by a same-leaf prompt (heuristic leaf/filepath-stem matches are now
  restricted to the resolving prompt's context territory), Windows drive-qualified
  architecture filenames/filepaths (`D:/x`, `D:x`) are rejected alongside absolute
  and `..` paths, and the agentic architecture repair prompt no longer instructs
  agents to flatten the Issue #617 path-mirroring layout this change relies on.
  Further hardening: every prompt returned by a recursive `prompts_root` scan must
  now resolve inside the root, so a same-leaf file symlink can no longer be followed
  out of the repository and written through by an update; and the context-territory
  guard anchors its `.pddrc` lookup at the project (the `architecture.json`
  directory) instead of the process CWD, so context isolation still holds when sync
  is driven from a parent/sibling working directory. The governing
  `sync_determine_operation` prompt documents all three invariants so a future
  regeneration preserves them.
  Final hardening: containment now gates the Step 1 direct/case-insensitive fast
  path too (not only recursive candidates), so the exact expected prompt being a
  symlink to an external file can no longer be returned and truncated by an update
  (in-root aliases are preserved); same-leaf context tie-breaking loads its
  `context_prefix` from a `.pddrc` anchored at the prompt root rather than the CWD,
  so a parent/sibling-CWD run with a broad prompt root no longer picks the
  wrong-context prompt; and `architecture.json` is parsed once per resolution and
  the immutable snapshot is shared across ambiguity, prompt, and code-path phases so
  a concurrent rewrite cannot produce a torn prompt/code pair.
  Metadata robustness: the architecture HINT used for prompt discovery now applies the
  same context-territory ownership as code-path selection, so a broad-root bare-leaf
  lookup no longer borrows a sibling context's row and returns its prompt/code; an
  unsafe same-leaf architecture filename (absolute, `..`, backslash, Windows drive) is
  excluded from bare-basename ambiguity detection so it can no longer raise a spurious
  `AmbiguousModuleError` that blocks a valid module; and a `null`/non-string
  architecture `filename` is treated as absent (coerced before any `.lower()`) so the
  module resolves by filepath stem instead of hitting an `AttributeError` that the
  broad fallback swallowed into a cwd-relative path.
  Deeper hardening: context-territory ownership now covers EVERY heuristic architecture
  borrow — including the basename+language and flat-filename match loops — so a flat
  legacy same-leaf row (`credits_Python.prompt` -> `frontend/credits.py`) can no longer
  be borrowed by a backend resolution; `_get_filepath_from_architecture` prefers the
  caller's resolved context and anchors all context-prefix `.pddrc` lookups at the
  architecture project root, so a path-qualified basename resolved from a parent/sibling
  CWD aligns with its canonical `backend/services/foo.py` target instead of falling back
  to the default output; and an unsafe OUTPUT filepath (absolute, `..`, backslash,
  Windows drive, symlink escape) is excluded from bare-basename ambiguity counting so a
  same-filename unsafe row can no longer block a valid module.
  CWD-independence completed: the project/prompts-root `.pddrc` anchor now reaches EVERY
  context lookup on the resolution path — context inference for a path-qualified basename
  with NO explicit override, `_find_prompt_file`'s own candidate building and
  path-qualified alignment (so a custom prompt root finds the existing prompt instead of
  creating a duplicate), and the `construct_paths_basename` used for exact example/test
  template expansion (so a `{category}` template no longer duplicates the context prefix,
  e.g. `backend/examples/backend/foo_example.py`) — all resolved correctly from a
  parent/sibling CWD. The unsafe-output-path containment check in ambiguity detection is
  now deferred until after the cheap filename/stem match, so only matching rows are
  filesystem-resolved (fixes an ~8x slowdown on large architectures).
  Output-side anchoring and proven-ownership: a NEW module resolved from a parent/sibling
  CWD now writes under the subproject — the missing-prompt `construct_paths` call receives
  the prompt path as an anchoring hint and project-relative template outputs are anchored
  at the project root — and the non-architecture `{category}` template branch no longer
  duplicates the context prefix. Context-territory ownership is now applied only to
  UNRESOLVED heuristic borrows: an architecture row whose physical prompt owner IS the
  resolved prompt (a proven, explicit mapping) keeps its authoritative code target even
  when it lies outside the context's own globs — as long as that target is not owned by a
  sibling context — so an intentionally shared code path is honored while a stale
  cross-context row is still rejected.
  Output anchoring, absolute-path territory, and flat-hint context: the existing-prompt
  template branch now anchors project-relative outputs at the subproject too (previously
  only the missing-prompt branch did), re-anchoring only when the project root differs
  from the CWD; context-territory matching re-expresses absolute `.pddrc` `paths`/output
  values relative to the project before comparison, so a sibling context with an absolute
  `generate_output_path` still owns its code and a stale flat row targeting it is rejected;
  and a legacy FLAT architecture filename that matches the same leaf in two context
  subdirectories now resolves the prompt in the REQUESTED context (not the
  shallowest/lexicographic-first), keeping the prompt and code in the same context.
  Exact-match and configured-root follow-up: exact/case-insensitive architecture
  filename matches now enforce the same sibling-territory ownership check as every
  other match branch; context prompt selection compares path components, so `backend`
  cannot match `a-backend`; and contained absolute sibling `prompts_dir` roots now
  participate in physical prompt ownership discovery.
  Root-output and prompt-containment follow-up: sibling-context ownership now wins
  even when the resolving context declares a repo-root (`./`) output; custom context
  prefixes are derived relative to the active prompt root; and an escaping direct
  prompt symlink rejected during discovery now raises a hard path-resolution error
  instead of being reconstructed by the missing-prompt fallback.
  Deeper custom-root and ownership handling: path-qualified new modules under a
  broad custom root keep their configured context prefix; a sibling context's `./`
  output no longer falsely claims every path and vetoes a valid current-context
  architecture target; unsafe nested symlink-only prompt matches fail closed instead
  of becoming new modules; and architecture rows share one parsed context-territory
  snapshot per lookup, avoiding repeated `.pddrc` parsing on duplicate matches.
  Final lexical/context isolation: user basenames are rejected lexically before any
  filesystem path expression when absolute, traversal-bearing, backslash, or Windows
  drive-qualified; nested missing modules compose both the custom context prefix and
  remaining basename directories; unsafe recursive matches are filtered to the active
  context before hard failure; and duplicate architecture rows memoize their ownership
  decision while sibling-owned targets short-circuit before physical-owner walks.
  Language/context alignment: caller language values must be one safe filename
  component; explicit context selection is strict for both safe and unsafe recursive
  candidates (a sibling prompt cannot be borrowed or mask an active-context escape);
  and flat architecture hints must align with path-qualified basenames before a
  recursive prompt can be paired with the canonical code target.
  Direct/nested completion: the direct prompt fast path now obeys strict context
  scope; an architecture helper never returns a direct join it already rejected for
  path-qualified basename misalignment; and legacy missing-module artifacts defer
  nested-directory insertion to the shared one-time reanchor, avoiding duplicated
  paths such as `utils/utils/test_foo.py`.
  Alignment/input completion: nested exact architecture filenames now retain the
  path-qualified filepath alignment gate; nested prompt directory suffix comparison
  is case-insensitive so Linux reuses existing casing; and path validators reject
  NUL/control characters plus leading/trailing whitespace instead of validating a
  stripped copy while callers continue with the original invalid value.
  Direct/input finalization: explicit-context scope is checked lexically before a
  direct candidate's containment walk, so an unsafe root-level sibling cannot block
  backend creation; raw basename/language are logged only after validation; and
  degenerate/noncanonical basenames (`.`, `foo/`, `foo/.`, duplicate separators)
  are rejected rather than normalized into hidden or empty artifact names.
  Portable-root determinism: `prompts_dir` is control/format-character validated
  before resolution and logged with escaping; basename components reject NTFS ADS,
  Windows device identities, and Unicode control/format characters; and case-folded
  directory-index buckets are stably sorted so Linux case collisions do not depend
  on raw `os.scandir` order.
  Portable output parity: all Windows-invalid component characters (`<>:"|?*`) and
  Unicode line/paragraph separators are rejected for caller prompt identities, and
  architecture code filepaths now use the same portable component/device/control
  checks before containment, so deceptive or device/ADS outputs cannot remain
  authoritative.
  Duplicate/log/device completion: unsafe output rows are rejected during candidate
  eligibility so they cannot shadow a later valid duplicate; raw architecture
  filepaths are INFO-logged only after validation and with escaped rendering; and
  Windows superscript device aliases (`COM¹`–`COM³`, `LPT¹`–`LPT³`) are rejected.
  Prompt hygiene: the governing `sync_determine_operation` prompt now states these
  path-resolution obligations as stable observable `<contract_rules>` (R1–R15) with a
  `<coverage>` map to the accumulated resolver tests, replacing the transcribed
  implementation walkthrough per the prompting guide; the `metadata_sync` and
  `sync_determine_operation` `.pdd/meta` fingerprints were refreshed so `pdd sync`
  reports no drift.
  Configured-output containment (R16): the same project-boundary containment applied
  to `architecture.json` code filepaths now also gates every `.pddrc`-derived output
  destination — `generate_output_path`, `example_output_path`/`test_output_path`, and
  `outputs.<artifact>.path` templates. A configured code/example/test path that
  resolves outside the project (parent traversal, escaping symlink, or an away-pointing
  absolute path) fails closed with a path-resolution error, and path discovery no longer
  creates a temporary probe file outside the tree — closing a gap where a `.pddrc`
  could make `pdd sync` (even `--dry-run`) write outside the project while the
  architecture filepath itself stayed contained. Configured `.pddrc` output values
  now also get the same portable/canonical validation as architecture filepaths
  (rejecting parent traversal, reserved device names, NTFS ADS colons, drive
  markers, and control characters) on the RAW value before it is resolved, so a
  normalized-away `..` or a non-portable destination fails closed independent of the
  process CWD. Separately, `SyncLock` now builds its lock filename from a sanitized,
  separator-free token of the basename/language — the lock is acquired before those
  inputs are validated, so a traversal-bearing `language` can no longer create or
  truncate a `.lock` file outside the locks directory.
- **auto-heal**: resolve changed code, prompt, context, and flattened test names through canonical architecture/prompt ownership before drift classification. Path-qualified identities now distinguish root and nested duplicate basenames (for example `pdd/cli` versus `core/cli`); non-conventional cross-cutting tests use exact canonical `pdd...` import ownership, while ambiguous or genuinely unmapped managed candidates still fail closed. Excluded reverse dependencies stay excluded, and drift selection resolves each requested selector only once per unique language (#2059).
- **preprocess/content_selector**: parse comma-separated `select=` values without splitting `pytest:test_one,test_two`; index `async def` tests in `pytest_slicer`.
...
- **split**: move the `pdd checkup --validate-arch-includes` gate after `7c_arch_sync` so it validates freshly refreshed architecture metadata and feeds failures into the global repair loop.
...
- **split**: `pdd split` now runs a per-child verify gate (`6v`) immediately after each child extraction, calling `split_validation.validate_extraction()` filtered to that child's files only and routing error-severity failures into a bounded per-child repair sub-loop (max 2 attempts per child) with `ValidationFailure.message` strings as structured RETRY NOTICE feedback. Per-child status is persisted in `state["children_extracted_status"]` (`pending` / `extracted` / `verified` / `failed_extract` / `failed_verify` / `failed`) with a companion `repair_attempts` dict; state writes happen BEFORE each LLM repair call (persist-before-act) so a crash never loses a consumed attempt. On resumption, the orchestrator picks up at the first non-terminal child — both `verified` (success) and `failed` (per-child budget exhausted) are terminal and skipped without re-billing tokens; non-terminal statuses (`pending`, `extracted`, `failed_extract`, `failed_verify`) are re-attempted with the saved `repair_attempts` count carried forward. The end-of-pipeline `7a_verify_local` step continues to run as a final cross-cutting check (catches issues only visible at full-package scope, e.g. circular imports between children), then `7c_arch_sync` refreshes architecture metadata and `7d_checkup` invokes `pdd checkup --validate-arch-includes --project-root <worktree>` on the new package to catch architecture↔include drift invisible to file-level checks; cross-cutting failures route through the global step-8 repair channel (max 5 iterations across the whole plan, surfaced via the new `CHECKUP_FAILURE` category in the repair prompt). When any child is in terminal `failed` after the per-child gate, the global step-8 loop is short-circuited (it cannot recover what the per-child gate gave up on after 2 attempts). The improvement gate downgrades AUTO_SHIP to HUMAN_REVIEW_REQUIRED for ANY non-`verified` child status — including `failed` (per-child verify budget exhausted), `failed_extract` (step 6 file-existence retries exhausted), and any other non-terminal status that somehow survives to the gate — surfacing per-child reason text from `state["terminal_child_failures"]` (which captures both `failed_extract` missing-file detail and `failed` `ValidationFailure.message` detail) so the user must inspect the unfixable child(ren) before merging. (#850)
...
- **`pdd sync-architecture` CLI command**: new explicit entry point under `pdd/commands/maintenance.py` and registered in `pdd/commands/__init__.py`. Accepts prompt filenames (or the whole tree when none are given) with a `--dry-run` flag and prints a concise per-module summary of updated/skipped/errored entries plus sync and validation errors/warnings. Previously this workflow was reachable only via the `server/routes/architecture.py` HTTP route; the new command moves the full prompt-to-architecture sync loop into `pdd/architecture_sync.py` (`sync_prompts_to_architecture`, circular-dependency detection, and `_normalize_prompt_filename` for `./` / `prompts/`-prefixed inputs) so the server route becomes a thin wrapper. Covered by new CLI unit tests in `tests/test_commands_maintenance.py` and `tests/test_architecture_sync.py`
- **`extract_modules` / `extract_prd_files` tolerant accessors (#1256, #1264)**: `pdd/architecture_registry.py` exposes two never-raising helpers that normalize every `architecture.json` read across the codebase. `extract_modules` accepts either a bare `[ {...} ]` array or the newer `{"prd_files": [...], "modules": [...]}` object form and returns the module list; `extract_prd_files` pulls the optional PRD paths out of the object form only. Both helpers are now the single entry point for the 13 consumer sites that previously branched on `isinstance(data, list)` and silently dropped dict-format architectures

### Fix

- **tolerant `architecture.json` loader across all consumers (#1256, #1264)**: before this release, `agentic_sync`, `agentic_sync_runner`, `agentic_architecture`, `agentic_architecture_orchestrator`, `agentic_change_orchestrator`, `architecture_include_validation`, `architecture_sync`, `auto_deps_architecture`, `code_generator_main`, `render_mermaid`, `server/routes/architecture`, and `sync_determine_operation` each checked for a bare list and treated the `{prd_files, modules}` wrapper as empty — modules were silently dropped, sync rescans iterated dict keys instead of module records, and `render_mermaid` crashed outright. Every site now routes through `extract_modules`/`extract_prd_files`; `render_mermaid.py` carries an inline fallback (with a sync warning comment and `test_render_mermaid_standalone_fallback_mirrors_registry` guard) so it still works when invoked as a standalone subprocess with no parent package. Write-back paths in `architecture_sync.register_untracked_prompts` and `update_architecture_from_prompt` (both rename and main flows) preserve the on-disk dict shape instead of collapsing it to a bare array. `agentic_change_orchestrator` post-Step-10 scoping (`_scope_architecture_to_changed_files`, `_validate_architecture_filepaths`, `_sanitize_architecture_interfaces`, and the pre-Step-10 snapshot) uses the helpers so dict-format architectures are no longer corrupted. 20 failing regression tests were added up front (across 12 test files covering every affected site) and all now pass
- **`auto_deps_architecture` — surgical dependency patch supports dict-format**: the byte-span patcher that rewrites a single module's dependencies used to start scanning at the first `[` in the file, which found nothing under the `{"prd_files": ..., "modules": [...]}` wrapper and fell back to a full-file rewrite that warned "Could not patch dependencies." New `_find_modules_array_start` helper locates the `[` that opens the modules array (handles both bare-array and `"modules":\s*\[` shapes) and `_find_array_element_span` takes an explicit array start offset. `_find_top_level_array_element_span` remains as a thin byte-compatible wrapper so existing callers keep working. 3 new dict-format regression tests in `tests/test_auto_deps_architecture.py`
- **`pdd sync` — `CodePatchResult.patched_code` no-op contract (#1273)**: `pdd sync` failed with `ValidationError: Failed to parse response into CodePatchResult` whenever the LLM correctly reported "no change needed" by omitting `patched_code` or emitting `null`. The Pydantic field is now `Optional[str]` with `default=None`, and the caller in `pdd/incremental_code_generator.py` treats `None`/missing as the canonical no-op signal instead of requiring a string round-trip. 5 targeted tests were added up front — 3 failing that now pass and 2 regression guards
- **`pdd sync` — LLM prompt + strict-mode schema signal null is valid (#1273 review blockers)**: the OpenAI strict-mode schema rewriter (`_ensure_all_properties_required` in `pdd/llm_invoke.py`) promotes every property to required, so `Optional[str]` survives as `anyOf: [{type: string}, {type: null}]` — required but nullable. `pdd/prompts/code_patcher_LLM.prompt` previously documented `patched_code` as `"string"` with a string-only example, leaving the LLM no signal that `null` was acceptable; updated to `"string OR null"` and adds an `output_json_example_noop` block. 3 end-to-end tests in `test_incremental_code_generator.py` parse real payloads without mocking past validation and assert the rewritten schema still admits `null` via `anyOf`
- **`architecture_sync` — nearest-cwd sync-root path resolution**: `_resolve_sync_paths` used to anchor on the global `find_project_root()` result, which broke nested layouts where a subtree has its own `prompts/` and `architecture.json`. It now walks up from `cwd` looking for either `architecture.json` or a `prompts/` directory, stops at the first match (or the global project root), and resolves the architecture path from that sync-root's parent. Explicit `--prompts-dir`/`--architecture-path` values are preserved and made absolute relative to cwd when given as relative paths
- **`architecture_sync` — tolerate leading `%` preamble and XML-style helper tags**: `parse_prompt_tags` previously assumed the first content in a prompt was a `<pdd-*>` tag, so files that began with `% You are an expert ...` lines or a bare `<include>context/...</include>` before the real metadata header were treated as having no header at all — `example` / `context` tags buried in the prompt body would then be mistakenly harvested as metadata. New state machine skips blank and `%` lines, starts collecting on the first `<` line (tracking `<pdd-` as the signal that a real header began), and bails to "no metadata" only when ordinary prose appears before any tag. Regression coverage in `tests/test_architecture_sync.py`
- **`pdd example` — pass `language` into postprocess LLM invocation**: `pdd/postprocess.py` omitted the `language` kwarg when calling `llm_invoke` to extract code from the model's response, so non-Python languages fell through the default Python-syntax validation retry and wasted calls. The call now forwards `language`, and a new `test_llm_invoke_skips_python_validation_retry_for_typescript_language` test asserts valid TypeScript no longer triggers "invalid Python syntax" retries
- **`sync_determine_operation` — prefer basename-derived example/test artifacts (#225, #1284)**: `get_pdd_file_paths` anchored example/test file resolution on the architecture filepath stem, so a flat prompt basename like `lib_sse` whose prompt file is `lib_sse_typescript.prompt` would never find an intentionally-namespaced `lib_sse_example.ts` or `test_lib_sse.ts` sibling even when those files existed. New branch checks basename-derived paths first and falls back to the architecture stem only when they're absent; `test_stem` is also now `glob.escape`-d so basenames with regex metacharacters no longer silently glob wider than intended. Logging at INFO level records when the basename override wins. Regression tests in `tests/test_issue_225_paths_and_includes.py`
- **`generate_tags_for_prompt` route success path**: `pdd/server/routes/architecture.py` used `Path(...)` in the success branch but never imported `pathlib.Path`, so the happy path raised `NameError` and only the error branches were reachable. Import added and a new `test_generate_tags_for_prompt_success_uses_cwd_prompts_dir` test in `tests/server/routes/test_architecture.py` exercises the full success path

### Docs

- **spell out `None`/omitted no-op contract on `CodePatchResult.patched_code`**: the `Field` description on the Pydantic model and the matching NOTE in `pdd/prompts/incremental_code_generator_python.prompt` now document that omitting the field or emitting JSON `null` is the valid "no change needed" signal, so a future auto-heal regeneration pass cannot silently re-tighten the field back to a required string

### Build

- **auto-healed prompt/example drift**: refreshed prompt templates for the tolerant-architecture rollout — new prompts added for `architecture_include_validation`, `architecture_registry`, and `auto_deps_architecture`; `agentic_architecture_python.prompt`, `render_mermaid_python.prompt`, `code_generator_main_python.prompt`, `core/duplicate_cli_guard_python.prompt`, `incremental_code_generator_python.prompt`, `server/routes/architecture_python.prompt`, `split_python.prompt`, and `llm_invoke_python.prompt` re-synced so auto-heal regeneration cannot revert back to array-only implementations or restore the meta-framing removed in v0.0.215

## v0.0.215 (2026-04-21)

### Fix

- **`duplicate_cli_guard` — only record successful runs so failed invocations can be retried immediately (#1275)**: the dedup guard persisted every guarded invocation regardless of outcome, so a failed `pdd sync` (hang, network error, returned-`None` failure sentinel) blocked an immediate retry until the dedup window expired. `record_after_guarded_command` now takes `success: bool = True` and skips recording when `success=False`; `process_commands` derives the flag via a new `_derive_success_from_normalized_results` helper that pins the "None entry signals failure" convention in one place. Click's `result_callback` already skipped on exceptions; this fix closes the "cleanly returned failure sentinel" gap
...
- **E2E environment pre-flight check (issue #791)**: `_check_e2e_environment()` skips Step 2 immediately when playwright/npx is unavailable, and `skipped_steps` memory persists across cycles to prevent identical timeout retries (~45-60 min saved)
- **Anti-TDD test detection (issue #794)**: bug orchestrator Step 10 now runs independent pytest verification — tests that pass on buggy code are rejected as anti-TDD instead of trusting LLM self-reporting
- **Test file discovery via merge-base diff (issue #794)**: `_get_modified_and_untracked()` finds files committed on the feature branch (not just uncommitted changes), plus directory-scan fallback when git-based discovery finds nothing new

### Fix

- **Duplicate prompt file creation in auto-deps/fix commands**: `generate_output_paths` now prioritizes `input_file_dirs` over context config (`.pddrc`) for fix/auto-deps, preventing duplicate directory structures like `frontend/prompts/frontend/...`
- **Commit-and-push with already-committed files**: `_commit_and_push` handles "nothing to commit" when merge-base fallback files were already committed — pushes existing unpushed commits instead of failing

### Test

- New E2E test `test_e2e_issue_791_e2e_timeout_retry.py` exercising full orchestrator with E2E environment skip and cross-cycle memory
- New `test_issue_794_anti_tdd_and_test_discovery.py` covering anti-TDD detection and merge-base test file discovery
- New `test_generate_output_paths_regression.py` with regression tests for fix/auto-deps path duplication
- New `test_issue_791_reproduction.py` and `test_issue_794_repro.py` reproduction tests
- Stale pipeline status recovery test coverage
