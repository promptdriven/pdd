...
- **local-routing**: add `--provider TEXT` global option (also reads `PDD_PROVIDER`) so `pdd --local --provider gemini sync ...` is unambiguous when multiple local credentials are configured. Resolution order: (1) canonical-alias lookup against the litellm prefix table (`vertex_ai`, `azure_ai`, `gemini`, `anthropic`) and a normalized view of distinct CSV provider names (e.g. `azure_openai`, `github_copilot`); (1b) for a canonical alias that resolves but has no exact provider rows, an anchored `<alias>/` model-routing-prefix match (e.g. a legacy `~/.pdd/llm_model.csv` that labels both backends under one `Google` provider with `gemini/...` and `vertex_ai/...` model prefixes resolves `gemini`/`vertex_ai` to the right rows without cross-routing); (2) for non-reserved tokens only, case-insensitive substring fallback on the CSV `provider` column; (3) only if both produce zero rows, substring fallback on the `model` column. The documented canonical aliases (`openai`, `azure_openai`, `vertex_ai`, `azure_ai`, `anthropic`, `gemini`, `github_copilot`, `openrouter`, `perplexity`) are **reserved**: they match only the provider column or an anchored `<alias>/` model prefix and never fall through to a loose substring, so `--provider openai` on a CSV whose only `openai`-bearing row is a model named `lm_studio/openai/gpt-oss-120b` aborts instead of silently routing to lm_studio. The provider-first precedence plus anchored-prefix matching prevents cross-routed rows (e.g. `bedrock/anthropic.claude-...`, `github_copilot/gemini-...`) from silently leaking into an `anthropic` or `gemini` pin. All substring and prefix fallbacks raise `RuntimeError` listing the matched providers when the token spans more than one — so bare tokens like `open`/`azure` (provider column) and `claude`/`gpt` (model column), and a `<alias>/` prefix that spans two providers, fail loudly instead of cross-routing. The filter applies before strength-based selection so ELO/cost interpolation runs over the user's chosen provider only. An unknown pin aborts with a clear error listing the available providers — PDD never silently routes to a different provider. `_is_permanent_invalid_request_error` is intentionally left untouched: the Anthropic Opus 4.7 `thinking.type` `"is not supported for this model"` fast-fail from #1158 is preserved, and Copilot's verbatim `"the requested model is not supported"` error already cascades correctly because its word order does not match the existing marker (#1202).
