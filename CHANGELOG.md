...
- **ci: `/heal` comment actively clears the `heal` check-run on bot/flake PRs**: new "Mirror conclusion to `heal` check-run on PR head" step in `.github/workflows/auto-heal.yml` that, on every `issue_comment` `/heal` run, posts a fresh `heal` check-run on the PR head SHA with the run's final conclusion. Prior behavior: `issue_comment` events run in default-branch context so GitHub Actions' auto-posted `heal` check-run lands on `main`, not the PR head — the pre-existing `heal: failure` from the bot-author `pull_request_target` auth-gate (or a Cloud-Build flake on a human PR) sat permanently red even after a successful `/heal`. GitHub's checks-API rollup uses the most recently updated check-run per `(name, app)` on a SHA, and `GITHUB_TOKEN` runs as the same `github-actions` app that auto-posts the original, so the fresh check-run becomes authoritative. Scoped to `issue_comment` only (`pull_request_target` already gets a fresh GHA-auto-post), `fork == 'false'`, `skip != 'true'`, and `always()` so failures refresh state too. Matches operator mental model: comment `/heal`, red flips. Branch-protection semantics unchanged (only the on-disk state of the existing `heal` check-run is refreshed; no new required check name).
