...
- **local-routing**: add `--provider TEXT` global option (also reads `PDD_PROVIDER`) so `pdd --local --provider gemini sync ...` is unambiguous when multiple local credentials are configured. The pin is a case-insensitive substring match against the CSV `provider`/`model` columns. Also narrow `_is_permanent_invalid_request_error` so `Github_copilotException` "is not supported for this model" failures cascade to the next CSV candidate rather than aborting the run; the Anthropic `thinking.type`/`unsupported parameter` shape errors still fast-fail as before (#1202).
