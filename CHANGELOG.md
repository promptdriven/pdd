...
- **auto-heal**: resolve changed code, prompt, context, and flattened test names through canonical architecture/prompt ownership before drift classification. Path-qualified identities now distinguish root and nested duplicate basenames (for example `pdd/cli` versus `core/cli`); ambiguous or unmapped managed candidates fail closed instead of being guessed (#2059).
