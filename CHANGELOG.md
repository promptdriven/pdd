...
- **checkup**: add a PR-Scope Convergence Guard to `pdd checkup --pr` so the workflow cannot hide failing tests, let the fixer push unrelated code, or loop forever on a clean signal (#1228, related to #1212). Step 5 now emits a machine-parseable `<step5_signal>` block (plus `.pdd/checkup-context/step5-junit.xml` for pytest runners) with exact failing nodeids, files, and tracebacks. The orchestrator computes an in-scope path set — `(PR's changed files) ∪ (failing-test files) ∪ (test files for changed sources)` — and substitutes it into the Step 6a fixer prompt; the fixer MUST stay inside that set or list an explicit `EXPANSION_ITEMS:` block. A new pre-push diff sanity gate caps added LOC at `max(10 × failing-test-surface-LOC, 200)` (override via `BUDGET_OVERRIDE:` in `EXPANSION_ITEMS`). Step 7's report carries a `causality_check` object (`related_files`, `unrelated_files`, `unresolved_failures`); the orchestrator independently re-runs `causality_check()` and that result is authoritative. A clean Step 5 (`exit_code: 0`, `failures: []`, no critical findings from Steps 2-4) short-circuits to zero commits/pushes. All five helpers (`parse_step5_signal`, `compute_in_scope_paths`, `enforce_scope`, `diff_sanity_gate`, `causality_check`) live in `pdd/checkup_gates.py` alongside the existing #1092 deterministic gates and share the same fail-closed-on-violation / fail-open-on-parse-error contract.
