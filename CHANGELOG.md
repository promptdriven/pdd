...
- **test**: add `pdd test --from-story <story>` — a deterministic, offline generator
  that compiles a story's sibling contract (`## Covers` rule IDs, `## Oracle`,
  `## Negative Cases`, plus optional `## Entry Point` / `## Seams`) into runnable
  pytest tagged `@pytest.mark.story(story_id="<slug>")` and named
  `test_story_<slug>_R<n>_*`. Tests run with zero API keys (declared seams
  monkeypatched, seeds/clock frozen) and carry a `story-hash` header for
  byte-stable idempotent regeneration. Registers the `story` marker in
  `pyproject.toml` / `pytest.ini`; `## Entry Point` / `## Seams` are additive
  optional contract sections (existing contracts unaffected) (#1700).
