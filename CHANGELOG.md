...
- **model-catalog**: flip `azure_ai/claude-opus-4-7` from `reasoning_type=budget` to `reasoning_type=adaptive` and `max_reasoning_tokens=16000` now that Azure AI has adopted the same adaptive-thinking enforcement as the direct Anthropic API (legacy `thinking.type="enabled"` 400s on these models); extend `_infer_reasoning_type` and `_infer_max_reasoning_tokens` in `generate_model_catalog.py` to include `azure_ai/claude-opus-4-7` in the adaptive allowlist; extend the adaptive-serialization branch in `llm_invoke.py` to trigger on `provider_lower in ('anthropic', 'azure ai')` (#1163, related to #1156).
