...
- **checkup**: Step 7 verdict parser now selects the LAST top-level JSON object in the output (via `json.JSONDecoder.raw_decode`), matching the prompt contract that the verdict be last. Earlier intermediate-reasoning blocks — fenced or unfenced — can no longer mask a final `issue_aligned=false` (round-5 Finding 2).
- **checkup**: PR mode now reloads `architecture.json` and `.pddrc` from the PR worktree after checkout so downstream prompts audit the PR's project state, not the pre-PR parent checkout's. The refresh also runs on the resume path (round-5 Finding 3).
