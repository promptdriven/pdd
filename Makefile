...
publish-public:
	...
	@if git -C "$(PUBLIC_PDD_REPO_DIR)" rev-parse --is-inside-work-tree >/dev/null 2>&1; then \
		cd "$(PUBLIC_PDD_REPO_DIR)" && git fetch origin && git rebase origin/main && git add . && git commit -m "Bump version" && git push; \
		else \
...
publish-public-cap:
	...
	@if git -C "$(PUBLIC_PDD_CAP_REPO_DIR)" rev-parse --is-inside-work-tree >/dev/null 2>&1; then \
		cd "$(PUBLIC_PDD_CAP_REPO_DIR)" && git fetch origin && git rebase origin/main && git add . && git commit -m "Bump version" && git push; \
		else \
