...
# Section 3.12: Subtitle Track — Rolling Word-by-Word Narration

**Tool:** Remotion
**Duration:** ~280.73s (full Part 3)
**Timestamp:** 0:00 - 4:40

## Visual Description
A persistent subtitle overlay at the bottom third of the frame, displaying narration text word-by-word in sync with the audio. The subtitle uses a 12-word rolling window — as new words appear, oldest words scroll off the left. The current word highlights briefly as it's spoken. The text sits on a semi-transparent dark band that ensures readability over any Veo background. The subtitle track runs across the entire Part 3 duration, appearing after the title card fades and persisting until the final fade-to-black.
...
- Backdrop band: full-width rectangle from y=860 to y=1020
  ...
  - Top/bottom edge: feathered gradient (20px), hard band to transparent
...
5. **Final 30 frames:** Subtitle fades out with the overall fade-to-black.

### Word Timing
- Source: `part3_mold/word_timestamps.json`
...
- Silence gap: 0.3s default between segments
...
## Narration Sync
> Synced to every word in the Part 3 narration via word_timestamps.json
...
```typescript
<Sequence from={SUBTITLE_START} durationInFrames={PART3_TOTAL_FRAMES}>
  <AbsoluteFill style={{ pointerEvents: 'none' }}>
    {/* Backdrop band */}
    <SubtitleBackdrop
      opacity={interpolate(frame, [0, 15, PART3_TOTAL_FRAMES - 30, PART3_TOTAL_FRAMES], [0, 0.55, 0.55, 0])}
    />
...
```json
{
  "word_timestamps_file": "part3_mold/word_timestamps.json",
  "narration_audio": "part3_mold/narration.wav",
  "segments": "part3_mold_001 through part3_mold_025",
  "silence_gap": 0.3,
  "window_size": 12,
