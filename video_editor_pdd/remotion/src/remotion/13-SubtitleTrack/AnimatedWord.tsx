...
const RECENT_THRESHOLD_FRAMES = Math.round(0.5 * FPS);
...
export const AnimatedWord: React.FC<AnimatedWordProps> = ({
  ...
  // --- Recent/older transition: smooth opacity fade after word ends ---
  let fadedOpacity = targetOpacity;
  if (state === "recent" && wordEndFrame > 0) {
    const fadeProgress = interpolate(
      frame,
      [wordEndFrame, wordEndFrame + RECENT_THRESHOLD_FRAMES],
      [0, 1],
      { extrapolateLeft: "clamp", extrapolateRight: "clamp" }
    );
    const easedFade = Easing.out(Easing.quad)(fadeProgress);
    fadedOpacity = interpolate(easedFade, [0, 1], [1.0, RECENT_OPACITY]);
  }
  ...
  const baseOpacity = state === "recent" ? fadedOpacity : targetOpacity;
  const finalOpacity = baseOpacity * exitOpacity * appearOpacity;

  // Green underline only on current word
