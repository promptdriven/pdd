import { z } from "zod";

export const SECTION_FPS = 30;
export const SECTION_DURATION_SECONDS = 20.420;
export const SECTION_DURATION_FRAMES = Math.ceil(SECTION_FPS * SECTION_DURATION_SECONDS);

const s2f = (seconds: number) => Math.round(seconds * SECTION_FPS);

export const BEATS = {
  VISUAL_00_START: s2f(0.800),
  VISUAL_00_END: s2f(11.200),
  VISUAL_01_START: s2f(14.586),
  VISUAL_01_END: s2f(20.420),
  VISUAL_02_START: s2f(16.740),
  VISUAL_02_END: s2f(18.340),
  VISUAL_03_START: s2f(18.560),
  VISUAL_03_END: s2f(20.420),
  VISUAL_04_START: s2f(18.560),
  VISUAL_04_END: s2f(20.420),
};

export const VISUAL_SEQUENCE = [
  { start: BEATS.VISUAL_00_START, end: BEATS.VISUAL_00_END, id: "01_split_screen_darning", desc: "01 split screen darning", lane: 0 },
  { start: BEATS.VISUAL_01_START, end: BEATS.VISUAL_01_END, id: "06_sock_toss", desc: "06 sock toss", lane: 0 },
  { start: BEATS.VISUAL_02_START, end: BEATS.VISUAL_02_END, id: "07_code_cursor_blink", desc: "07 code cursor blink", lane: 0 },
  { start: BEATS.VISUAL_03_START, end: BEATS.VISUAL_03_END, id: "08_code_regeneration", desc: "08 code regeneration", lane: 0 },
  { start: BEATS.VISUAL_04_START, end: BEATS.VISUAL_04_END, id: "09_title_card_pdd", desc: "09 title card pdd", lane: 0 },
];

export const ColdOpenSectionProps = z.object({
  showTitle: z.boolean().default(true),
});

export const defaultColdOpenSectionProps: z.infer<typeof ColdOpenSectionProps> = {
  showTitle: true,
};

export type ColdOpenSectionPropsType = z.infer<typeof ColdOpenSectionProps>;
