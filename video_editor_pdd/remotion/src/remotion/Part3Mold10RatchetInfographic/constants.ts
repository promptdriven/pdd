...
export const BAR_DATA: BarData[] = (() => {
  ...
  for (let i = 0; i < TOTAL_BARS; i++) {
    ...
    // Heights grow over time with some randomness
    const baseHeight = BAR_MIN_HEIGHT + t * (BAR_MAX_HEIGHT - BAR_MIN_HEIGHT) * 0.6;
    const randomFactor = 0.5 + seededRandom(i) * 1.0;
    const height = Math.min(BAR_MAX_HEIGHT, baseHeight * randomFactor + 20);
