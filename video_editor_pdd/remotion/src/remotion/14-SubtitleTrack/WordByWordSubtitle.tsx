...
export const WordByWordSubtitle: React.FC<WordByWordSubtitleProps> = ({
  ...
  // Determine word state: current, recent (spoken within last 0.5s), or older
  const getWordState = (globalIndex: number): WordState => {
    ...
    // Check if this word ended within RECENT_WINDOW_FRAMES of current frame
    const wordEntry = appearedWords[globalIndex];
    if (wordEntry && frame - wordEntry.endFrame < RECENT_WINDOW_FRAMES) {
      return "recent";
    }
