...
export const IndustryColumn: React.FC<IndustryColumnProps> = ({
  ...
  // Slide in from left: translateX(-100) → 0
  const slideX = interpolate(frame, [slideStart, slideEnd], [-100, 0], {
    extrapolateLeft: "clamp",
    extrapolateRight: "clamp",
  });

  // Column opacity: fade in with slide, fade out at end
  ...
  // BEFORE row dims from 1.0 → 0.6 during hold
  ...
  // AFTER icon scales in with spring
  const afterScale =
  ...
  // AFTER text fades in over 30 frames after reveal
  const afterTextOpacity =
    frame >= AFTER_REVEAL_START
      ? interpolate(
          frame,
          [AFTER_REVEAL_START, AFTER_REVEAL_START + 30],
          [0, 1],
          { extrapolateRight: "clamp" },
        )
      : 0;
