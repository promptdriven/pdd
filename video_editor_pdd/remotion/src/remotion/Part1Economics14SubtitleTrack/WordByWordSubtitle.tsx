...
export const WordByWordSubtitle: React.FC<WordByWordSubtitleProps> = ({
  ...
  // The sequence starts at SUBTITLE_START_FRAME, so the local frame
  // within this Sequence is `frame`. But word startFrames are absolute,
  // so we need to use absolute frame for matching.
  ...
  const getWordState = (
    ...
    // "Recent" = word ended within last RECENT_WINDOW_FRAMES
    if (absoluteFrame - w.endFrame <= RECENT_WINDOW_FRAMES) return "recent";

    return "older";
  };

  // Smooth horizontal scroll when window shifts (easeInOutQuad)
  const scrollOffset = interpolate(
    appearedWords.length,
