...
function computeParts(): PartInfo[] {
  ...
  // Find the part that will be roughly at 40% across the conveyor at STREAM_PAUSE
  const conveyorTargetX = CONVEYOR_X_START + (CONVEYOR_X_END - CONVEYOR_X_START) * 0.4;
  ...
  for (let i = 0; i < parts.length; i++) {
    ...
    const dist = Math.abs(x - conveyorTargetX);
    if (dist < bestDist) {
      bestDist = dist;
      bestIdx = i;
    }
...
const SinglePart: React.FC<SinglePartProps> = ({ part, globalFrame }) => {
  ...
  // Compute travel position — defective part freezes once defect appears
  ...
  if (part.isDefect) {
    ...
    // Flash red once defect is revealed
    ...
    // Dissolve at fix time (opacity 1→0, scale 1→0.5)
