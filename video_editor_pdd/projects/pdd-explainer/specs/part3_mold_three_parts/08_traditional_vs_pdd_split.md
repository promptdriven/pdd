...
**LEFT — "Traditional":** A cascade of patch-and-repeat. "Bug found" → "Patch code" → "Similar bug elsewhere" → "Patch again" → "Different bug" → "Patch again..." Each step is a red-tinted card. The chain grows longer, implying endless repetition. A subtle exhaustion — the steps get slightly less crisp, slightly more faded, as the cycle continues.
...
> "And sometimes the bug isn't in the code — it's in the prompt. The code correctly implements a wrong specification."

Segments: `part3_mold_three_parts_015`, `part3_mold_three_parts_016`

- **2:15** ("traditional development"): Split screen appears, both timelines building
- **2:24** ("PDD, a test prevents"): Right side reaches "Bug impossible forever"
- **2:33** ("sometimes the bug isn't in the code"): Transition out
...
```json
{
  ...
  "narrationSegments": ["part3_mold_three_parts_015", "part3_mold_three_parts_016"]
}
```
