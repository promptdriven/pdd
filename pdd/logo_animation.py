...
def _parse_logo_art(logo_art_lines: List[str]) -> List[AnimatedParticle]:
...
def _get_centered_logo_positions(
    particles: List[AnimatedParticle],
    logo_art_lines: List[str],
    console_width: int,
    console_height: int
) -> List[Tuple[int, int]]:
    """Calculates target positions for particles to form the centered logo."""
    if not logo_art_lines: return [(0,0)] * len(particles)
...
def _render_particles_to_text(
    ...
    char_grid = [[' ' for _ in range(console_width)] for _ in range(fixed_height)]
    ...
    style_map = [[base_style for _ in range(console_width)] for _ in range(fixed_height)]
    ...
    for p in particles:
        if p.visible:
            ...
            if 0 <= y < fixed_height and 0 <= x < console_width:
    ...
    for r_idx in range(fixed_height):
...
def _animation_loop(console: Console):
    ...
    # Ensure ASCII_LOGO_ART is a list of strings
    local_ascii_logo_art = ASCII_LOGO_ART
    ...
    if not particles: return # No particles to animate

    console_width, console_height = console.width, 18
    ...
    logo_target_positions = _get_centered_logo_positions(particles, local_ascii_logo_art, console_width, console_height)
    for i, p in enumerate(particles):
        p.start_x = 0.0  # Start at bottom-left
        p.start_y = float(console_height - 1)
    ...
    with Live(console=console, refresh_per_second=effective_frame_rate, transient=True, screen=True) as live:
        ...
        while not _stop_animation_event.is_set():
            ...
            live.update(_render_particles_to_text(particles, console_width, console_height))
        ...
        while not _stop_animation_event.is_set():
            ...
            live.update(_render_particles_to_text(particles, console_width, console_height)) # Keep rendering
            if _stop_animation_event.wait(frame_duration): break
        
        if _stop_animation_event.is_set(): return

        # Animation Stage 2: Expansion to Box
        box_target_positions = _get_box_perimeter_positions(particles, console_width, console_height)
        ...
        # Hold final frame until stop event is set
        while not _stop_animation_event.wait(0.1): # Check event periodically
            pass # Live object keeps the last frame displayed
...
def stop_logo_animation():
    ...
    if _animation_thread and _animation_thread.is_alive():
        ...
        timeout = (LOGO_FORMATION_DURATION or 0) + \
                  (LOGO_HOLD_DURATION or 0) + \
                  (LOGO_TO_BOX_TRANSITION_DURATION or 0) + \
                  2.0 # Add a buffer
...
def run_logo_animation_inline(console: Console, stop_event: threading.Event):
    ...
    particles = _parse_logo_art(local_ascii_logo_art)
    if not particles: 
        return # No particles to animate
    ...
    # Use Live with screen=True to match sync_animation positioning
    with Live(console=console, refresh_per_second=effective_frame_rate, transient=True, screen=True) as live:
        # Animation Stage 1: Formation
        ...
        while not stop_event.is_set():
            ...
            for p_obj in particles: 
                p_obj.update_progress(progress)
            live.update(_render_particles_to_text(particles, console_width, console_height))

            if progress >= 1.0: 
                break
            if stop_event.wait(frame_duration): 
                break
        ...
        hold_start_time = time.monotonic()
        while not stop_event.is_set():
            if time.monotonic() - hold_start_time >= hold_duration: 
                break
            live.update(_render_particles_to_text(particles, console_width, console_height))
            if stop_event.wait(frame_duration): 
                break
