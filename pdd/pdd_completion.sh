...
_pdd() {
    ...
    local checkup_opts="--validate-arch-includes --project-root --strict --no-fix --timeout-adder --start-step --no-github-state --pr --issue --review-loop --final-gate --review-only --reviewers --reviewer --fixer --reviewer-fallback --fixer-fallback --allow-same-reviewer-fixer --max-review-rounds --max-review-cost --max-review-minutes --require-all-reviewers-clean --no-require-all-reviewers-clean --continue-on-reviewer-limit --fallback-reviewer-on-failure --require-final-fresh-review --no-require-final-fresh-review --blocking-severities --clean-reviewer-states --test-scope --no-gates --gate-timeout --gate-allow"
