...
import { api, ExtractMetadata, ExtractContent, ExtractListResponse, PruneResponse } from '../api';
...
const ExtractsPanel: React.FC = () => {
  ...
  const handlePrune = useCallback(async () => {
    if (!window.confirm('Delete orphaned extracts not referenced by any prompt file? This cannot be undone.')) return;
    setPruning(true);
    setPruneResult(null);
    try {
      const result = await api.pruneExtracts();
      setPruneResult(result);
      await loadExtracts();
    } catch (e) {
      setError(e instanceof Error ? e.message : 'Failed to prune extracts');
    } finally {
      setPruning(false);
    }
  }, [loadExtracts]);
    ...
    <div>
      ...
      <div className="flex items-center gap-3 mb-4 text-sm">
        ...
        <button
          onClick={handlePrune}
          disabled={pruning}
          title="Delete cached extracts that are no longer referenced by any prompt file"
          className="ml-auto px-3 py-1 bg-red-600/20 hover:bg-red-600/40 text-red-400 border border-red-500/30 rounded-lg text-xs transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-1.5"
        >
          {pruning && <SpinnerIcon className="w-3 h-3 animate-spin" />}
          {pruning ? 'Pruning...' : 'Prune Orphaned Extracts'}
        </button>
      </div>
      {pruneResult && (
        <div className="mb-4 px-3 py-2 bg-green-500/10 border border-green-500/30 rounded-lg text-sm text-green-400">
          {pruneResult.message}
        </div>
      )}
