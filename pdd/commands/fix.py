...
def fix(
    ...
    Accepts one or more UNIT_TEST_FILES and an ERROR_FILE argument.

    Loop mode: ERROR_FILE is treated as an additional unit test file. All files
               are test files. Errors are generated by running the verification program.

    Non-loop mode: UNIT_TEST_FILES are test files, ERROR_FILE contains static errors.

    Each test file is processed separately, allowing the AI to run and fix tests
    individually rather than as a concatenated blob.
    """
    # Validate arguments based on mode
    if loop:
        # Loop mode: verification_program is required
        if not verification_program:
            raise click.UsageError("--verification-program is required when using --loop")

        # In loop mode, error_file is actually a test file (Click parsed last file as error_file)
        # Recombine it back into unit_test_files
        unit_test_files = unit_test_files + (error_file,)
        error_file = ""  # Will be generated by verification program
    else:
        # Non-loop mode: error_file stays as-is (contains static errors)
        pass

    # Ensure at least one unit test file is provided
