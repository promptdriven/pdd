...
def generate_mermaid_code(architecture, app_name="System"):
    ...
    # Match test expectation: add a trailing space only if quotes were present in the original
    prd_label = f'{escaped_app_name} ' if '"' in app_name else escaped_app_name
    ...
    for group_name, modules in [("Frontend", frontend), ("Backend", backend), ("Shared", shared)]:
        if modules:
            ...
            for m in modules:
                name = html.escape(Path(m['filename']).stem, quote=False)  # Escape the module name
                pri = html.escape(str(m.get('priority', 0)))  # Escape the priority
    ...
    # Dependencies
    for m in architecture:
        src = html.escape(Path(m['filename']).stem, quote=False)  # Escape the source module name
        for dep in m.get('dependencies', []):
            dst = html.escape(Path(dep).stem, quote=False)  # Escape the destination module name
    ...
    # Apply classes
    if frontend:
        lines.append(f"{INDENT * LEVELS['style']}class {','.join([html.escape(Path(m['filename']).stem, quote=False) for m in frontend])} frontend")
    if backend:
        lines.append(f"{INDENT * LEVELS['style']}class {','.join([html.escape(Path(m['filename']).stem, quote=False) for m in backend])} backend")
    if shared:
        lines.append(f"{INDENT * LEVELS['style']}class {','.join([html.escape(Path(m['filename']).stem, quote=False) for m in shared])} shared")
