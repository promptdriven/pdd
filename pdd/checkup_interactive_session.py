        ...
        if isinstance(report, Mapping):
            for finding in report.get("findings", []):
                if not isinstance(finding, Mapping):
                    continue
                _id = finding.get("id")
                finding_id = _id if isinstance(_id, str) and _id else finding.get("finding_id")
                options = finding.get("options")
                if isinstance(finding_id, str) and isinstance(options, Sequence) and not isinstance(options, str):
                    typed_options = [
