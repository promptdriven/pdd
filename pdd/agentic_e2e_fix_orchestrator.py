            ...
            for step_num in range(1, 10):
                ...
                if step_num == 9:
                    ...
                    if _step9_token in ("ALL_TESTS_PASS", "LOCAL_TESTS_PASS"):
                        ...
                        if not step_success:
                            # Step 9 failed (timeout/provider); continue so it runs again next cycle.
                            console.print("[yellow]Step 9 failed with no parseable loop-control token; starting next cycle.[/yellow]")
                            break

                        # Step 9 succeeded but no token: retry once to get an explicit loop control.
                        console.print("[yellow]No loop control token in Step 9 output; retrying Step 9 once.[/yellow]")
                        ...
                        if retry_token in ("ALL_TESTS_PASS", "LOCAL_TESTS_PASS"):
                            ...
                            if test_files:
                                verified, verify_output = _verify_tests_independently(test_files, cwd)
                                if verified:
                                    console.print("[green]LOCAL_TESTS_PASS verified by independent pytest run (Step 9 retry).[/green]")
                                    success = True
                                    final_message = "All tests passed after fixes (independently verified)."
                                    break
                                else:
                                    console.print("[bold red]LLM claimed tests pass at Step 9 retry but independent verification FAILED.[/bold red]")
                            ...
                            console.print("[bold yellow]Step 9 did not emit a loop control token after retry; stopping.[/bold yellow]")
                            final_message = "Workflow stopped: Step 9 did not emit a loop control token after retry."
                            break
                    else:
