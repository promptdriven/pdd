...
def _one_line(value: str, *, max_chars: int = 240) -> str:
    """Collapse free-form text for PR metadata."""
...
def _render_fix_pr_body(
    *,
    ...
    """Render the final fix PR body after the bug-phase draft has been solved."""
    files = sorted({path for path in changed_files if path})
    changed_files_section = "\n".join(f"- `{path}`" for path in files)
    if not changed_files_section:
        changed_files_section = "- No tracked file changes were detected by PDD."

    verification_lines = ["- PDD bug/fix workflow completed successfully."]
    if ci_message:
        verification_lines.append(f"- CI validation: {_one_line(ci_message)}")
    verification_lines.append(
        "- Final PDD checkup/verifier status is recorded in the latest PR or "
        "issue comment."
    )

    issue_summary = _one_line(issue_title) or f"Issue #{issue_number}"
    return f"""## Summary
Fixes #{issue_number} by implementing the code changes for: {issue_summary}
...
---
*Generated by PDD agentic bug/fix workflow*
"""
...
def _refresh_fix_pr_metadata(
    *,
    ...
    """Replace stale bug-phase PR metadata with the final fix summary."""
    ...
    if edit.returncode != 0:
        return False, f"Failed to update PR #{pr_number} metadata: {edit.stderr}"
    ...
    ready_output = f"{ready.stdout}\n{ready.stderr}".lower()
    ready_idempotent = "not a draft" in ready_output or "already ready" in ready_output
    if ready.returncode != 0 and not ready_idempotent:
        return False, f"Failed to mark PR #{pr_number} ready: {ready.stderr}"

    return True, f"Updated PR #{pr_number} metadata and marked ready for review"
    ...
    try:
        ...
        if success:
            ...
            if skip_ci:
                ...
                metadata_ok, metadata_message = _refresh_fix_pr_metadata(
                    cwd=cwd,
                    issue_number=issue_number,
                    issue_url=issue_url,
                    issue_title=issue_title,
                ...
                if metadata_ok:
                    if not quiet:
                        console.print(f"[green]{metadata_message}[/green]")
                else:
                    return False, metadata_message, total_cost, model_used, changed_files
