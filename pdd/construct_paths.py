...
# Module-level valid keys for .pddrc validation (Issue #1198)
_VALID_PDDRC_ROOT_KEYS = {'version', 'contexts'}
_VALID_CONTEXT_KEYS = {'paths', 'defaults'}
_VALID_DEFAULTS_KEYS = {
    'generate_output_path', 'test_output_path', 'example_output_path',
    'prompts_dir', 'prompt_path', 'default_language', 'target_coverage',
    'strength', 'temperature', 'budget', 'max_attempts', 'outputs'
}
...
def _validate_pddrc_keys(config: Dict[str, Any], pddrc_path: Path) -> None:
    """Validate .pddrc keys against known valid schema keys and emit warnings."""
                ...
                if isinstance(defaults, dict):
                    for def_key in defaults.keys():
                        if def_key not in _VALID_DEFAULTS_KEYS:
                            logging.warning(f"WARNING: .pddrc contains unknown key '{def_key}' at path 'contexts.{ctx_name}.defaults.{def_key}', ignored. Run 'pdd setup' to regenerate.")
