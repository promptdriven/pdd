...
_CANDIDATE_TRANSACTION_SOURCE = "\n".join((
    "import hashlib,json,os,pathlib,selectors,signal,stat,subprocess,sys,time,venv",
    _VENV_TREE_VALIDATOR_SOURCE.strip(),
    ...
    "        remaining=deadline-time.monotonic()",
    "        if remaining<=0: failure='lifecycle child deadline expired';break",
    "        for key,_mask in selector.select(min(remaining,.05)):",
    "            stream=key.fileobj",
    ...
    "                failure='lifecycle child output exceeded limit';break",
    "            target.extend(chunk)",
    "        if failure: break",
    "    if failure:",
    ...
    "        try: os.killpg(child.pid,signal.SIGKILL)",
    "        except ProcessLookupError: pass",
    "        child.wait();raise RuntimeError('lifecycle child deadline expired') from None",
