...
def _prompt_path_candidates(prompt_path: Path, base: Path) -> tuple[str, ...]:
    """Return canonical and compatibility-alias paths for context matching."""
    candidates: list[str] = []
    for path in (prompt_path, prompt_path.resolve(strict=False)):
        ...
        # This repository retains ``prompts -> pdd/prompts`` for compatibility.
...
def _context_path_specificity(
    ...
    """Score matching context paths to resolve shared prompt-root ownership."""
