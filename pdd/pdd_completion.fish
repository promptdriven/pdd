...
# checkup command (local diagnostics + PR/issue review-loop)
complete -c pdd -n "__fish_seen_subcommand_from checkup" -l validate-arch-includes -d "Cross-validate architecture.json vs prompt includes (local)"
complete -c pdd -n "__fish_seen_subcommand_from checkup" -l project-root -r -d "Project root for --validate-arch-includes"
complete -c pdd -n "__fish_seen_subcommand_from checkup" -l strict -d "Treat validation drift as error"
complete -c pdd -n "__fish_seen_subcommand_from checkup" -l no-fix -d "Report-only; do not apply fixes"
complete -c pdd -n "__fish_seen_subcommand_from checkup" -l timeout-adder -x -d "Add seconds to each step's timeout"
complete -c pdd -n "__fish_seen_subcommand_from checkup" -l no-github-state -d "Disable GitHub state persistence"
complete -c pdd -n "__fish_seen_subcommand_from checkup" -l pr -r -d "Verify an existing PR URL"
complete -c pdd -n "__fish_seen_subcommand_from checkup" -l issue -r -d "Source issue URL (paired with --pr)"
complete -c pdd -n "__fish_seen_subcommand_from checkup" -l review-loop -d "Run primary-reviewer/fixer review loop on PR"
complete -c pdd -n "__fish_seen_subcommand_from checkup" -l review-only -d "Only run primary reviewer first pass (with --review-loop)"
complete -c pdd -n "__fish_seen_subcommand_from checkup" -l reviewers -x -d "Comma-separated role order (default codex,claude)"
complete -c pdd -n "__fish_seen_subcommand_from checkup" -l reviewer -x -d "Explicit primary reviewer role"
complete -c pdd -n "__fish_seen_subcommand_from checkup" -l fixer -x -d "Explicit fixer role"
complete -c pdd -n "__fish_seen_subcommand_from checkup" -l max-review-rounds -x -d "Maximum reviewer/fixer rounds (default 5)"
complete -c pdd -n "__fish_seen_subcommand_from checkup" -l max-review-cost -x -d "Maximum review-loop USD cost (default 10.0)"
complete -c pdd -n "__fish_seen_subcommand_from checkup" -l max-review-minutes -x -d "Maximum review-loop wall-clock minutes (default 90.0)"
complete -c pdd -n "__fish_seen_subcommand_from checkup" -l require-all-reviewers-clean -d "Require all reviewers clean (default)"
complete -c pdd -n "__fish_seen_subcommand_from checkup" -l no-require-all-reviewers-clean -d "Do not require all reviewers clean"
complete -c pdd -n "__fish_seen_subcommand_from checkup" -l continue-on-reviewer-limit -d "Report provider/rate/timeout failures as degraded instead of failed"
complete -c pdd -n "__fish_seen_subcommand_from checkup" -l require-final-fresh-review -d "Require a final fresh reviewer pass (default)"
complete -c pdd -n "__fish_seen_subcommand_from checkup" -l no-require-final-fresh-review -d "Do not require a final fresh reviewer pass"
complete -c pdd -n "__fish_seen_subcommand_from checkup" -l blocking-severities -x -d "Comma-separated highest-priority severities (default blocker,critical,medium)"
complete -c pdd -n "__fish_seen_subcommand_from checkup" -l clean-reviewer-states -x -d "Compatibility parser for reviewer-status gates (default clean)"
