...
complete -c pdd -n "__fish_seen_subcommand_from checkup" -l final-gate -d "Canonical final PR gate: PR checkup then review-loop (needs --pr; --issue optional)"
...
complete -c pdd -n "__fish_seen_subcommand_from checkup" -l gate-allow -x -d "Repeatable: extra gate name to opt into discovery"
