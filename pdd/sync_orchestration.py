                                ...
                                try:
                                    ...
                                    for _conform_attempt in range(MAX_CONFORMANCE_ATTEMPTS):
                                        try:
                                            ...
                                            result = code_generator_main(ctx, prompt_file=str(pdd_files['prompt'].resolve()), output=str(pdd_files['code'].resolve()), original_prompt_file_path=None, force_incremental_flag=False, output_from_config=True, snapshot_context=snapshot_context, compressed_context=_phase_compressed_context('generate', os.environ.get("PDD_REPAIR_DIRECTIVE")))
                                ...
                                if agentic_mode and language.lower() in ('typescript', 'javascript', 'typescriptreact', 'javascriptreact') and pdd_files['code'].exists():
                                    ...
                                    if unresolved:
                                        error_msg = (
                                            f"Hallucinated imports detected in generated code: "
                                            f"{', '.join(unresolved)}. "
                                            f"These modules do not exist on disk or in the project."
                                        )
                                        errors.append(error_msg)
                                        log_event(basename, language, "import_validation_failed", {
                                            "unresolved_imports": unresolved,
