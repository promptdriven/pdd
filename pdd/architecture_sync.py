...
def _extract_metadata_header(prompt_content: str) -> str:
    """
    Return the metadata header region (before first instruction section marker).

    PDD prompts conventionally start instruction bodies with `%` section lines.
    Metadata tags must be parsed only from the preamble before the first `%`
    line to avoid extracting instructional examples from prompt bodies.
    """
...
def parse_prompt_tags(prompt_content: str) -> Dict[str, Any]:
    ...
    try:
        ...
        result['has_dependency_tags'] = len(dep_elems) > 0 or '<pdd-dependency>' in header_content
        result['dependencies'] = [
            elem.text.strip()
