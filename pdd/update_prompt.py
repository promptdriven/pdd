...
def update_prompt(
    ...
    if not is_new_prompt_generation:
        # For updating an existing prompt, input_prompt and input_code must be non-empty.
        if not input_prompt.strip() or not input_code.strip():
            raise ValueError("For updating an existing prompt, input_prompt and input_code must be non-empty.")
