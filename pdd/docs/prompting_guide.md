...
References: [`docs/whitepaper.md`](../../docs/whitepaper.md), [prompt-driven-development-doctrine.md](../../docs/prompt-driven-development-doctrine.md), [`README.md`](../../README.md) (repo structure, conventions), [Effective Context Engineering](https://www.anthropic.com/engineering/effective-context-engineering-for-ai-agents), [Anthropic Prompt Engineering Overview](https://docs.anthropic.com/en/docs/build-with-claude/prompt-engineering/overview).
...
*For the conceptual foundation of why this works, see [The Mold Paradigm](../../docs/prompt-driven-development-doctrine.md#the-mold-paradigm) in the doctrine.*
...
- Regeneration preserves conceptual integrity and reduces long‑term maintenance cost (see [`docs/whitepaper.md`](../../docs/whitepaper.md)).
...
For a deeper exploration of why this paradigm shift matters—and an analogy to manufacturing's wood‑to‑plastic transition—see [The Mold Paradigm](../../docs/prompt-driven-development-doctrine.md#the-mold-paradigm) in the doctrine.
...
![Goldilocks Prompt](../../docs/goldilocks_prompt.jpeg)
...
- For ordinary code, one prompt → one file/module. For frontend work where appearance matters, follow the [Frontend UI Specifications](#frontend-ui-specifications) guidance: one written specification may describe a part of the interface that produces more than one code file, but each code file still has one generator.
...
Frontend requirements must be versioned just like requirements for any other
file generated by PDD. Generated frontend code is an output of those
requirements. A text prompt for one code file is still a good default for a
simple button, icon, or component whose appearance is already defined by the
shared design system. It is often insufficient for a dialog or screen. Written
instructions do not show
hierarchy, alignment, relative size, spacing, layering, or how a layout changes
with screen size. A picture does not explain state changes, keyboard behavior,
accessible names, or actions that must never happen.

When appearance is important, create a **frontend UI specification**. It may
describe a reusable component, dialog, screen, or user flow. Choose something
that can be generated and reviewed on its own, even if producing it requires
more than one code file. Each generated code file must still have exactly one
generator. A screen specification may show where a separately specified shared
component appears, but it must not redefine the inside of that component.

This recommendation is based on existing UI research, standards, and tools. It
has not yet been proven to be the best choice for every frontend. Before using
it across a large project, generate representative interfaces several times
from the same requirements. Use the same behavior, appearance, screen-size,
and accessibility tests for each approach. Compare text-only instructions,
instructions with images, and the files described below for consistency,
editing effort, cost, and test results.

### Decide Which Files Are Needed

Use only the files required to make the intended result clear to a reviewer.

| UI kind | Requirements files | Typical tests |
|---|---|---|
| Simple component whose appearance comes from the shared design system | Short text prompt and shared design-system include | Component behavior and accessibility tests |
| Component where layout or appearance matters | Text prompt, one named visual reference, and a small YAML index | State, appearance, and accessibility tests |
| Interactive component or dialog | Text prompt, state table, visual references, and YAML index | Interaction, forbidden-action, focus, keyboard, and appearance tests |
| Screen that changes with screen size | Text prompt, the minimum useful desktop/mobile references, written layout-change rules, and YAML index | Typical screen widths, widths near layout changes, overflow, and accessibility tests |
| User flow across screens | User story and a short transition or flow description linked to the screen specifications | Browser tests that cover the complete flow |

Do not create a visual reference for every breakpoint or possible state. Add
one when the layout changes in an important way; describe ordinary changes with
written rules and tests. A short Mermaid flow is normally enough for
navigation. Use a statechart only when simultaneous states, nested states,
cancellation, or recovery would otherwise be unclear.

### What Each File Describes

A frontend UI specification can contain three kinds of files:

1. **Short text prompt:** describes purpose, visible text, states,
   interactions, rules for different screen sizes, accessibility, and
   MUST/MUST NOT rules. It must not copy a React component tree, Tailwind class
   list, or measurements already shown in a visual reference.
2. **Visual reference:** an SVG, or another versioned visual file when more
   appropriate, shows a named state at a named screen size when layout or
   appearance matters. It should name meaningful regions and shared
   design-system values, not event handlers or a framework-specific DOM tree.
3. **Small YAML index:** records the specification name, the prompt used to
   generate the code, generated code files, required states and screen sizes,
   how strictly each
   visual reference must be followed, and where each requirement is written.
   It is a short index, not a new layout language or a second implementation.

Shared design-system prompts define values such as color, type, spacing,
radius, shadow, and motion. A frontend UI specification references their names
rather than copying hex values or utility classes.

Write each requirement in one file. For example, an SVG can specify a large
media region beside a narrower details region. The text prompt requires focus
to enter the dialog, Tab and Shift+Tab to stay inside it, Escape
to close it, background controls to be unavailable, and focus to return to the
opening control. The YAML index records which visual reference applies to each
state and screen size. Tests and generated code demonstrate compliance; they
do not replace the requirements files.

### State How Strictly a Visual Reference Must Be Followed

Give each visual reference one of these plain-language levels:

- **Example only:** communicates an idea or general appearance. A difference
  is not automatically a defect.
- **Layout requirements:** grouping, order, alignment, relative size, spacing,
  visible text, and named design-system values must be preserved. Browser font
  rendering and antialiasing normally do not have to match exactly.
- **Exact requirements:** listed measurements, visible text, and
  design-system values must match within a stated tolerance. Use this only when
  exact matching is necessary, and explain how it will be checked.

A raw whiteboard photo, annotated screenshot, or external design is useful
during design. Before generating code, correct its mistakes and omissions, then
record the decisions in the text prompt, YAML index, and visual references.
Label the original image as design input only. Do not leave an ambiguous sketch
and generated code as two conflicting descriptions of the intended result.

For a visual reference stored in the repository, an SVG whose groups and
regions have meaningful names is a promising default. It is directly viewable
and its XML can be reviewed in Git. Use stable human-readable IDs,
`title`/`desc`, and attributes such as `data-intent-role` and `data-token`.
Avoid editor metadata, generated IDs, hidden alternative layers, embedded
base64 files, external resources, and unnecessary formatting changes. SVG does
not describe interaction, accessibility behavior, changes between screen
sizes, or the production DOM.

Current PDD preprocessing reads SVG as text, not as image input. If the model
also needs to see the picture, render the SVG to PNG in a repeatable way and
record the SVG file hash beside the PNG. For a dense screen, provide a stable
overview and clearly named close-up images instead of one unreadable large
image. The SVG and text prompt remain the requirements; the PNG is generated
input for the model. A screenshot produced from generated frontend code is a
test result, not a requirement, unless a reviewer explicitly replaces the
original visual reference with it.

An HTML/CSS prototype is an option when resizing, timing, motion, or interaction
cannot be described clearly with fewer files. It must be substantially smaller
and less framework-specific than the generated implementation. State exactly
which requirements it specifies so it does not become a second frontend that
must be maintained separately.

### Describe States, Screen Sizes, and Accessibility

Interactive UI needs a short state table rather than instructions scattered
across several files. Each row should name what causes the state, any condition
that must be true, the visual reference to use, the resulting action, and what
must not happen. A dialog prompt must explicitly describe its accessible name,
where focus starts, how Tab and Shift+Tab stay inside the dialog, what Escape
and backdrop clicks do, whether background controls can be used, where focus
returns, how duplicate submission is prevented, and how errors are announced.
A dialog is incorrect if it looks right but keyboard focus can leave it.

For UI that changes with screen size, include only the visual references that
show important layout differences. Add a table that says what reorders, stacks,
collapses, becomes a drawer, scrolls, or must remain reachable. Test typical
screen widths and widths near each layout change, including long text,
overflow, and keyboard access. The order of shapes in an SVG does not determine
DOM or keyboard-focus order unless the text prompt explicitly requires it.

### Separate Requirements from Examples and Test Results

Give every file a clear role:

| File | Role |
|---|---|
| Whiteboard sketch, issue image, or external design | Design input only; record where it came from |
| Text prompt, YAML index, and visual references | Definitive UI requirements |
| Storybook/CSF story | Example and test setup that normally imports the generated component; not the independent design requirements |
| Playwright screenshot comparison | Appearance test run in a fixed browser environment |
| Playwright ARIA snapshot and keyboard assertions | Accessibility tests; not a replacement for written focus and interaction rules |
| Generated browser screenshot, trace, or PNG rendered from SVG | Generated test or preprocessing output |

Screenshot comparisons can show that rendering changed, but they depend on the
browser environment and do not explain whether a change is correct. ARIA
snapshots check roles, names, and hierarchy but do not prove focus timing or all
interactions. Link both to the state IDs and rules in the prompt. For important
requirements, keep some behavior, appearance, screen-size, and accessibility
tests unavailable to the generation model so they can detect overfitting.

### Specify Which Prompt Generates Each Code File

The YAML index records which prompt generates each code file. It does not allow
more than one generator for a file:

- A simple component normally has one prompt and one generated code file.
- One part of the interface may produce several code files through one declared
  generating prompt when those files cannot be generated and reviewed
  separately.
- Additional visual or written requirements may be included by that generating
  prompt, but they may not run as separate generators for the same code file.
- If several independently specified parts of the interface are implemented in
  one code file, either split the code file or select one prompt to generate it,
  then label the other specifications as additional input until the code can be
  split.
- Do not create one specification for the entire application or for every
  feature in a large client file. Write separate specifications for stable
  components, dialogs, screens, or user flows.

Use `<include>` for the YAML index and only the visual references needed for the
generation. Prefer a shared design-system or preamble include to copying shared
rules. Dynamic web, shell, and semantic-query context can change between runs;
use the snapshot workflow when it contains a critical requirement. Store
requirement visuals separately from generated screenshot and baseline
directories so reviewers cannot confuse their roles.

Example layout:

```text
prompts/ui/thumbnail-preview/
├── thumbnail-preview.ui.prompt       # behavior, states, and accessibility
├── thumbnail-preview.ui.yaml         # generating prompt, code files, and visual-reference list
├── views/
│   ├── open__1440x900.ui.svg          # desktop layout requirements
│   └── unavailable__390x844.ui.svg    # compact-screen layout requirements
└── references/
    └── whiteboard.jpg                 # design input only

.pdd/ui-renders/thumbnail-preview/     # generated PNG input and recorded SVG hash
```

The prompt used to generate the code stays short and includes only the needed
requirements files:

```xml
% Implement the `thumbnail-preview` component and its required states.
...
<states>
| State | What causes it | Visual reference | What must not happen |
...
### Checklist Before Relying on Frontend Regeneration

Before repeatedly regenerating part of the interface, confirm:

- [ ] The prompt used for generation is named, and every code file has exactly
  one generator.
- [ ] The prompt declares its public route, properties, events, states, and
  important MUST/MUST NOT rules.
- [ ] Each important state and screen size has a named visual reference or a
  written explanation of why a separate reference is unnecessary.
- [ ] Every visual reference says whether it is an example, layout
  requirements, or exact requirements; lists what it specifies; and names the
  state and screen size where it applies.
- [ ] Shared design-system values are referenced rather than copied.
- [ ] Dialogs and interactive controls have role/name, keyboard, focus,
  pending/error, and forbidden-action tests.
- [ ] Tests cover typical screen widths, widths immediately before and after
  each layout change, long content, order, overflow, focus containment, and
  reachability.
- [ ] Appearance tests use written review criteria for layout or exact
  requirements; screenshot comparisons run in a fixed environment and remain
  test results rather than requirements.
- [ ] ARIA snapshots, keyboard assertions, and behavior tests check
  accessibility separately from pixels; important cases include tests not
  shown to the generation model where practical.
- [ ] SVG files use stable formatting, and every PNG supplied as image input can
  be reproduced from its SVG with the recorded file hash.
- [ ] For important interfaces, repeated generations from unchanged
  requirements pass the same behavior, appearance, screen-size, and
  accessibility tests.

If this checklist is unnecessary for a simple component, keep its requirements
in a text prompt. If a visual reference, YAML index, or prototype does not
improve verified behavior enough to justify its editing and review cost, do not
standardize it.
