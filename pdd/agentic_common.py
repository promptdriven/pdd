...
# with a bounded ``readline(cap)`` and DROPS any line longer than this cap
# (draining the remainder in bounded segments) WITHOUT ever decoding it, so the
# parent never materializes more than ~cap bytes of any single line. The cap is
# deliberately generous: every event we retain (agent_message / result /
# session.end / turn.completed, and any error/failed event the error scanner
# keys on) is tiny, so a line this large is always a giant tool-output blob —
# never the final answer or usage — and dropping it cannot lose a real event.
...
def _agent_spool_dir() -> Optional[str]:
    ...
    if spool and os.path.isdir(spool) and os.access(spool, os.W_OK):
        return spool
    return None
...
def _iter_spooled_lines(file_obj: Any):
    """Yield decoded NDJSON lines from a spool file with a HARD per-line cap.

    Issue #1646: each line is read with a bounded ``readline(cap)``
    (``cap == _AGENT_MAX_JSONL_LINE_BYTES``) so the parent never materializes
    more than ~cap bytes of any single line. A line longer than the cap is
    drained in <=cap segments (each freed before the next) and dropped WITHOUT
    being decoded or parsed — at that size it is always a giant tool-output blob,
    never a retained event. This bounded reader lives only here (the
    spooled/production path); the in-RAM string path is unaffected.
    """
    ...
    while True:
        ...
        if line.endswith(b"\n") or len(line) < cap:
            ...
            continue
        # Oversize line: ``line`` holds only its first ``cap`` bytes. Drain the
        # remainder in bounded segments until the terminating newline (or EOF),
        # then drop the line — never decoded, never held whole.
        while True:
            segment = file_obj.readline(cap)
            if not segment or segment.endswith(b"\n") or len(segment) < cap:
                break
