...
def _build_quick_start_lines(oauth_only_setup: bool) -> List[str]:
    ...
    if oauth_only_setup:
        return [
            ...
            "",
            "2) Direct prompt commands:",
            "   With a Codex (ChatGPT) subscription login these work now WITHOUT an",
            "   API key (PDD routes them through the chatgpt/ model family; set",
            "   PDD_MODEL_DEFAULT=chatgpt/gpt-5.3-codex). Other providers call litellm",
            "   directly and need ANTHROPIC_API_KEY / OPENAI_API_KEY / GEMINI_API_KEY.",
