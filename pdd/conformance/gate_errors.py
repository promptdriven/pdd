...
class LanguageMismatchError(click.UsageError):
    """Raised when Python output fails the language-validity gate."""
...
def _verify_language_validity(
    ...
    """Return a Python syntax error, or the exact candidate that parsed.

    Uniformly indented source is accepted after dedenting. Callers must write
    the returned candidate, never the pre-validation bytes.
    """
