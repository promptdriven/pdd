...
@click.option(
    "--verbose", is_flag=True, help="Increase output verbosity for more detailed information."
)
...
@click.option(
    "--output", type=click.Path(), help="Specify where to save the generated example code."
)
@click.pass_context
@track_cost
def example(
    ctx, prompt_file: str, code_file: str, output: Optional[str]
) -> Tuple[str, float, str]:
...
@click.option(
    "--merge", is_flag=True, default=False, help="Merge new tests into existing tests."
)
@click.pass_context
@track_cost
def test(
...
@click.argument("prompt_file", type=click.Path(exists=True))
@click.option(
    "--output", type=click.Path(), help="Specify where to save the preprocessed prompt file."
)
@click.option(
    "--xml", is_flag=True, help="Automatically insert XML delimiters for long and complex prompt files."
)
@click.option(
    "--recursive", is_flag=True, help="Recursively preprocess all prompt files in the prompt file."
)
...
@track_cost
def preprocess(
    ctx, prompt_file: str, output: Optional[str], xml: bool, recursive: bool, double: bool, exclude: List[str]
) -> Tuple[str, float, str]:
...
@click.option(
    "--output-test", type=click.Path(), help="Specify where to save the fixed unit test file."
)
@click.option("--output-code", type=click.Path(), help="Specify where to save the fixed code file.")
@click.option(
    "--output-results",
    type=click.Path(),
    help="Specify where to save the results from the error fixing process.",
)
...
@click.option(
    "--max-attempts", type=int, default=3, help="Set the maximum number of fix attempts before giving up."
)
@click.option(
    "--budget", type=float, default=5.0, help="Set the maximum cost allowed for the fixing process."
)
@click.option(
    "--auto-submit",
    is_flag=True,
...
@click.option(
    "--output-sub", type=click.Path(), help="Specify where to save the generated sub-prompt file."
)
@click.option(
    "--output-modified", type=click.Path(), help="Specify where to save the modified prompt file."
)
@click.pass_context
@track_cost
def split(
...
@click.option(
    "--csv", is_flag=True, help="Use a CSV file for the change prompts instead of a text file."
)
@click.pass_context
@track_cost
def change(
...
@click.option(
    "--output", type=click.Path(), help="Specify where to save the CSV file containing the analysis results."
)
@click.pass_context
@track_cost
def detect(
    ctx, prompt_files: List[str], change_file: str, output: Optional[str]
...
@click.option(
    "--output", type=click.Path(), help="Specify where to save the CSV file containing the conflict analysis results."
)
@click.pass_context
@track_cost
def conflicts(
    ctx, prompt1: str, prompt2: str, output: Optional[str]
) -> Tuple[List[dict], float, str]:
...
@click.option(
    "--output-program", type=click.Path(), help="Specify where to save the fixed program file."
)
@click.option("--loop", is_flag=True, help="Enable iterative fixing process.")
@click.option(
    "--max-attempts", type=int, default=3, help="Set the maximum number of fix attempts before giving up."
...
def install_completion():
    ...
    completion_script_path = os.path.join(PDD_PATH, "pdd_completion." + shell)
    if not os.path.exists(completion_script_path):
        rprint(f"[bold red]Completion script not found: {completion_script_path}[/bold red]")
        sys.exit(1)
    ...
    try:
        ...
        if source_command not in content:
            ...
            rprint(
                f"Please restart your shell or run 'source {rc_file}' to enable completion."
            )
        else:
            rprint(
                f"[bold yellow]Shell completion already installed for {shell}.[/bold yellow]"
            )
...
@click.option(
    "--output", type=click.Path(), help="Specify where to save the trace analysis results."
)
@click.pass_context
@track_cost
def trace(
    ctx, prompt_file: str, code_file: str, code_line: int, output: Optional[str]
) -> Tuple[str, float, str]:
...
@click.option(
    ...
    metavar="LOCATION",
    type=click.Path(),
    help="Specify where to save the bug-related unit test.",
)
...
def bug(
    ...
    return bug_main(
        ctx, prompt_file, code_file, program_file, current_output, desired_output, output, language
    )
...
@click.option(
    "--force-scan", is_flag=True, help="Force rescanning of all potential dependency files."
)
@click.pass_context
@track_cost
def auto_deps(
