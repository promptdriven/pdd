#!/usr/bin/env python3
"""
Generate segment 08 Part A (developer edit complete) for section 02.

This is a single developer shot (not split screen), using the last frame
from segment 07 as reference for character consistency.

Usage:
    python generate_segment_08.py --generate  # Generate video
    python generate_segment_08.py --extract-ref  # Extract reference frame from seg 07
    python generate_segment_08.py --all  # Do everything
"""
...
def extract_reference_frame():
    """Extract reference frame from segment 07 developer video."""
    print("\n=== Extracting Reference Frame ===")
    FRAMES_DIR.mkdir(exist_ok=True)

    seg07_video = OUTPUTS_DIR / "07_left_developer.mp4"
    ref_frame = FRAMES_DIR / "07_developer_last.png"

    return extract_last_frame(seg07_video, ref_frame)


def generate_segment(client):
    """Generate segment 08 Part A video."""
    print("\n=== Generating Segment 08 Part A ===")
    OUTPUTS_DIR.mkdir(exist_ok=True)

    # Reference frame from segment 07
    ref_frame_path = FRAMES_DIR / "07_developer_last.png"
    ...
    # Load prompt
    prompt_path = PROMPTS_DIR / "08_developer_edit_complete.md"
    ...
    # Generate video (16:9 horizontal for this segment)
    output_path = OUTPUTS_DIR / "08_developer_edit_complete.mp4"
    return generate_video(client, prompt, output_path, reference_image=ref_image, aspect_ratio="16:9")


def main():
    parser = argparse.ArgumentParser(
        description="Generate segment 08 Part A (developer edit complete)",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
    # Extract reference frame from segment 07:
    python generate_segment_08.py --extract-ref

    # Generate video (requires reference frame):
    python generate_segment_08.py --generate

    # Do everything:
    python generate_segment_08.py --all
"""
    )
    parser.add_argument("--extract-ref", action="store_true",
                        help="Extract reference frame from segment 07")
