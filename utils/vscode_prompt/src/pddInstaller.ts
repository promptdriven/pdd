...
export class PddInstaller {
    ...
    async installPddCli(): Promise<void> {
        try {
            ...
            const choice = await vscode.window.showInformationMessage(
                'How would you like to install PDD CLI?',
                {
                    modal: false,
            ...
            const installationMethod = await vscode.window.withProgress({
                ...
                let installationMethod: 'uv' | 'uv-installed' | 'fallback' = 'fallback';
            ...
            if (installationMethod === 'uv') {
                vscode.window.showInformationMessage(
                    'PDD CLI has been installed successfully using uv (the recommended package manager)! Run "pdd setup" in your terminal or use the "PDD: Run PDD Setup" command to configure API keys and install tab completion.',
                    'Open Terminal',
                    'Run Setup',
                    'View PDD Docs',
                    'View uv Docs'
                ...
                vscode.window.showInformationMessage(
                    'PDD CLI has been installed successfully! We also installed uv (the recommended Python package manager) for future use. Run "pdd setup" in your terminal or use the "PDD: Run PDD Setup" command to configure API keys and install tab completion.',
                    'Open Terminal',
                    'Run Setup',
                    'View PDD Docs',
                    'View uv Docs'
                ...
                vscode.window.showInformationMessage(
                    'PDD CLI has been installed using pip. Note: This may be an older version. For the latest features, consider installing uv and reinstalling with "uv tool install pdd-cli". Run "pdd setup" in your terminal or use the "PDD: Run PDD Setup" command to configure API keys.',
                    'Open Terminal',
                    'Run Setup',
                    'View PDD Docs',
                    'Install uv'
            ...
            vscode.window.showErrorMessage(
                `Failed to install PDD CLI: ${error instanceof Error ? error.message : 'Unknown error'}. Please install manually via: uv tool install pdd-cli (recommended) or pip install pdd-cli`
            );
        }
    }
    ...
    async runPddSetup(): Promise<void> {
        try {
            ...
            try {
                ...
                vscode.window.showInformationMessage(
                    'PDD setup is running in the terminal with conda environment activated. Follow the prompts to configure your API keys, then reload your shell.',
                    'OK'
                );
            } catch {
            ...
            vscode.window.showErrorMessage(
                `Failed to run PDD setup: ${error instanceof Error ? error.message : 'Unknown error'}`
            );
        }
    }

    private async getFallbackInstallCommand(): Promise<string> {
    ...
    async upgradeToUvInstallation(): Promise<void> {
        try {
            ...
            vscode.window.showErrorMessage(
                `Failed to upgrade PDD CLI to uv: ${error instanceof Error ? error.message : 'Unknown error'}`
            );
        }
    }

    private async detectInstallationMethod(): Promise<{type: 'uv' | 'pip' | 'conda-env', pythonCmd?: string}> {
    ...
    async checkAndPromptInstallation(): Promise<void> {
        try {
            ...
            const shouldPrompt = config.get<boolean>('promptForInstallation', true);
