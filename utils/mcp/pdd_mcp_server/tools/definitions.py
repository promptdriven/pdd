...
PDD_SPLIT = types.Tool(
    name="pdd-split",
    description=f"""Diagnose and split a large PDD dev unit (prompt + code + example + tests) into smaller, independent dev units.
{LLM_PARAMETER_GUIDANCE}

WHEN TO USE: Choose this tool when a source file has grown too large or has too many responsibilities and you want to decompose it into smaller, independently maintainable PDD dev units.

HOW IT WORKS (14-step agentic pipeline):
1. Classifies intent (reduce monolith, enable parallel work, extract reusable layer, reduce test time)
2. Surveys the codebase neighborhood and diagnoses the architectural problem
3. Proposes 2-4 split options with responsibility-based scoring
4. Extracts full dev units (prompt + code + example + tests) for each child
5. Runs deterministic verification gates (tests, lint, test-seam resolution, parent-wiring)
6. Proves new prompts regenerate via pdd sync

Examples:
- ✅ CORRECT (full pipeline): {{"target_file": "pdd/large_module.py"}}
- ✅ CORRECT (diagnose only): {{"target_file": "pdd/large_module.py", "diagnose": true}}
- ✅ CORRECT (propose options, no extraction): {{"target_file": "pdd/large_module.py", "propose_only": true}}
- ✅ CORRECT (explicit intent): {{"target_file": "pdd/large_module.py", "intent": "reduce"}}
- ❌ INCORRECT: Do NOT pass prompt/code/example files — use target_file (the source code file to split)

LEGACY MODE: If you need the old 2-LLM-call split (prompt + code + example → sub-prompt), pass legacy=true with the old three positional args.""",
    inputSchema={
        "type": "object",
        "properties": {
            "target_file": {
                "type": "string",
                "description": "REQUIRED: Path to the source file to diagnose and potentially split (e.g. pdd/large_module.py)"
            },
            "diagnose": {
                "type": "boolean",
                "description": "Run steps 0-2 only and return the diagnosis report without extracting"
            },
            "propose_only": {
                "type": "boolean",
                "description": "Run steps 0-4 only and show all split options with scores (cheap preview, no extraction)"
            },
            "intent": {
                "type": "string",
                "enum": ["reduce", "parallel", "reuse", "tests"],
                "description": "Goal of this split: reduce=REDUCE_MONOLITH, parallel=ENABLE_PARALLEL_WORK, reuse=EXTRACT_REUSABLE_LAYER, tests=REDUCE_TEST_TIME"
            },
            "force_split": {
                "type": "boolean",
                "description": "Override LEAVE_ALONE diagnosis and force extraction"
            },
            "no_verify": {
                "type": "boolean",
                "description": "Skip step 7a test gate (dev only)"
            },
            "skip_regen_gate": {
                "type": "boolean",
                "description": "Skip step 7b regen gate (dev only, logged loudly)"
            },
            "no_phase_extraction": {
                "type": "boolean",
                "description": "Skip step 6a phase extraction (only move whole symbols, no internal refactoring)"
            },
            "strangler": {
                "type": "boolean",
                "description": "Strangler mode: produce N sequential PRs, one child per PR"
            },
            "delete_dead": {
                "type": "boolean",
                "description": "Opt-in dead symbol deletion (default: surface candidates for human review)"
            },
            "experimental_language": {
                "type": "boolean",
                "description": "Enable non-Python language support (Python is the only supported tier in this release)"
            },
            "timeout_adder": {
                "type": "number",
                "description": "Additional seconds added to each step's timeout (agentic mode only)"
            },
            "no_github_state": {
                "type": "boolean",
                "description": "Disable GitHub state persistence (agentic mode only)"
            },
            "legacy": {
                "type": "boolean",
                "description": "Use the old 2-LLM-call split path. When true, 'input_prompt', 'input_code', and 'example_code' are required instead of 'target_file'."
            },
            "input_prompt": {
                "type": "string",
                "description": "LEGACY ONLY (set legacy=true): path to the input prompt file."
            },
            "input_code": {
                "type": "string",
                "description": "LEGACY ONLY (set legacy=true): path to the input code file."
            },
            "example_code": {
                "type": "string",
                "description": "LEGACY ONLY (set legacy=true): path to the example code file."
            },
            "output_sub": {
                "type": "string",
                "description": "LEGACY ONLY: output path for the sub-prompt file."
            },
            "output_modified": {
                "type": "string",
                "description": "LEGACY ONLY: output path for the modified prompt file."
            }
        },
        # target_file is required for the agentic default; the handler validates
        # that input_prompt/input_code/example_code are all supplied when legacy=true.
    }
)
