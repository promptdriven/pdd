...
# Dynamically add project root to sys.path (assuming this script is in examples/)
...
def mock_run_agentic_task(
    ...
    **_: object,
    ...
    if "step11" in label:
        return True, "Cleanup applied.", 0.05, "gpt-4.1"
    return False, f"Unexpected label: {label}", 0.0, "gpt-4.1"


def main() -> None:
    """Run a mocked demonstration of the E2E fix orchestrator."""
    with patch("pdd.agentic_e2e_fix_orchestrator.load_prompt_template", side_effect=mock_load_prompt_template), \
         ...
         patch("pdd.agentic_e2e_fix_orchestrator._revert_out_of_scope_changes"), \
