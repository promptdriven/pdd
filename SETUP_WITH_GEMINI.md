...
The setup wizard runs these steps:
  1.  Detects agentic CLI tools (Claude, Antigravity `agy` preferred / legacy Gemini `gemini` fallback, Codex, OpenCode) and offers installation and credential configuration if needed. The Google provider supports both the Antigravity CLI (installed via `curl -fsSL https://antigravity.google/cli/install.sh | sh`) and the legacy Gemini CLI (`npm install -g @google/gemini-cli`); set `PDD_AGENTIC_PROVIDER=antigravity` or `PDD_GOOGLE_CLI=agy|gemini|auto` to control selection. Google announced consumer-tier Gemini CLI cutoff on **2026-06-18**; the legacy `gemini` binary remains the safe rollback (`PDD_GOOGLE_CLI=gemini`).
