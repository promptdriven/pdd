...
- `pdd story verify` runs the validation suite. It classifies every criterion in a story's contract against the linked prompts and reports each one individually:
...
- `pdd detect --stories` is the older spelling of the same check and continues to work unchanged. Pass `--legacy-detect` to either command to fall back to the open-ended change detector that story validation used previously.
