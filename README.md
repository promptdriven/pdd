...
**Available subscription models** (selectable via `PDD_MODEL_DEFAULT` or `pdd setup`): `chatgpt/gpt-5.5` (the preferred default), `chatgpt/gpt-5.4`, `chatgpt/gpt-5.3-codex`, `chatgpt/gpt-5.2`, `chatgpt/gpt-5.3-codex-spark`. Because `chatgpt/*` rows are interactive-only, non-interactive (`PDD_FORCE`) runs pin the chosen default (e.g. `chatgpt/gpt-5.5`); set `PDD_ALLOW_INTERACTIVE=1` to let `--strength` rank across the family. (Exact models depend on what your ChatGPT plan serves.)

**Anthropic API keys are opt-in for direct `llm_invoke`.** The direct/local default route prefers the Codex subscription when available, so that path does not require `ANTHROPIC_API_KEY` unless you explicitly opt in by setting `PDD_MODEL_DEFAULT` to a `claude-*` model and exporting `ANTHROPIC_API_KEY`. When you do, normal model selection across the full catalog is restored.
