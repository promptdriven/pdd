...
- **`CLAUDE_MODEL`**: Override the model used by Claude Code in agentic workflows (e.g., `claude-sonnet-4-5-20250929`). Claude Code always receives `--model`: when unset, PDD requests its Opus 5 compatibility default and passes Claude Code the verified `claude-fable-5` provider ID. `CLAUDE_MODEL=claude-opus-5` is canonicalized to that same Fable 5 ID; other explicit values pass through unchanged. The `.pdd/agentic-logs/` audit trail records the effective command-boundary model when response JSON does not carry an explicit model name (Issue #1376).
