...
For the deterministic architecture completeness gate — enforcing a prompt ↔ artifact ↔ `architecture.json` bijection, failure categories, waiver schema, and shadow/required modes — see [docs/architecture_completeness.md](docs/architecture_completeness.md).
...
- **`PDD_ARCH_COMPLETENESS_MODE`**: Controls the architecture completeness gate (`tests/test_architecture_completeness.py`). `shadow` (default) collects and reports all bijection gaps but exits 0 so existing CI is not broken; `required` exits 1 on any unwaived gap. Start in `shadow` mode until all 27–29 identified gaps are resolved or waived, then switch to `required` in CI. See [docs/architecture_completeness.md](docs/architecture_completeness.md).
- **`PDD_BASE_REF`**: Git ref for the base branch used by the completeness gate's base/head union (e.g. `origin/main`). When set, the gate unions the waiver file from both refs so that a waiver deleted in the PR branch cannot silently un-waive modules — the base ref's waivers remain active. Note: inventory-level deletion-evasion (detecting prompts and architecture entries deleted from the PR branch) is not yet implemented; the base/head union scope is limited to the waiver file. Leave unset for local runs; these variables are not set automatically — add them to the CI job's `env:` block (e.g. `PDD_BASE_REF: origin/main`) when deletion-evasion enforcement is ready.
- **`PDD_HEAD_REF`**: Git ref for the head branch used by the completeness gate's base/head union (e.g. `HEAD` or the PR branch SHA). Works with `PDD_BASE_REF` to form the union of the waiver file (not the full inventory). Leave unset for local runs.
