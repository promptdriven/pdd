...
- attempted_models: Semicolon-delimited chronological list of every model PDD attempted for the command, across all LLM calls the command made (e.g. `generate` runs code-generation followed by postprocess code extraction — both contribute). The list ends with the model that produced the command's final output (the `model` column). When PDD's default model fails and the run falls back to another provider (for example Vertex AI → DeepSeek), each attempted model appears here so users can see the full fallback history rather than only the final successful model. Note: for commands with multiple LLM substeps, intermediate substep successes also appear in this list — entries before the last are not all "abandoned" attempts. For a single-attempt command this column contains just the successful model. Semicolons inside model names are sanitized to preserve the delimiter.
