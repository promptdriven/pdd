...
12. **Final PR Gate**: After CI passes, run the canonical two-layer final gate (issue #1406) against the PR head — `pdd checkup --pr <pr-url> --issue <issue-url> --final-gate` (Layer 1 = PR-scoped checkup, Layer 2 = maintainer-style review-loop), returning a real ship verdict. If either layer pushes additional fixes, CI is re-validated on the new head (the head is accepted as the gate's own when it matches either the Layer-1 checkup worktree or the Layer-2 review-loop verified head) before the workflow returns success
...
- `compressed_context`: Enable or disable compressed sync context for generation and repair phases (default: unset, inheriting from the `--compressed-context` / `--no-compressed-context` CLI flag). When `true`, sync builds bounded phase packages from the prompt, existing tests, examples, contract sections, and recent repair evidence, then passes those packages to generate, verify, test, and fix attempts. This is the `.pddrc` context-level equivalent of the `--compressed-context` CLI flag.
- `context_compression`: Compression mode for sync context. Set to `"off"` to disable all compression for debugging, overriding both `compressed_context` and the sub-settings below. Other values (e.g. `"on"`) enable compression. Maps to the `PDD_CONTEXT_COMPRESSION` environment variable.
