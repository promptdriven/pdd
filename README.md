...
- **Health check**: `pdd checkup <url>` identifies what needs attention next; `pdd checkup --pr ...` reviews an existing PR (add `--issue` to verify it resolves a specific issue)
...
`checkup` can also run against an existing pull request, with or without a source issue. Default PR mode runs the standard checkup steps on the PR branch, can commit and push generated fixes back to that same PR, and skips PR creation because the PR already exists. When `--issue` is omitted, the PR is reviewed on its own merits (correctness/quality) and the issue-alignment gate is skipped; pass `--issue` to additionally verify the PR resolves that issue. Use `--no-fix` for verification-only PR checks, or `--review-loop` for the separate reviewer/fixer loop.
...
- `--no-github-state`: Disable GitHub state persistence, use local-only
- `--pr PR_URL`: Review an existing pull request instead of creating a new one. Cannot be combined with a positional issue URL. `--issue` is optional in default PR mode: without it, the PR is reviewed on its own merits; with it, checkup also verifies the PR resolves that issue. (`--review-loop` still requires `--issue`.)
- `--issue ISSUE_URL`: Optional source GitHub issue for `--pr`; when provided, it is used as the expected behavior and acceptance criteria for PR verification. Omit it to review the PR on its own merits. Only valid together with `--pr`. Required when `--review-loop` is used.
- `--review-loop`: In PR mode, run the primary-reviewer/fixer loop. Requires `--issue` (the merit-review, PR-without-issue path is not yet supported under `--review-loop`). The primary reviewer reviews the PR, the fixer addresses actionable findings, fixes are committed and pushed to the PR branch, and the primary reviewer verifies until clean or a limit is reached.
...
In **issue mode**, each step posts its findings as a comment on the GitHub issue, providing a detailed audit trail. In **PR mode** (see below), step-level posting is suppressed — the orchestrator posts a single canonical final report on the PR thread once it knows the outcome (gate pass after push, gate fail, push failure, refusal, or `--no-fix` pass/fail); when an `--issue` is also provided, the same report is additionally posted to that issue thread, but with no `--issue` the issue-thread post is skipped entirely. If GitHub commenting fails, the report body is also saved to `.pdd/checkup-pr-<n>/final-report.md` so it is never lost; the failure is surfaced via the run's returned message and persisted as `step_outputs["pr_post_status"]`, but it does not flip the gate outcome.

**PR Mode**: Use `--pr` to run checkup against an existing PR. `--issue` is optional: with it, checkup verifies the PR resolves that issue (the issue-alignment gate applies); without it, the PR is reviewed on its own merits (correctness/quality) and the alignment gate is skipped. By default this runs the full standard checkup flow on the PR worktree, commits eligible generated fixes, pushes them back to the same PR branch, and skips step 8 because the PR already exists. Add `--no-fix` when you want verification-only behavior. PR mode reloads `architecture.json` and `.pddrc` from the PR worktree before running, so audits see the PR's project state and not the parent checkout's.
