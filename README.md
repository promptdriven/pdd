...
**Metadata finalization (on success):** When `auto-deps` is invoked in true in-place mode (i.e. `--output` resolves to the same file as `PROMPT_FILE`), the command writes/updates the fingerprint for the mutated prompt in `.pdd/meta/` (with `operation="auto-deps"` and the current include-dependency hashes) and clears any stale per-module `_run.json` report, so downstream commands see a consistent view. The fingerprint write and the include-dependency metadata update are committed together (atomic from the caller's perspective) and use hash-based, language-agnostic helpers from `pdd.operation_log`. Finalization errors are surfaced as warnings and do not mask a successful `auto-deps` result.

In the default CLI flow the output goes to a separate `<basename>_with_deps.prompt` file and the canonical prompt is untouched, so finalization is intentionally **skipped** to avoid recording the hash of a non-canonical file under the canonical module identity (a `Note:` is printed when not running quietly). Re-run `auto-deps` with `--output <PROMPT_FILE>` to refresh the fingerprint and clear the stale `_run.json`.

**Note on `pdd sync`:** `pdd sync` invokes `auto-deps` with a `*_with_deps.prompt` temp output and then moves it to the canonical prompt, so this finalizer (intentionally) skips in that flow. Sync writes its own fingerprint via its atomic state machinery and handles `_run.json` lifecycle separately. Tracking the `_run.json` clear for sync's auto-deps path is a sync-side concern outside this change's scope.
