...
2. **generate**: Create or update the code module from the prompt. After generation an **architecture conformance gate** validates the output against both `architecture.json` and the prompt's `<pdd-interface>` block:
    ...
    - **Test-churn gate**: if `pdd sync` is about to overwrite an existing test file through either the code-generation writer or the `cmd_test_main` test-generation path, and the unified-diff churn ratio between the pre-sync and proposed test file exceeds `PDD_TEST_CHURN_THRESHOLD` (default `0.40`, i.e. 40%), the gate fails fast with `TestChurnError` so a small prompt change cannot land a thousand-line test rewrite that drops broad existing coverage. The threshold is module-scoped (skips first-time test generation) and can be tuned per-project via the env var.
