...
- `--final-gate`: The canonical final PR gate (issue #1406). Requires `--pr`; `--issue` is optional (issue #1441) — with no issue the PR is reviewed on its own merits and the issue-alignment check is skipped, with `--issue` the PR is also verified for issue alignment. Runs the PR-scoped checkup as Layer 1 (no new PR) and the review-loop as Layer 2, then returns a real ship verdict (exit non-zero unless the PR is genuinely shippable). This is what "ready for maintainer review" means once a PR exists, and the gate `pdd-issue` uses for its final verification. Cannot be combined with `--review-loop`, `--no-fix`, `--review-only`, `--start-step`, `--no-gates`, or `--test-scope targeted`.
