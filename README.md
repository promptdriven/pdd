...
```
pdd [GLOBAL OPTIONS] fix [OPTIONS] PROMPT_FILE CODE_FILE UNIT_TEST_FILES... [ERROR_FILE]
```
...
- `UNIT_TEST_FILES`: One or more unit test file(s). Multiple files can be provided, and each will be processed individually.
- `ERROR_FILE`: (Optional) The filename containing the unit test runtime error messages.
  - **Required in non-loop mode**: Provides static error messages for a single fix attempt.
  - **Not used in loop mode**: Errors are automatically generated by running the verification program.
...
- `--loop`: Enable iterative fixing process.
  - `--verification-program PATH`: **(Required for `--loop`)** Specify the path to a Python program that verifies if the code still runs correctly. The program should exit with code 0 if tests pass, non-zero if they fail.
...
**Example 1: Non-loop mode (single fix attempt with static error file)**
```bash
pdd [GLOBAL OPTIONS] fix --output-code src/factorial_fixed.py --output-results results/fix_results.log \
  factorial_python.prompt src/factorial.py tests/test_factorial.py tests/test_edge_cases.py errors.log
```
In this example, the last file `errors.log` is the ERROR_FILE containing static error messages. Each test file will be processed separately.

**Example 2: Loop mode (iterative fixing with verification program)**
```bash
pdd [GLOBAL OPTIONS] fix --loop --verification-program verify_factorial.py \
  --max-attempts 5 --budget 2.5 --output-code src/factorial_fixed.py \
  factorial_python.prompt src/factorial.py tests/test_factorial.py tests/test_edge_cases.py
```
In this example, no ERROR_FILE is provided. The verification program `verify_factorial.py` runs after each fix attempt to generate fresh error messages. The loop continues until tests pass, max attempts reached, or budget exhausted.
