...
- **`PDD_CHANGED_MODULES`**: Comma-separated module aliases (e.g. `agentic_sync,sync_main`) that provide authoritative ordered selection for `pdd sync <issue-url>`. When non-empty, this input is evaluated before branch-diff heuristics and never invokes the identification LLM. `_parse_changed_modules_env` splits on commas, trims whitespace, removes empty entries, and deduplicates in first-occurrence order while preserving path-qualified aliases such as `commands/maintenance`. Every alias must resolve against the already-frozen architecture-backed candidate inventory; an unknown or runtime-only entry, ambiguous bare leaf, or unowned path prefix rejects the entire request before dry-run or writes. Dependencies are added from the same frozen graph, and caller order is retained only where dependency-safe. When unset or empty, selection falls through to branch-diff evidence, a proven runtime-template-only no-op, exact issue mentions, and finally a bounded ambiguity-only request whose selection must be non-empty. Intended for cloud runners and CI pipelines where the changed modules are already known. No default. (Cloud-only; no effect on local direct-basename sync.)
