...
2. **generate**: Create or update the code module from the prompt. After generation an **architecture conformance gate** validates the output against both `architecture.json` and the prompt's `<pdd-interface>` block:
    ...
    - **Public-surface regression gate**: when a module already has a generated code file in the working tree (i.e., not a first-time generation), the gate ALSO snapshots the pre-generation public surface (top-level functions, classes, and `class.method` symbols for Python) and rejects a generation that removes or renames any public symbol unless the prompt diff carries an explicit `BREAKING-CHANGE:` marker. This stops a narrow prompt edit from silently dropping helpers that existing callers/tests import (issue #1012). First-time generation (no prior code file) is exempt.
    - **Test-churn gate**: if `pdd sync` is about to overwrite an existing test file through the code-generation path (the `code_generator_main` writer) and the unified-diff churn ratio between the pre-sync and proposed test file exceeds `PDD_TEST_CHURN_THRESHOLD` (default `0.40`, i.e. 40%), the gate fails fast with `TestChurnError` so a small prompt change cannot land a thousand-line test rewrite that drops broad existing coverage. Scope note: the gate covers `pdd generate`-style writes to test artifacts; the `pdd sync test` operation that flows through `cmd_test_main` (which has its own `--merge` policy) is not covered by this gate in #1012 and is tracked separately. The threshold is module-scoped (skips first-time test generation) and can be tuned per-project via the env var.
    - On failure, sync retries the generation step up to `MAX_CONFORMANCE_ATTEMPTS` with a `PDD_REPAIR_DIRECTIVE` that names the function to fix and the parameters/annotations/defaults to add or restore. Public-surface and test-churn failures use the same repair loop. `.pddrc` context/strength are pinned across the entire retry sequence so a retry never silently switches model or context. The retry stops early when the missing-symbol set repeats across attempts, and the final failure is surfaced as a structured `=== architecture conformance failure ===`, `=== public surface regression ===`, or `=== test churn threshold exceeded ===` block listing the offending symbols / churn ratio plus a `Reproduce locally: pdd sync <basename>` line. Authors who genuinely intend the breaking change opt in by including `BREAKING-CHANGE:` in the prompt diff body.
