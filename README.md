...
When an agentic provider or credential hits a real rate, usage, session, or credential limit, PDD emits a single secret-safe marker line on the machine-readable CLI/log surface that automation can parse without scraping raw provider stderr. Quiet mode may suppress explanatory diagnostics, but it does not suppress this scheduling marker.
...
`reset_at` is normalized to UTC when PDD can read or infer a reset time, for example from Claude Code limit text. Generic provider 429s without reset metadata still emit the marker with an empty `reset_at` and `reset_source=none`. The marker is additive: existing `credential-limit` classification text remains for backward compatibility. The marker never includes raw provider stderr, tokens, API keys, user prompt content, or other untrusted provider text.
