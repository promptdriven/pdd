...
Single-file fingerprint finalization: A successful single-file `pdd update <code>` now writes a current fingerprint to `.pdd/meta/` for the affected (prompt, code) pair, reusing the same fingerprint helper used by repo-mode update and `pdd sync`. Stale `_run.json` reports continue to be cleared when runtime verification is invalidated. If metadata is intentionally not finalized (e.g. a dry run, an explicit opt-out flag, or a precondition that failed), the command emits an explicit `metadata-not-finalized:` log line naming the reason instead of silently leaving stale state. The behavior is language-agnostic and does not depend on AST/tree-sitter.
