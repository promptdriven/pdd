...
**Long-Running Sync** (progress-aware watchdog):

For long agentic syncs (35–60 minutes is normal for multi-module issue sync), `pdd sync --agentic` uses a two-tier watchdog so a real but slow run is not killed prematurely while a genuinely stalled run is still terminated quickly. Set `PDD_PROGRESS_EVENTS_PATH=/path/to/progress.jsonl` to enable structured progress events. With events enabled:

- The soft stall watchdog (`PDD_PROGRESS_STALL_SECONDS`, default 600s) becomes the primary stuck-detection mechanism. It resets only on real-progress events (phase/step/output/cost/file progress) — liveness-only heartbeats do not reset it.
- The hard ceiling (`PDD_MODULE_TIMEOUT_SECONDS`, default 2700s) remains as an absolute backstop and is always enforced.
- The sink file is append-only JSONL with one redacted, bounded event per line (≤ 4 KiB). Concurrent parent and child `pdd sync` processes share the same sink safely. Disabled mode (the default) writes no file and changes no behavior.

Example:

```bash
export PDD_PROGRESS_EVENTS_PATH="$PWD/.pdd/progress.jsonl"
export PDD_PROGRESS_STALL_SECONDS=600        # soft stall (optional override)
export PDD_MODULE_TIMEOUT_SECONDS=2700        # hard backstop (optional override)
pdd --force sync https://github.com/myorg/myrepo/issues/1234
```

PDD Cloud (and other consumers) can later tail this JSONL to observe phase/step/cost progression, distinguish real progress from liveness, and enforce its own watchdog policy without parsing free-form CLI output.
...
- **`PDD_MODULE_TIMEOUT_SECONDS`**: Hard backstop wall-clock cap per module for `pdd sync --agentic` runs, in seconds. Default is 2700 (45 minutes). Always enforced. When `PDD_PROGRESS_EVENTS_PATH` is set, the soft progress-aware stall watchdog (`PDD_PROGRESS_STALL_SECONDS`) becomes the primary stuck-detection mechanism and this value remains as the absolute hard ceiling.
- **`PDD_PROGRESS_EVENTS_PATH`**: Optional path to a JSONL sink file. When set, long-running `pdd sync` commands append versioned, redacted, bounded structured progress events (one JSON object per line) for downstream consumers (CI dashboards, PDD Cloud watchdog, debugging). When unset, no event file is created and CLI behavior is byte-identical to a build without progress events.
- **`PDD_PROGRESS_STALL_SECONDS`**: Soft progress-aware stall window for `pdd sync --agentic`, in seconds. Default is 600 (10 minutes). Only active when `PDD_PROGRESS_EVENTS_PATH` is set. If a module emits no real-progress events (phase/step/output/cost/file progress) for this many seconds, the runner terminates that module early with a clear stall reason; liveness-only events (heartbeats, repeated messages) do not reset this timer. The hard `PDD_MODULE_TIMEOUT_SECONDS` ceiling still fires independently.
