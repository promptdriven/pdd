...
- `--adversarial-prompt TEXT`: Optional mirror instruction for `--agentic-review-loop`; it constrains reviewers to canonical `pdd checkup` criteria and must not introduce a new merge criterion.
- `--fresh-final-review ROLE`: Compatibility hook accepted only with `--agentic-review-loop`; canonical `--final-gate` still owns fresh final review semantics.
