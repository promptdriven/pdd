...
**Metadata finalization (on success):** After every successful `auto-deps` run — whether the output is written in place (as `pdd sync` does) or to the default `<basename>_with_deps.prompt` file — the command writes/updates the fingerprint for the mutated prompt in `.pdd/meta/` under the canonical module identity inferred from `PROMPT_FILE` (with `operation="auto-deps"` and the current include-dependency hashes computed from the mutated output) and clears any stale per-module `_run.json` report, so downstream sync commands see a consistent view. The fingerprint write and the include-dependency metadata update are committed together (atomic from the caller's perspective) and use hash-based, language-agnostic helpers from `pdd.operation_log`. Finalization errors are surfaced as warnings and do not mask a successful `auto-deps` result.
