...
- **`PDD_BASE_REF`**: Git ref for the base branch used by the completeness gate's base/head union (e.g. `origin/main`). When set, the gate enumerates modules present in the base ref and unions them with the head inventory so that deleting a prompt, architecture entry, and fingerprint together on the PR branch cannot make a gap disappear from the report. Leave unset for local runs; the CI workflow sets this automatically.
- **`PDD_HEAD_REF`**: Git ref for the head branch used by the completeness gate's base/head union (e.g. `HEAD` or the PR branch SHA). Works with `PDD_BASE_REF` to form the union inventory. Leave unset for local runs.
