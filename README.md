...
**PR-Scope Convergence Guard (issue #1228)**: `pdd checkup --pr` enforces a scope/causality contract on every fixer iteration so the workflow cannot silently push code unrelated to the issue's failing tests. Step 5 emits a machine-parseable `<step5_signal>` block (also captured as `.pdd/checkup-context/step5-junit.xml` when the runner is pytest) with exact failing-test nodeids, files, and tracebacks. The orchestrator computes the in-scope path set — `(PR's changed files) ∪ (failing-test files from Step 5) ∪ (test files for changed sources)` — and substitutes it into the Step 6a fixer prompt. Every fixer-touched file MUST live in that set OR appear in an explicit `EXPANSION_ITEMS:` block with a one-line justification referencing a failing nodeid or PR file; otherwise the orchestrator's `enforce_scope()` gate refuses the commit and resets the worktree. A pre-push diff sanity gate caps the added LOC at `max(10 × failing-test-surface-LOC, 200)` (override via a `BUDGET_OVERRIDE:` line in `EXPANSION_ITEMS`). Step 7's report MUST include a `causality_check` object naming `related_files`, `unrelated_files`, and `unresolved_failures`; the orchestrator independently re-runs `causality_check()` after Step 7 returns and that result is AUTHORITATIVE — a disagreement between the LLM verdict and the orchestrator check blocks the push. When Step 5 is clean (`exit_code: 0` AND `failures: []` AND no critical findings from Steps 2-4), the workflow short-circuits to zero commits/pushes (the Step 6a prompt emits `NO_CHANGES_REQUIRED`). All five helpers (`parse_step5_signal`, `compute_in_scope_paths`, `enforce_scope`, `diff_sanity_gate`, `causality_check`) live in `pdd/checkup_gates.py` next to the existing #1092 deterministic gates and share the same fail-closed-on-violation / fail-open-on-parse-error contract.
