...
10. **CI Validation**: First poll external CI, retrieve logs on failure, and run an LLM fix loop to remediate any CI-specific issues (lint, artifacts, build). Once CI is green, run a real pre-checkup gate (the fix path's first drift-sync step) that **blocks** until green. It first drift-syncs code ↔ prompt ↔ `architecture.json` ↔ `.pdd/meta`, then executes its own deterministic build/smoke checks — compile touched files, import changed modules, confirm new routes/routers mount, lint, caller-compatibility sweep, and run targeted unit tests by git-diff. It runs these checks itself rather than relying solely on GitHub required checks (which are often absent or vacuous)
...
12.5. **Pre-PR Build/Smoke Gate** (issue #1293): Before creating the PR, run a blocking build/smoke pass over the changed files — compile touched files, import changed modules, confirm new routes/routers mount, lint, caller-compatibility sweep, and targeted unit tests by git-diff — so the change/feature PR enters `checkup --pr` already building and wired. Under `PDD_STRICT_DOC_SYNC` strict mode a red gate aborts before the PR; in default mode the structured findings are surfaced in the PR body for checkup review
