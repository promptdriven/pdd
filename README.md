...
- `--agentic-review-loop`: **Fallback/mirror pass around canonical `pdd checkup --final-gate`** (not a standalone review). Requires `--pr`; `--issue` is optional; `--no-fix` is allowed. The artifact encodes the canonical × agentic verdict matrix via one of five `canonical_*` status strings: `canonical_pass_agentic_mirror_clean`, `canonical_pass_agentic_mirror_blocking` (canonical pass, agentic blocking findings found), `canonical_unknown_agentic_fallback_pass`, `canonical_unknown_agentic_fallback_blocking` (no upstream canonical report), and `canonical_fail_agentic_not_authoritative` (canonical failure — agentic result is non-authoritative). Cannot be combined with `--final-gate`.
- `--adversarial-prompt TEXT`: Reviewer instruction for `--agentic-review-loop`. When not set, the loop uses the bounded mirror default: `"Using the same criteria as canonical pdd checkup, find concrete reasons this PR should not merge. Do not introduce new merge criteria. Report only verifiable blockers or material risks."` This mirrors the same criteria as canonical `pdd checkup` (correctness, source-of-truth, tests, issue-alignment) while constraining reviewers to verifiable claims. Override with this flag to focus on a specific concern.
- `--fresh-final-review ROLE`: Role for the fresh final review in `--agentic-review-loop` mode; runs in a new context with no prior reviewer/fixer conversation state.
- `--final-gate`: The canonical final PR gate (issue #1406). Requires both `--pr` and `--issue`. Runs the PR-scoped checkup as Layer 1 (no new PR) and the review-loop as Layer 2, then returns a real ship verdict (exit non-zero unless the PR is genuinely shippable and issue-aligned). This is what "ready for maintainer review" means once a PR exists; run it explicitly as a standalone `pdd checkup` command (it is no longer invoked automatically by `pdd fix`/`pdd-issue`). Cannot be combined with `--review-loop`, `--agentic-review-loop`, `--no-fix`, `--review-only`, `--start-step`, or `--no-gates`.
