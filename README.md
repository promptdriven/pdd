...
**Finalization Signal**: To guarantee that the surrounding workflow cannot silently treat a non-finalized run as clean, every successfully mutated module emits an explicit, machine-detectable signal on stdout of the form `PDD_FINALIZATION: module=<basename> language=<lang> status=<finalized|skipped|failed> reason=<short_reason>`, and the run additionally writes an aggregate `PDD_FINALIZATION: all_finalized=true|false healed=<N> finalized=<N> skipped_finalization=<N>` line plus a `metadata_finalized=true|false` flag to `$GITHUB_OUTPUT` when running under GitHub Actions. `status=finalized` requires that the shared `run_metadata_sync` orchestrator was invoked AND its `MetadataSyncResult.ok` was True; `status=failed` covers orchestrator failure, missing-invocation paths, and any other unfinalized success and is treated as a hard heal failure for the module under R15 PR-mode semantics (no commit, exit 1). The only sanctioned non-finalized success path is the env var `PDD_SKIP_METADATA_FINALIZATION=1`, which deliberately bypasses the orchestrator call after the prompt update + example refresh; in that case the module emits `status=skipped reason=PDD_SKIP_METADATA_FINALIZATION` and the aggregate flag is forced to `all_finalized=false` so log scrapers and the workflow always see a non-success outcome even though per-module `healed` accounting is preserved for push-to-main commit gating. Scoped run-report invalidation (`.pdd/meta/<safe_basename>_<language>_run.json` cleared for the mutated module only) is part of the orchestrator's `run_report` stage and only runs when the orchestrator is invoked — it is therefore also suppressed when `PDD_SKIP_METADATA_FINALIZATION=1`, which is one of the reasons the aggregate flag reports `false` in that mode.
...
**Metadata Finalization**: The auto-heal `update` path invokes the same `run_metadata_sync` orchestrator as `pdd update --sync-metadata` and preflight drift-heal, so prompt tags, architecture entries, run reports, and fingerprint state are always reconciled together. **Any stage reporting `failed`** blocks the auto-heal checkpoint commit so a PR cannot land a half-synced state; `skipped` is permitted for legitimate cases (no `architecture.json`, unregistered modules, LLM-first tag refresh pending #870) and does not block — matching the contract documented in the CI Drift Detection section above. Every mutated module emits an explicit `PDD_FINALIZATION:` stdout marker and the run writes `metadata_finalized=true|false` to `$GITHUB_OUTPUT`, with the env var `PDD_SKIP_METADATA_FINALIZATION=1` as the only sanctioned non-finalized success path (forces `all_finalized=false` so the surrounding workflow cannot silently treat it as clean).
