...
- `--final-gate`: The canonical final PR gate (issue #1406). Requires `--pr`; `--issue` is optional. Runs the PR-scoped checkup as Layer 1 (no new PR) and the review-loop as Layer 2, then returns a real ship verdict (exit non-zero unless the PR is genuinely shippable). This is what "ready for maintainer review" means once a PR exists, and the gate `pdd-issue` uses for its final verification. Cannot be combined with `--review-loop`, `--no-fix`, `--review-only`, `--start-step`, or `--no-gates`.
