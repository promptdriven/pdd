...
2. **One Agentic CLI** - Required to run the workflows (install at least one):
   ...
   - **Antigravity CLI (`agy`, preferred)**: install via `curl -fsSL https://antigravity.google/install.sh | sh` (uses `~/.gemini/antigravity-cli/` OAuth login if present, otherwise `GEMINI_API_KEY`/`GOOGLE_API_KEY`/`ANTIGRAVITY_API_KEY`). Set `PDD_AGENTIC_PROVIDER=antigravity` to pin Antigravity, or `PDD_GOOGLE_CLI=agy|gemini|auto` to control binary selection (`auto` prefers `agy` when both are installed).
