...
- **Python** seeds the report, invokes the Pi bridge (`_pi_repair_bridge.mjs`), and reads the structured `approved_patches` output. Python applies patches and gates `--apply` in all cases.
...
| Run interactive QA, propose repairs, render menus | Pi (via `_pi_repair_bridge.mjs`) |
...
`pdd/_pi_repair_bridge.mjs` creates a Pi session with an explicit tool allowlist and a `propose_repair_options` custom tool. Pi drives the session; Python serialises input and reads output:

```js
// pdd/_pi_repair_bridge.mjs
...
const proposeRepairOptions = defineTool({
  ...
  execute: async (_toolCallId, { finding_id, patches }) => {
    ...
    return { content: [{ type: "text", text: `Recorded ${patches.length} patch(es) for ${finding_id}.` }], details: {} };
  },
});
...
class PiInteractiveSession:
    ...
    def run(self):
        # writes context.json, calls node _pi_repair_bridge.mjs, reads output.json
        ...
...
`_pi_available()` returns `False` when Node is absent, its major version is below 22, or `@earendil-works/pi-coding-agent` is not installed (verified via a dynamic import probe). When `False`, callers should skip the Pi path or error with a clear message rather than attempting a Python TTY fallback (no fallback workflow is developed under this decision).
...
> - Python seeds the report, invokes `_pi_repair_bridge.mjs`, and applies the approved patches returned as structured JSON.
