...
- **One Agentic CLI** - Install at least one:
  ...
  - **Antigravity CLI (`agy`, preferred)**: `curl -fsSL https://antigravity.google/cli/install.sh | sh` (`~/.gemini/antigravity-cli/` OAuth or `GEMINI_API_KEY`/`GOOGLE_API_KEY`/`ANTIGRAVITY_API_KEY`; pin with `PDD_AGENTIC_PROVIDER=antigravity` or `PDD_GOOGLE_CLI=agy`)
