...
| `<capabilities>` | What the module may do (deterministically enforced by `pdd checkup policy check`) |
...
While `pdd checkup contract check` validates the *structure* of your contract sections, the side-effect rules in `<capabilities>` are enforced by `pdd checkup policy check`. This command performs AST-based analysis of the generated code to ensure it respects the specified modals (`MAY` vs `MUST NOT`).
