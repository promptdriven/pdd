...
| 3 | `pdd context` token visualization — color usage-box/table by token category (`status`) from the central palette; `--color/--no-color` auto-detect (NO_COLOR / non-TTY); JSON unchanged. See `docs/design/context-token-colors.md` | sohni-tagirisa/pdd#3 | 🟡 In progress |
