...
8.  **Replayable Context**: When prompts rely on dynamic inputs such as shell output, web content, or semantic include extraction, durable snapshots of the expanded prompt context make later audits and policy checks possible. Replay verifies the prompt/context that shaped a run, while still acknowledging that the LLM call itself can remain nondeterministic.
