...
**Companion to:** [`design.md`](./design.md) — *Agentic Localization Degradation Under Repository Bloat* (issue [#1209](https://github.com/promptdriven/pdd/issues/1209))
...
2. **The instrumentation must match the agent's retrieval substrate.** The locked filesystem tap (Linux container + OverlayFS + FUSE byte-extent reads, [`design.md` §6.2](./design.md#62-how-we-capture-it-defense-in-depth)) captures reads that cross the kernel — appropriate for local file reads and shell-spawned commands, once the pinned agent's execution path is calibrated. It silently under-captures an agent whose "search" is an **embedding query answered server-side** (see §3, §6). This is the open item already flagged in [`design.md` §10, "Still to confirm" #2](./design.md#10-locked-decisions).
...
### Family A — Agentic / tool-based search (on-demand local retrieval)

The model autonomously issues filesystem and text-search operations (`glob`/`grep`/`read`, or a shell that can run tools such as `ripgrep`), reads results, and decides what to search next, with reasoning between steps [5, 6, 16]. Anthropic's Claude Code is the canonical documented example: it exposes `Grep`, `Glob`, and `Read` and explores on demand rather than indexing the repository [5, 6]; its content search is backed by `ripgrep` [7, 15]. Reportedly, an Anthropic engineer stated that in their testing "agentic search outperformed [RAG] by a lot, and this was surprising" [7]. SWE-agent's ACI is the research instantiation: "tailored search and navigation commands that efficiently locate relevant files and content" [12].

OpenAI's Codex CLI belongs here **provisionally for this benchmark's pinned local-CLI arm**, because current OpenAI docs establish local file reads/edits, model-generated shell commands under sandboxing, web search modes, MCP tools, and `apply_patch` hook support [1, 2, 17]. Those docs do **not** establish the stronger claims that Codex has no precomputed local/server index or that its exact read/search path is `rg`/`find`/`cat`. This document therefore treats Codex's exact default retrieval mechanics as an implementation detail to calibrate before runs, not as a primary-source fact.

*Cost signature under bloat:* when retrieval is local on-demand search/read, cost is **paid at run time and scales with the amount of plausible-but-irrelevant code** — more candidate matches per search, more files worth opening, more tokens pulled into the window. Failure modes are *window dilution* and *wrong-file reads*. This family is the one the FUSE/OverlayFS tap captures natively once calibration confirms that reads cross the kernel.
...
Most mature tools now combine A and B and let the model choose. Cursor explicitly runs **both** semantic search and agentic `grep`, with the "Agent pick[ing] the right tool based on [the] prompt" — grep for specific symbols, embeddings for behavioral/conceptual queries, chained searches for complex exploration [10]. Agentless couples embedding- and prompt-based retrieval with LLM re-ranking in a **hierarchical** file → class/function → edit-location localization [13]. Claude Code is predominantly Family A, while Codex is treated as Family A only for a pinned local-CLI configuration whose local read/search path is calibrated; both can be extended toward B-like behavior via **MCP** servers (e.g., a semantic-search tool) [1, 5, 17].

### Comparison

| Agent (CLI/tool) | Primary mechanism | Persistent index? | Search primitives | What enters the window | Where search runs | Family |
|---|---|---|---|---|---|---|
| **Codex CLI** (OpenAI) [1,2,4,17] | Agentic local coding loop; exact retrieval path still to calibrate | Not established by official docs | Documented local file reads/edits, shell commands, `apply_patch` hooks; optional web/MCP | Whatever local tool observations enter the transcript, cumulatively | Local sandbox for file/shell work; web/MCP external if enabled | A (provisional) |
...
### Concrete search primitives — local commands and structured wrappers

Family A commonly reduces to a small set of POSIX text/file utilities invoked either directly (a shell tool) or through thin structured wrappers. The same four abstract operations recur — *discover files*, *search content*, *read a slice*, *navigate within a file* — and agents realize them with concrete commands or wrapper tools. For a benchmark this matters concretely: every local read/search command that actually opens files bottoms out in `open()`/`read()` syscalls that the FUSE tap logs ([`design.md` §6.2](./design.md#62-how-we-capture-it-defense-in-depth)), so the kernel tap captures it once calibration confirms the pinned agent is using local filesystem retrieval. It is exactly here that read **volume** diverges from in-context **tokens** (a `grep -r`/`rg` scan touches far more bytes than ever enter the window; a `sed -n`/`head` reads a slice that mostly does) — the read-volume-≠-tokens distinction in [`design.md` §6.1](./design.md#61-what-we-capture).

| Abstract operation | Typical local shell commands (shell-capable agents; Codex examples require calibration) [1,2,4,5,12,17] | Structured-tool form |
|---|---|---|
| **Discover files** (by name/glob) | `find . -name '*.py'`, `fd`, `ls -R`, `tree`, shell globs `**/*.py` | Claude Code `Glob`; SWE-agent `find_file` |
...
Three clarifications matter for interpreting the benchmark:
...
- **Codex `@` / `/mention` is prompt attachment, not autonomous search.** Current Codex docs describe `@` and `/mention` as composer mechanisms for the user to attach files or paths as context [17]. They are relevant to task setup and contamination controls, but they are not listed above as an agent-issued search primitive.
...
**Codex CLI (OpenAI).** Open-source, implemented in Rust; runs an agentic local coding workflow that can read the working directory, make edits, and run model-generated shell commands in a sandbox [1, 2, 17]. Current OpenAI docs establish local file reads/edits, shell commands, first-party **web search** (cached by default; live via `--search`), **MCP** servers for additional tools, and hook matching for `apply_patch` [1, 2, 17]. They do **not** establish a supported "no precomputed index" guarantee or the exact commands Codex chooses for repository search. Reverse-engineering write-ups and observed sessions often show shell-style search/read commands such as `rg`, `grep`, `sed -n`, `cat`, `ls`, and `find` [4], but this should be treated as inferred behavior until the pinned benchmark build is traced. Sandbox: macOS uses **Seatbelt**; Linux/WSL2 uses **bubblewrap** (`bwrap`) when available or a bundled helper with user-namespace support [17]. *Benchmark relevance:* if calibration confirms Codex's local repository reads cross the kernel, the FUSE/OverlayFS tap captures them without an in-process shim ([`design.md` §6.2](./design.md#62-how-we-capture-it-defense-in-depth)); if a future/pinned configuration uses server-side or MCP retrieval, a tool-boundary tap is required as well.
...
Mapping the families onto the three quantities the instrumentation already distinguishes — distractor volume *on disk* ⊇ distractor files *visited* (FS tap → `irrelevant_file_read_ratio`) ⊇ distractor tokens *in context* (transcript tap → `input_tokens_*`) ([`design.md` §6.1](./design.md#61-what-we-capture)) — yields concrete, pre-registerable predictions.
...
1. **The FS tap is correct for confirmed local filesystem retrieval, partial for Family B/C.** The kernel-level FUSE/OverlayFS tap ([`design.md` §6.2](./design.md#62-how-we-capture-it-defense-in-depth)) captures every `open`/`read` an agent makes *through the filesystem* — the right choice for shell-spawned searches and direct local file reads, and one the calibration gate must verify for the pinned Codex build. But an embedding agent's semantic retrieval (Cursor) is answered **off the local FS** (server-side vectors), and a repo-map agent (Aider) reads files once at index time, not at query time. For those, "what the agent searched" lives at the **tool/retrieval boundary**, not the syscall boundary, and a cross-agent extension must add a **tool-call/transcript tap that records the retrieval query and the returned chunks** — an extension of the transcript tap in [`design.md` §6.2](./design.md#62-how-we-capture-it-defense-in-depth).
...
4. **It sharpens "Still to confirm" #2.** [`design.md` §10](./design.md#10-locked-decisions) asks what the pinned Codex read/search execution path actually is. The taxonomy answers *why it matters*: shelling out or direct local file reads keep the FS tap authoritative for read-volume metrics; server-side retrieval, MCP semantic search, or other index-backed paths require the tool-boundary tap of point 1.
...
- **Mixed source quality.** Where a vendor publishes mechanism details, we cite the **primary** source (OpenAI Codex docs [1, 2, 17]; Anthropic engineering [5]; Cursor docs [10, 11]; Aider docs [8, 9]; the SWE-agent [12] and Agentless [13] papers). Some operational specifics (e.g., Codex's exact local search commands, the "agentic search > RAG" quote) come from **reverse-engineering write-ups and reported statements** [4, 6, 7, 16] and are labeled as such; they should be re-verified before being load-bearing in the paper.
...
17. OpenAI. *Codex manual* (documentation). https://developers.openai.com/codex/codex-manual.md
