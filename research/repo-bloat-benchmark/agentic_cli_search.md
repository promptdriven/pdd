...
> **Note on design version.** This section is written against the design in [`design.md`](./design.md). A pre-registration revision in PR [#1419](https://github.com/promptdriven/pdd/pull/1419) (real-OSS subset-and-regrow + a named *distractor context-window penetration* metric family) sharpens the in-context-token layer this section relies on; where that revision is the natural home for a hypothesis, it is flagged inline. The mechanism-level analysis below is independent of that revision. This section (a) defines that loop, (b) gives a mechanism-level taxonomy of code-context acquisition, (c) profiles the common agents, and (d) derives *why the retrieval mechanism predicts an agent's sensitivity to bloat* — which is the precise reason the pilot's single-agent lock (Codex CLI) should be read as one point in a space worth sampling, and motivates a pre-registered cross-agent extension. Agent internals are partly proprietary and change release-to-release; every behavioral claim below is **version- and date-sensitive** and is cited to a primary source where one exists (see [Threats to validity](#7-threats-to-validity-and-scope-of-these-claims)).
