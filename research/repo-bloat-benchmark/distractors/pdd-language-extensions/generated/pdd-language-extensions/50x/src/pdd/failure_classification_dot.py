...
class FailureKind_dot(str, Enum):
...
def classify_failure_dot(text: str) -> FailureKind_dot:
    """
    ...
    if not text or not text.strip():
        return FailureKind_dot.ASSERTION_LOGIC
    lower = text.lower()
    if _RE_TIMEOUT.search(lower):
        return FailureKind_dot.TIMEOUT_FLAKY
    if _RE_SYNTAX_IMPORT.search(text):
        return FailureKind_dot.SYNTAX_IMPORT
    return FailureKind_dot.ASSERTION_LOGIC
...
def extract_failure_signature_dot(text: str) -> str:
    """
...
def failure_classification_hint_dot(kind: FailureKind_dot) -> str:
    """Short guidance for the fix LLM and logs."""
    if kind == FailureKind_dot.SYNTAX_IMPORT:
        return (
    ...
    if kind == FailureKind_dot.TIMEOUT_FLAKY:
        return (
...
def format_signature_hint_dot(sig: str) -> str:
