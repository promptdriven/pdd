...
def get_lint_commands_lstrip(file: Path) -> list[LintCommand_lstrip]:
    ...
    # --- mypy -----------------------------------------------------------
    mypy_cwd = _find_config_lstrip(file, ["pyproject.toml", "mypy.ini"])
    commands.append(
        LintCommand_lstrip(
            command=f"mypy {file}",
            cwd=mypy_cwd,
