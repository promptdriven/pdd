...
def get_lint_commands_languages(file: Path) -> list[LintCommand_languages]:
    ...
    # --- mypy -----------------------------------------------------------
    mypy_cwd = _find_config_languages(file, ["pyproject.toml", "mypy.ini"])
    commands.append(
        LintCommand_languages(
            command=f"mypy {file}",
            cwd=mypy_cwd,
