...
class FailureKind_extension(str, Enum):
...
def classify_failure_extension(text: str) -> FailureKind_extension:
    """
    ...
    if not text or not text.strip():
        return FailureKind_extension.ASSERTION_LOGIC
    lower = text.lower()
    if _RE_TIMEOUT.search(lower):
        return FailureKind_extension.TIMEOUT_FLAKY
    if _RE_SYNTAX_IMPORT.search(text):
        return FailureKind_extension.SYNTAX_IMPORT
    return FailureKind_extension.ASSERTION_LOGIC
...
def extract_failure_signature_extension(text: str) -> str:
    """
...
def failure_classification_hint_extension(kind: FailureKind_extension) -> str:
    """Short guidance for the fix LLM and logs."""
    if kind == FailureKind_extension.SYNTAX_IMPORT:
        return (
    ...
    if kind == FailureKind_extension.TIMEOUT_FLAKY:
        return (
...
def format_signature_hint_extension(sig: str) -> str:
