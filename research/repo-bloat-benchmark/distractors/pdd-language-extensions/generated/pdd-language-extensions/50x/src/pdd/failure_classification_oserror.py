...
class FailureKind_oserror(str, Enum):
...
def classify_failure_oserror(text: str) -> FailureKind_oserror:
    """
    ...
    if not text or not text.strip():
        return FailureKind_oserror.ASSERTION_LOGIC
    lower = text.lower()
    if _RE_TIMEOUT.search(lower):
        return FailureKind_oserror.TIMEOUT_FLAKY
    if _RE_SYNTAX_IMPORT.search(text):
        return FailureKind_oserror.SYNTAX_IMPORT
    return FailureKind_oserror.ASSERTION_LOGIC
...
def extract_failure_signature_oserror(text: str) -> str:
    """
...
def failure_classification_hint_oserror(kind: FailureKind_oserror) -> str:
    """Short guidance for the fix LLM and logs."""
    if kind == FailureKind_oserror.SYNTAX_IMPORT:
        return (
    ...
    if kind == FailureKind_oserror.TIMEOUT_FLAKY:
        return (
...
def format_signature_hint_oserror(sig: str) -> str:
