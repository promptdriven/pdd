...
def get_lint_commands_exc(file: Path) -> list[LintCommand_exc]:
    ...
    # --- mypy -----------------------------------------------------------
    mypy_cwd = _find_config_exc(file, ["pyproject.toml", "mypy.ini"])
    commands.append(
        LintCommand_exc(
            command=f"mypy {file}",
            cwd=mypy_cwd,
