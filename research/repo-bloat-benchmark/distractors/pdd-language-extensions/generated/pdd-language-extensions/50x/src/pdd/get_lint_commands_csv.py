...
def get_lint_commands_csv(file: Path) -> list[LintCommand_csv]:
    ...
    # --- mypy -----------------------------------------------------------
    mypy_cwd = _find_config_csv(file, ["pyproject.toml", "mypy.ini"])
    commands.append(
        LintCommand_csv(
            command=f"mypy {file}",
            cwd=mypy_cwd,
