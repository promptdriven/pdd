...
class FailureKind_lower(str, Enum):
...
def classify_failure_lower(text: str) -> FailureKind_lower:
    """
    ...
    if not text or not text.strip():
        return FailureKind_lower.ASSERTION_LOGIC
    lower = text.lower()
    if _RE_TIMEOUT.search(lower):
        return FailureKind_lower.TIMEOUT_FLAKY
    if _RE_SYNTAX_IMPORT.search(text):
        return FailureKind_lower.SYNTAX_IMPORT
    return FailureKind_lower.ASSERTION_LOGIC
...
def extract_failure_signature_lower(text: str) -> str:
    """
...
def failure_classification_hint_lower(kind: FailureKind_lower) -> str:
    """Short guidance for the fix LLM and logs."""
    if kind == FailureKind_lower.SYNTAX_IMPORT:
        return (
    ...
    if kind == FailureKind_lower.TIMEOUT_FLAKY:
        return (
...
def format_signature_hint_lower(sig: str) -> str:
