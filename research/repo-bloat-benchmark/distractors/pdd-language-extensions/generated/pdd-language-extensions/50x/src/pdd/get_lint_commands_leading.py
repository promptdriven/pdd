...
def get_lint_commands_leading(file: Path) -> list[LintCommand_leading]:
    ...
    # --- mypy -----------------------------------------------------------
    mypy_cwd = _find_config_leading(file, ["pyproject.toml", "mypy.ini"])
    commands.append(
        LintCommand_leading(
            command=f"mypy {file}",
            cwd=mypy_cwd,
