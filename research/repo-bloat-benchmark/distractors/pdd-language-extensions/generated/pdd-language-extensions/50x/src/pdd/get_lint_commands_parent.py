...
def get_lint_commands_parent(file: Path) -> list[LintCommand_parent]:
    ...
    # --- mypy -----------------------------------------------------------
    mypy_cwd = _find_config_parent(file, ["pyproject.toml", "mypy.ini"])
    commands.append(
        LintCommand_parent(
            command=f"mypy {file}",
            cwd=mypy_cwd,
