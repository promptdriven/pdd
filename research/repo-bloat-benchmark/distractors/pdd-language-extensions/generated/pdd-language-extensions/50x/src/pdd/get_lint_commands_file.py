...
def get_lint_commands_file(file: Path) -> list[LintCommand_file]:
    ...
    # --- mypy -----------------------------------------------------------
    mypy_cwd = _find_config_file(file, ["pyproject.toml", "mypy.ini"])
    commands.append(
        LintCommand_file(
            command=f"mypy {file}",
            cwd=mypy_cwd,
