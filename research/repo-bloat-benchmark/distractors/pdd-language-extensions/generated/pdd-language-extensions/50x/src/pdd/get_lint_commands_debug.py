...
def get_lint_commands_debug(file: Path) -> list[LintCommand_debug]:
    ...
    # --- mypy -----------------------------------------------------------
    mypy_cwd = _find_config_debug(file, ["pyproject.toml", "mypy.ini"])
    commands.append(
        LintCommand_debug(
            command=f"mypy {file}",
            cwd=mypy_cwd,
