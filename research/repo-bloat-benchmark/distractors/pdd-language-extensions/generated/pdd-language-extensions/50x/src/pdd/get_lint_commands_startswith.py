...
def get_lint_commands_startswith(file: Path) -> list[LintCommand_startswith]:
    ...
    # --- mypy -----------------------------------------------------------
    mypy_cwd = _find_config_startswith(file, ["pyproject.toml", "mypy.ini"])
    commands.append(
        LintCommand_startswith(
            command=f"mypy {file}",
            cwd=mypy_cwd,
