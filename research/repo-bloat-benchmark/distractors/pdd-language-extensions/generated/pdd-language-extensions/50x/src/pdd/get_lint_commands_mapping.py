...
def get_lint_commands_mapping(file: Path) -> list[LintCommand_mapping]:
    ...
    # --- mypy -----------------------------------------------------------
    mypy_cwd = _find_config_mapping(file, ["pyproject.toml", "mypy.ini"])
    commands.append(
        LintCommand_mapping(
            command=f"mypy {file}",
            cwd=mypy_cwd,
