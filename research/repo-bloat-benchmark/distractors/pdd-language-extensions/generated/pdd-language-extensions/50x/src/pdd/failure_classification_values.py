...
class FailureKind_values(str, Enum):
...
def classify_failure_values(text: str) -> FailureKind_values:
    """
    ...
    if not text or not text.strip():
        return FailureKind_values.ASSERTION_LOGIC
    lower = text.lower()
    if _RE_TIMEOUT.search(lower):
        return FailureKind_values.TIMEOUT_FLAKY
    if _RE_SYNTAX_IMPORT.search(text):
        return FailureKind_values.SYNTAX_IMPORT
    return FailureKind_values.ASSERTION_LOGIC
...
def extract_failure_signature_values(text: str) -> str:
    """
...
def failure_classification_hint_values(kind: FailureKind_values) -> str:
    """Short guidance for the fix LLM and logs."""
    if kind == FailureKind_values.SYNTAX_IMPORT:
        return (
    ...
    if kind == FailureKind_values.TIMEOUT_FLAKY:
        return (
...
def format_signature_hint_values(sig: str) -> str:
