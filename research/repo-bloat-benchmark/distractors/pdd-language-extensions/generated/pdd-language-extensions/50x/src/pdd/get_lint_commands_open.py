...
def get_lint_commands_open(file: Path) -> list[LintCommand_open]:
    ...
    # --- mypy -----------------------------------------------------------
    mypy_cwd = _find_config_open(file, ["pyproject.toml", "mypy.ini"])
    commands.append(
        LintCommand_open(
            command=f"mypy {file}",
            cwd=mypy_cwd,
