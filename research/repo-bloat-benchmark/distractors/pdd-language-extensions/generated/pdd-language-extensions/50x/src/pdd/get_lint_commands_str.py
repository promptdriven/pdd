...
def get_lint_commands_str(file: Path) -> list[LintCommand_str]:
    ...
    # --- mypy -----------------------------------------------------------
    mypy_cwd = _find_config_str(file, ["pyproject.toml", "mypy.ini"])
    commands.append(
        LintCommand_str(
            command=f"mypy {file}",
            cwd=mypy_cwd,
