...
class FailureKind_case(str, Enum):
...
def classify_failure_case(text: str) -> FailureKind_case:
    """
    ...
    if not text or not text.strip():
        return FailureKind_case.ASSERTION_LOGIC
    lower = text.lower()
    if _RE_TIMEOUT.search(lower):
        return FailureKind_case.TIMEOUT_FLAKY
    if _RE_SYNTAX_IMPORT.search(text):
        return FailureKind_case.SYNTAX_IMPORT
    return FailureKind_case.ASSERTION_LOGIC
...
def extract_failure_signature_case(text: str) -> str:
    """
...
def failure_classification_hint_case(kind: FailureKind_case) -> str:
    """Short guidance for the fix LLM and logs."""
    if kind == FailureKind_case.SYNTAX_IMPORT:
        return (
    ...
    if kind == FailureKind_case.TIMEOUT_FLAKY:
        return (
...
def format_signature_hint_case(sig: str) -> str:
