...
class FailureKind_handle(str, Enum):
...
def classify_failure_handle(text: str) -> FailureKind_handle:
    """
    ...
    if not text or not text.strip():
        return FailureKind_handle.ASSERTION_LOGIC
    lower = text.lower()
    if _RE_TIMEOUT.search(lower):
        return FailureKind_handle.TIMEOUT_FLAKY
    if _RE_SYNTAX_IMPORT.search(text):
        return FailureKind_handle.SYNTAX_IMPORT
    return FailureKind_handle.ASSERTION_LOGIC
...
def extract_failure_signature_handle(text: str) -> str:
    """
...
def failure_classification_hint_handle(kind: FailureKind_handle) -> str:
    """Short guidance for the fix LLM and logs."""
    if kind == FailureKind_handle.SYNTAX_IMPORT:
        return (
    ...
    if kind == FailureKind_handle.TIMEOUT_FLAKY:
        return (
...
def format_signature_hint_handle(sig: str) -> str:
