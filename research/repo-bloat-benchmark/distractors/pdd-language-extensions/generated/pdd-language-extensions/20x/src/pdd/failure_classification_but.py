...
class FailureKind_but(str, Enum):
...
def classify_failure_but(text: str) -> FailureKind_but:
    """
    ...
    if not text or not text.strip():
        return FailureKind_but.ASSERTION_LOGIC
    lower = text.lower()
    if _RE_TIMEOUT.search(lower):
        return FailureKind_but.TIMEOUT_FLAKY
    if _RE_SYNTAX_IMPORT.search(text):
        return FailureKind_but.SYNTAX_IMPORT
    return FailureKind_but.ASSERTION_LOGIC
...
def extract_failure_signature_but(text: str) -> str:
    """
...
def failure_classification_hint_but(kind: FailureKind_but) -> str:
    """Short guidance for the fix LLM and logs."""
    if kind == FailureKind_but.SYNTAX_IMPORT:
        return (
    ...
    if kind == FailureKind_but.TIMEOUT_FLAKY:
        return (
...
def format_signature_hint_but(sig: str) -> str:
