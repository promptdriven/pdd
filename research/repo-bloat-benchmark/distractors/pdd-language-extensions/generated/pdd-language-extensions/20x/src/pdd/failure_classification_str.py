...
class FailureKind_str(str, Enum):
...
def classify_failure_str(text: str) -> FailureKind_str:
    """
    ...
    if not text or not text.strip():
        return FailureKind_str.ASSERTION_LOGIC
    lower = text.lower()
    if _RE_TIMEOUT.search(lower):
        return FailureKind_str.TIMEOUT_FLAKY
    if _RE_SYNTAX_IMPORT.search(text):
        return FailureKind_str.SYNTAX_IMPORT
    return FailureKind_str.ASSERTION_LOGIC
...
def extract_failure_signature_str(text: str) -> str:
    """
...
def failure_classification_hint_str(kind: FailureKind_str) -> str:
    """Short guidance for the fix LLM and logs."""
    if kind == FailureKind_str.SYNTAX_IMPORT:
        return (
    ...
    if kind == FailureKind_str.TIMEOUT_FLAKY:
        return (
...
def format_signature_hint_str(sig: str) -> str:
