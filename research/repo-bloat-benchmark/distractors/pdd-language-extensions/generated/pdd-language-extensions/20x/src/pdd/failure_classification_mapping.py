...
class FailureKind_mapping(str, Enum):
...
def classify_failure_mapping(text: str) -> FailureKind_mapping:
    """
    ...
    if not text or not text.strip():
        return FailureKind_mapping.ASSERTION_LOGIC
    lower = text.lower()
    if _RE_TIMEOUT.search(lower):
        return FailureKind_mapping.TIMEOUT_FLAKY
    if _RE_SYNTAX_IMPORT.search(text):
        return FailureKind_mapping.SYNTAX_IMPORT
    return FailureKind_mapping.ASSERTION_LOGIC
...
def extract_failure_signature_mapping(text: str) -> str:
    """
...
def failure_classification_hint_mapping(kind: FailureKind_mapping) -> str:
    """Short guidance for the fix LLM and logs."""
    if kind == FailureKind_mapping.SYNTAX_IMPORT:
        return (
    ...
    if kind == FailureKind_mapping.TIMEOUT_FLAKY:
        return (
...
def format_signature_hint_mapping(sig: str) -> str:
