...
def get_lint_commands_cache(file: Path) -> list[LintCommand_cache]:
    ...
    # --- mypy -----------------------------------------------------------
    mypy_cwd = _find_config_cache(file, ["pyproject.toml", "mypy.ini"])
    commands.append(
        LintCommand_cache(
            command=f"mypy {file}",
            cwd=mypy_cwd,
