...
def get_lint_commands_lookup(file: Path) -> list[LintCommand_lookup]:
    ...
    # --- mypy -----------------------------------------------------------
    mypy_cwd = _find_config_lookup(file, ["pyproject.toml", "mypy.ini"])
    commands.append(
        LintCommand_lookup(
            command=f"mypy {file}",
            cwd=mypy_cwd,
