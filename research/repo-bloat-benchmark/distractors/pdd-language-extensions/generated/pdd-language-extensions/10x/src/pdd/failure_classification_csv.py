...
class FailureKind_csv(str, Enum):
...
def classify_failure_csv(text: str) -> FailureKind_csv:
    """
    ...
    if not text or not text.strip():
        return FailureKind_csv.ASSERTION_LOGIC
    lower = text.lower()
    if _RE_TIMEOUT.search(lower):
        return FailureKind_csv.TIMEOUT_FLAKY
    if _RE_SYNTAX_IMPORT.search(text):
        return FailureKind_csv.SYNTAX_IMPORT
    return FailureKind_csv.ASSERTION_LOGIC
...
def extract_failure_signature_csv(text: str) -> str:
    """
...
def failure_classification_hint_csv(kind: FailureKind_csv) -> str:
    """Short guidance for the fix LLM and logs."""
    if kind == FailureKind_csv.SYNTAX_IMPORT:
        return (
    ...
    if kind == FailureKind_csv.TIMEOUT_FLAKY:
        return (
...
def format_signature_hint_csv(sig: str) -> str:
