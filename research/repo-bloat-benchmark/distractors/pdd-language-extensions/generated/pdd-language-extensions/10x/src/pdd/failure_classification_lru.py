...
class FailureKind_lru(str, Enum):
...
def classify_failure_lru(text: str) -> FailureKind_lru:
    """
    ...
    if not text or not text.strip():
        return FailureKind_lru.ASSERTION_LOGIC
    lower = text.lower()
    if _RE_TIMEOUT.search(lower):
        return FailureKind_lru.TIMEOUT_FLAKY
    if _RE_SYNTAX_IMPORT.search(text):
        return FailureKind_lru.SYNTAX_IMPORT
    return FailureKind_lru.ASSERTION_LOGIC
...
def extract_failure_signature_lru(text: str) -> str:
    """
...
def failure_classification_hint_lru(kind: FailureKind_lru) -> str:
    """Short guidance for the fix LLM and logs."""
    if kind == FailureKind_lru.SYNTAX_IMPORT:
        return (
    ...
    if kind == FailureKind_lru.TIMEOUT_FLAKY:
        return (
...
def format_signature_hint_lru(sig: str) -> str:
