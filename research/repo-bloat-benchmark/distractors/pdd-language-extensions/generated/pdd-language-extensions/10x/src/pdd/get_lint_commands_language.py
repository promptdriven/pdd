...
def get_lint_commands_language(file: Path) -> list[LintCommand_language]:
    ...
    # --- mypy -----------------------------------------------------------
    mypy_cwd = _find_config_language(file, ["pyproject.toml", "mypy.ini"])
    commands.append(
        LintCommand_language(
            command=f"mypy {file}",
            cwd=mypy_cwd,
