...
class FailureKind_index(str, Enum):
...
def classify_failure_index(text: str) -> FailureKind_index:
    """
    ...
    if not text or not text.strip():
        return FailureKind_index.ASSERTION_LOGIC
    lower = text.lower()
    if _RE_TIMEOUT.search(lower):
        return FailureKind_index.TIMEOUT_FLAKY
    if _RE_SYNTAX_IMPORT.search(text):
        return FailureKind_index.SYNTAX_IMPORT
    return FailureKind_index.ASSERTION_LOGIC
...
def extract_failure_signature_index(text: str) -> str:
    """
...
def failure_classification_hint_index(kind: FailureKind_index) -> str:
    """Short guidance for the fix LLM and logs."""
    if kind == FailureKind_index.SYNTAX_IMPORT:
        return (
    ...
    if kind == FailureKind_index.TIMEOUT_FLAKY:
        return (
...
def format_signature_hint_index(sig: str) -> str:
