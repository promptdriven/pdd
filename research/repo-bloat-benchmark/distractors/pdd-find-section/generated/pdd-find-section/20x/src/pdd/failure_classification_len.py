...
class FailureKind_len(str, Enum):
...
def classify_failure_len(text: str) -> FailureKind_len:
    """
    ...
    if not text or not text.strip():
        return FailureKind_len.ASSERTION_LOGIC
    lower = text.lower()
    if _RE_TIMEOUT.search(lower):
        return FailureKind_len.TIMEOUT_FLAKY
    if _RE_SYNTAX_IMPORT.search(text):
        return FailureKind_len.SYNTAX_IMPORT
    return FailureKind_len.ASSERTION_LOGIC
...
def extract_failure_signature_len(text: str) -> str:
    """
...
def failure_classification_hint_len(kind: FailureKind_len) -> str:
    """Short guidance for the fix LLM and logs."""
    if kind == FailureKind_len.SYNTAX_IMPORT:
        return (
    ...
    if kind == FailureKind_len.TIMEOUT_FLAKY:
        return (
...
def format_signature_hint_len(sig: str) -> str:
