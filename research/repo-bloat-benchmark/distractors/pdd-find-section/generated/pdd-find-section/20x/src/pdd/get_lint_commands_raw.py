...
def get_lint_commands_raw(file: Path) -> list[LintCommand_raw]:
    ...
    # --- mypy -----------------------------------------------------------
    mypy_cwd = _find_config_raw(file, ["pyproject.toml", "mypy.ini"])
    commands.append(
        LintCommand_raw(
            command=f"mypy {file}",
            cwd=mypy_cwd,
