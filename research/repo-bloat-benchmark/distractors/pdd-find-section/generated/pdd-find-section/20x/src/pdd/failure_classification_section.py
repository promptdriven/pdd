...
class FailureKind_section(str, Enum):
...
def classify_failure_section(text: str) -> FailureKind_section:
    """
    ...
    if not text or not text.strip():
        return FailureKind_section.ASSERTION_LOGIC
    lower = text.lower()
    if _RE_TIMEOUT.search(lower):
        return FailureKind_section.TIMEOUT_FLAKY
    if _RE_SYNTAX_IMPORT.search(text):
        return FailureKind_section.SYNTAX_IMPORT
    return FailureKind_section.ASSERTION_LOGIC
...
def extract_failure_signature_section(text: str) -> str:
    """
...
def failure_classification_hint_section(kind: FailureKind_section) -> str:
    """Short guidance for the fix LLM and logs."""
    if kind == FailureKind_section.SYNTAX_IMPORT:
        return (
    ...
    if kind == FailureKind_section.TIMEOUT_FLAKY:
        return (
...
def format_signature_hint_section(sig: str) -> str:
