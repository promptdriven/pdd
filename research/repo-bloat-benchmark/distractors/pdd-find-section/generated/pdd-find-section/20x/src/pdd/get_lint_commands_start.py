...
def get_lint_commands_start(file: Path) -> list[LintCommand_start]:
    ...
    # --- mypy -----------------------------------------------------------
    mypy_cwd = _find_config_start(file, ["pyproject.toml", "mypy.ini"])
    commands.append(
        LintCommand_start(
            command=f"mypy {file}",
            cwd=mypy_cwd,
