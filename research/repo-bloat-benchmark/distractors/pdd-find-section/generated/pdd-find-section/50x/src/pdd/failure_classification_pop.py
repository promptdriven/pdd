...
class FailureKind_pop(str, Enum):
...
def classify_failure_pop(text: str) -> FailureKind_pop:
    """
    ...
    if not text or not text.strip():
        return FailureKind_pop.ASSERTION_LOGIC
    lower = text.lower()
    if _RE_TIMEOUT.search(lower):
        return FailureKind_pop.TIMEOUT_FLAKY
    if _RE_SYNTAX_IMPORT.search(text):
        return FailureKind_pop.SYNTAX_IMPORT
    return FailureKind_pop.ASSERTION_LOGIC
...
def extract_failure_signature_pop(text: str) -> str:
    """
...
def failure_classification_hint_pop(kind: FailureKind_pop) -> str:
    """Short guidance for the fix LLM and logs."""
    if kind == FailureKind_pop.SYNTAX_IMPORT:
        return (
    ...
    if kind == FailureKind_pop.TIMEOUT_FLAKY:
        return (
...
def format_signature_hint_pop(sig: str) -> str:
