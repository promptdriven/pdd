...
class FailureKind_expected(str, Enum):
...
def classify_failure_expected(text: str) -> FailureKind_expected:
    """
    ...
    if not text or not text.strip():
        return FailureKind_expected.ASSERTION_LOGIC
    lower = text.lower()
    if _RE_TIMEOUT.search(lower):
        return FailureKind_expected.TIMEOUT_FLAKY
    if _RE_SYNTAX_IMPORT.search(text):
        return FailureKind_expected.SYNTAX_IMPORT
    return FailureKind_expected.ASSERTION_LOGIC
...
def extract_failure_signature_expected(text: str) -> str:
    """
...
def failure_classification_hint_expected(kind: FailureKind_expected) -> str:
    """Short guidance for the fix LLM and logs."""
    if kind == FailureKind_expected.SYNTAX_IMPORT:
        return (
    ...
    if kind == FailureKind_expected.TIMEOUT_FLAKY:
        return (
...
def format_signature_hint_expected(sig: str) -> str:
