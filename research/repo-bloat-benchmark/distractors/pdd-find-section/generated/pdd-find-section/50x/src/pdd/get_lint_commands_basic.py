...
def get_lint_commands_basic(file: Path) -> list[LintCommand_basic]:
    ...
    # --- mypy -----------------------------------------------------------
    mypy_cwd = _find_config_basic(file, ["pyproject.toml", "mypy.ini"])
    commands.append(
        LintCommand_basic(
            command=f"mypy {file}",
            cwd=mypy_cwd,
