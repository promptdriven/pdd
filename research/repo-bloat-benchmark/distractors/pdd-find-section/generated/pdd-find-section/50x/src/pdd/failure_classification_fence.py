...
class FailureKind_fence(str, Enum):
...
def classify_failure_fence(text: str) -> FailureKind_fence:
    """
    ...
    if not text or not text.strip():
        return FailureKind_fence.ASSERTION_LOGIC
    lower = text.lower()
    if _RE_TIMEOUT.search(lower):
        return FailureKind_fence.TIMEOUT_FLAKY
    if _RE_SYNTAX_IMPORT.search(text):
        return FailureKind_fence.SYNTAX_IMPORT
    return FailureKind_fence.ASSERTION_LOGIC
...
def extract_failure_signature_fence(text: str) -> str:
    """
...
def failure_classification_hint_fence(kind: FailureKind_fence) -> str:
    """Short guidance for the fix LLM and logs."""
    if kind == FailureKind_fence.SYNTAX_IMPORT:
        return (
    ...
    if kind == FailureKind_fence.TIMEOUT_FLAKY:
        return (
...
def format_signature_hint_fence(sig: str) -> str:
