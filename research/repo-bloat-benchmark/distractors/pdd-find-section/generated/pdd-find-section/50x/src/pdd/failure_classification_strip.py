...
class FailureKind_strip(str, Enum):
...
def classify_failure_strip(text: str) -> FailureKind_strip:
    """
    ...
    if not text or not text.strip():
        return FailureKind_strip.ASSERTION_LOGIC
    lower = text.lower()
    if _RE_TIMEOUT.search(lower):
        return FailureKind_strip.TIMEOUT_FLAKY
    if _RE_SYNTAX_IMPORT.search(text):
        return FailureKind_strip.SYNTAX_IMPORT
    return FailureKind_strip.ASSERTION_LOGIC
...
def extract_failure_signature_strip(text: str) -> str:
    """
...
def failure_classification_hint_strip(kind: FailureKind_strip) -> str:
    """Short guidance for the fix LLM and logs."""
    if kind == FailureKind_strip.SYNTAX_IMPORT:
        return (
    ...
    if kind == FailureKind_strip.TIMEOUT_FLAKY:
        return (
...
def format_signature_hint_strip(sig: str) -> str:
