...
class FailureKind_nested(str, Enum):
...
def classify_failure_nested(text: str) -> FailureKind_nested:
    """
    ...
    if not text or not text.strip():
        return FailureKind_nested.ASSERTION_LOGIC
    lower = text.lower()
    if _RE_TIMEOUT.search(lower):
        return FailureKind_nested.TIMEOUT_FLAKY
    if _RE_SYNTAX_IMPORT.search(text):
        return FailureKind_nested.SYNTAX_IMPORT
    return FailureKind_nested.ASSERTION_LOGIC
...
def extract_failure_signature_nested(text: str) -> str:
    """
...
def failure_classification_hint_nested(kind: FailureKind_nested) -> str:
    """Short guidance for the fix LLM and logs."""
    if kind == FailureKind_nested.SYNTAX_IMPORT:
        return (
    ...
    if kind == FailureKind_nested.TIMEOUT_FLAKY:
        return (
...
def format_signature_hint_nested(sig: str) -> str:
