...
def get_lint_commands_found(file: Path) -> list[LintCommand_found]:
    ...
    # --- mypy -----------------------------------------------------------
    mypy_cwd = _find_config_found(file, ["pyproject.toml", "mypy.ini"])
    commands.append(
        LintCommand_found(
            command=f"mypy {file}",
            cwd=mypy_cwd,
