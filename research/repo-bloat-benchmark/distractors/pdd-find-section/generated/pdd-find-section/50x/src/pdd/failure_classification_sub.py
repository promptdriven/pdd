...
class FailureKind_sub(str, Enum):
...
def classify_failure_sub(text: str) -> FailureKind_sub:
    """
    ...
    if not text or not text.strip():
        return FailureKind_sub.ASSERTION_LOGIC
    lower = text.lower()
    if _RE_TIMEOUT.search(lower):
        return FailureKind_sub.TIMEOUT_FLAKY
    if _RE_SYNTAX_IMPORT.search(text):
        return FailureKind_sub.SYNTAX_IMPORT
    return FailureKind_sub.ASSERTION_LOGIC
...
def extract_failure_signature_sub(text: str) -> str:
    """
...
def failure_classification_hint_sub(kind: FailureKind_sub) -> str:
    """Short guidance for the fix LLM and logs."""
    if kind == FailureKind_sub.SYNTAX_IMPORT:
        return (
    ...
    if kind == FailureKind_sub.TIMEOUT_FLAKY:
        return (
...
def format_signature_hint_sub(sig: str) -> str:
