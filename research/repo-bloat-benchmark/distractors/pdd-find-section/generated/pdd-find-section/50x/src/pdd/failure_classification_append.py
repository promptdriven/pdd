...
class FailureKind_append(str, Enum):
...
def classify_failure_append(text: str) -> FailureKind_append:
    """
    ...
    if not text or not text.strip():
        return FailureKind_append.ASSERTION_LOGIC
    lower = text.lower()
    if _RE_TIMEOUT.search(lower):
        return FailureKind_append.TIMEOUT_FLAKY
    if _RE_SYNTAX_IMPORT.search(text):
        return FailureKind_append.SYNTAX_IMPORT
    return FailureKind_append.ASSERTION_LOGIC
...
def extract_failure_signature_append(text: str) -> str:
    """
...
def failure_classification_hint_append(kind: FailureKind_append) -> str:
    """Short guidance for the fix LLM and logs."""
    if kind == FailureKind_append.SYNTAX_IMPORT:
        return (
    ...
    if kind == FailureKind_append.TIMEOUT_FLAKY:
        return (
...
def format_signature_hint_append(sig: str) -> str:
