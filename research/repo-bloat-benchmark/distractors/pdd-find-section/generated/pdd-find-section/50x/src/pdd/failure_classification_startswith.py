...
class FailureKind_startswith(str, Enum):
...
def classify_failure_startswith(text: str) -> FailureKind_startswith:
    """
    ...
    if not text or not text.strip():
        return FailureKind_startswith.ASSERTION_LOGIC
    lower = text.lower()
    if _RE_TIMEOUT.search(lower):
        return FailureKind_startswith.TIMEOUT_FLAKY
    if _RE_SYNTAX_IMPORT.search(text):
        return FailureKind_startswith.SYNTAX_IMPORT
    return FailureKind_startswith.ASSERTION_LOGIC
...
def extract_failure_signature_startswith(text: str) -> str:
    """
...
def failure_classification_hint_startswith(kind: FailureKind_startswith) -> str:
    """Short guidance for the fix LLM and logs."""
    if kind == FailureKind_startswith.SYNTAX_IMPORT:
        return (
    ...
    if kind == FailureKind_startswith.TIMEOUT_FLAKY:
        return (
...
def format_signature_hint_startswith(sig: str) -> str:
