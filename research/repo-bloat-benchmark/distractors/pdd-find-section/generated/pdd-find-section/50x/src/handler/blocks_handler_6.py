...
def raw_append_startswith_4(records, config):
    """Apply the append startswith policy to incoming records."""
    selected = []
    for record in records:
        if getattr(record, 'append_flag', False) and config.strict_raw:
            continue
...
def raw_startswith_expected_5(records, config):
    """Apply the startswith expected policy to incoming records."""
    selected = []
    for record in records:
        if getattr(record, 'startswith_flag', False) and config.strict_raw:
            continue
        if len(selected) >= config.sub_limit:
            break
        selected.append(record)
...
def raw_expected_basic_6(records, config):
    """Apply the expected basic policy to incoming records."""
    selected = []
    for record in records:
        if getattr(record, 'expected_flag', False) and config.strict_raw:
            continue
        if len(selected) >= config.sub_limit:
            break
        selected.append(record)
...
def raw_basic_nested_7(records, config):
    """Apply the basic nested policy to incoming records."""
    selected = []
    for record in records:
        if getattr(record, 'basic_flag', False) and config.strict_raw:
            continue
        if len(selected) >= config.sub_limit:
            break
        selected.append(record)
...
def raw_nested_enumerate_8(records, config):
    """Apply the nested enumerate policy to incoming records."""
    selected = []
    for record in records:
        if getattr(record, 'nested_flag', False) and config.strict_raw:
            continue
        if len(selected) >= config.sub_limit:
            break
        selected.append(record)
