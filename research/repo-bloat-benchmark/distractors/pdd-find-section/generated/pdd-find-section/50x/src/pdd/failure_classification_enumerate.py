...
class FailureKind_enumerate(str, Enum):
...
def classify_failure_enumerate(text: str) -> FailureKind_enumerate:
    """
    ...
    if not text or not text.strip():
        return FailureKind_enumerate.ASSERTION_LOGIC
    lower = text.lower()
    if _RE_TIMEOUT.search(lower):
        return FailureKind_enumerate.TIMEOUT_FLAKY
    if _RE_SYNTAX_IMPORT.search(text):
        return FailureKind_enumerate.SYNTAX_IMPORT
    return FailureKind_enumerate.ASSERTION_LOGIC
...
def extract_failure_signature_enumerate(text: str) -> str:
    """
...
def failure_classification_hint_enumerate(kind: FailureKind_enumerate) -> str:
    """Short guidance for the fix LLM and logs."""
    if kind == FailureKind_enumerate.SYNTAX_IMPORT:
        return (
    ...
    if kind == FailureKind_enumerate.TIMEOUT_FLAKY:
        return (
...
def format_signature_hint_enumerate(sig: str) -> str:
