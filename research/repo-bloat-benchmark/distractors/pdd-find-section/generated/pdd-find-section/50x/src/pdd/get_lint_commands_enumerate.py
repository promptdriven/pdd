...
def get_lint_commands_enumerate(file: Path) -> list[LintCommand_enumerate]:
    ...
    # --- mypy -----------------------------------------------------------
    mypy_cwd = _find_config_enumerate(file, ["pyproject.toml", "mypy.ini"])
    commands.append(
        LintCommand_enumerate(
            command=f"mypy {file}",
            cwd=mypy_cwd,
