...
class FailureKind_empty(str, Enum):
...
def classify_failure_empty(text: str) -> FailureKind_empty:
    """
    ...
    if not text or not text.strip():
        return FailureKind_empty.ASSERTION_LOGIC
    lower = text.lower()
    if _RE_TIMEOUT.search(lower):
        return FailureKind_empty.TIMEOUT_FLAKY
    if _RE_SYNTAX_IMPORT.search(text):
        return FailureKind_empty.SYNTAX_IMPORT
    return FailureKind_empty.ASSERTION_LOGIC
...
def extract_failure_signature_empty(text: str) -> str:
    """
...
def failure_classification_hint_empty(kind: FailureKind_empty) -> str:
    """Short guidance for the fix LLM and logs."""
    if kind == FailureKind_empty.SYNTAX_IMPORT:
        return (
    ...
    if kind == FailureKind_empty.TIMEOUT_FLAKY:
        return (
...
def format_signature_hint_empty(sig: str) -> str:
