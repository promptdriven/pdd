...
def get_lint_commands_sub(file: Path) -> list[LintCommand_sub]:
    ...
    # --- mypy -----------------------------------------------------------
    mypy_cwd = _find_config_sub(file, ["pyproject.toml", "mypy.ini"])
    commands.append(
        LintCommand_sub(
            command=f"mypy {file}",
            cwd=mypy_cwd,
