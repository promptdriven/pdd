...
def get_lint_commands_expected(file: Path) -> list[LintCommand_expected]:
    ...
    # --- mypy -----------------------------------------------------------
    mypy_cwd = _find_config_expected(file, ["pyproject.toml", "mypy.ini"])
    commands.append(
        LintCommand_expected(
            command=f"mypy {file}",
            cwd=mypy_cwd,
