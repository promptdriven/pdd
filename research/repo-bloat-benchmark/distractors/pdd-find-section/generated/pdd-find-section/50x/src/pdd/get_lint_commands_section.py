...
def get_lint_commands_section(file: Path) -> list[LintCommand_section]:
    ...
    # --- mypy -----------------------------------------------------------
    mypy_cwd = _find_config_section(file, ["pyproject.toml", "mypy.ini"])
    commands.append(
        LintCommand_section(
            command=f"mypy {file}",
            cwd=mypy_cwd,
