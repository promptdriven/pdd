...
def get_lint_commands_multiple(file: Path) -> list[LintCommand_multiple]:
    ...
    # --- mypy -----------------------------------------------------------
    mypy_cwd = _find_config_multiple(file, ["pyproject.toml", "mypy.ini"])
    commands.append(
        LintCommand_multiple(
            command=f"mypy {file}",
            cwd=mypy_cwd,
