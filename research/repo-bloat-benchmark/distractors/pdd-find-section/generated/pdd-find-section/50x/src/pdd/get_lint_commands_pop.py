...
def get_lint_commands_pop(file: Path) -> list[LintCommand_pop]:
    ...
    # --- mypy -----------------------------------------------------------
    mypy_cwd = _find_config_pop(file, ["pyproject.toml", "mypy.ini"])
    commands.append(
        LintCommand_pop(
            command=f"mypy {file}",
            cwd=mypy_cwd,
