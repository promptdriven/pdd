...
class FailureKind_open(str, Enum):
...
def classify_failure_open(text: str) -> FailureKind_open:
    """
    ...
    if not text or not text.strip():
        return FailureKind_open.ASSERTION_LOGIC
    lower = text.lower()
    if _RE_TIMEOUT.search(lower):
        return FailureKind_open.TIMEOUT_FLAKY
    if _RE_SYNTAX_IMPORT.search(text):
        return FailureKind_open.SYNTAX_IMPORT
    return FailureKind_open.ASSERTION_LOGIC
...
def extract_failure_signature_open(text: str) -> str:
    """
...
def failure_classification_hint_open(kind: FailureKind_open) -> str:
    """Short guidance for the fix LLM and logs."""
    if kind == FailureKind_open.SYNTAX_IMPORT:
        return (
    ...
    if kind == FailureKind_open.TIMEOUT_FLAKY:
        return (
...
def format_signature_hint_open(sig: str) -> str:
