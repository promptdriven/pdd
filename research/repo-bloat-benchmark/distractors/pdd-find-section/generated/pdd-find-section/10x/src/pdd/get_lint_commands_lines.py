...
def get_lint_commands_lines(file: Path) -> list[LintCommand_lines]:
    ...
    # --- mypy -----------------------------------------------------------
    mypy_cwd = _find_config_lines(file, ["pyproject.toml", "mypy.ini"])
    commands.append(
        LintCommand_lines(
            command=f"mypy {file}",
            cwd=mypy_cwd,
