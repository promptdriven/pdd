...
def get_lint_commands_hints(file: Path) -> list[LintCommand_hints]:
    ...
    # --- mypy -----------------------------------------------------------
    mypy_cwd = _find_config_hints(file, ["pyproject.toml", "mypy.ini"])
    commands.append(
        LintCommand_hints(
            command=f"mypy {file}",
            cwd=mypy_cwd,
