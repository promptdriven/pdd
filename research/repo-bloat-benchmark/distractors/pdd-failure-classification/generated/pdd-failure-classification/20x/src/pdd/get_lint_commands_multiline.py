...
def get_lint_commands_multiline(file: Path) -> list[LintCommand_multiline]:
    ...
    # --- mypy -----------------------------------------------------------
    mypy_cwd = _find_config_multiline(file, ["pyproject.toml", "mypy.ini"])
    commands.append(
        LintCommand_multiline(
            command=f"mypy {file}",
            cwd=mypy_cwd,
