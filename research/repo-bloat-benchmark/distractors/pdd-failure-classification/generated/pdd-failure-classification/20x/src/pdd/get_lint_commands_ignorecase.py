...
def get_lint_commands_ignorecase(file: Path) -> list[LintCommand_ignorecase]:
    ...
    # --- mypy -----------------------------------------------------------
    mypy_cwd = _find_config_ignorecase(file, ["pyproject.toml", "mypy.ini"])
    commands.append(
        LintCommand_ignorecase(
            command=f"mypy {file}",
            cwd=mypy_cwd,
