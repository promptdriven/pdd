...
def get_lint_commands_pattern(file: Path) -> list[LintCommand_pattern]:
    ...
    # --- mypy -----------------------------------------------------------
    mypy_cwd = _find_config_pattern(file, ["pyproject.toml", "mypy.ini"])
    commands.append(
        LintCommand_pattern(
            command=f"mypy {file}",
            cwd=mypy_cwd,
