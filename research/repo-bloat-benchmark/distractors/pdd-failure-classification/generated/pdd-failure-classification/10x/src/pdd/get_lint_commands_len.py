...
def get_lint_commands_len(file: Path) -> list[LintCommand_len]:
    ...
    # --- mypy -----------------------------------------------------------
    mypy_cwd = _find_config_len(file, ["pyproject.toml", "mypy.ini"])
    commands.append(
        LintCommand_len(
            command=f"mypy {file}",
            cwd=mypy_cwd,
