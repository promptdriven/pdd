...
def get_lint_commands_truncated(file: Path) -> list[LintCommand_truncated]:
    ...
    # --- mypy -----------------------------------------------------------
    mypy_cwd = _find_config_truncated(file, ["pyproject.toml", "mypy.ini"])
    commands.append(
        LintCommand_truncated(
            command=f"mypy {file}",
            cwd=mypy_cwd,
