...
def get_lint_commands_truncates(file: Path) -> list[LintCommand_truncates]:
    ...
    # --- mypy -----------------------------------------------------------
    mypy_cwd = _find_config_truncates(file, ["pyproject.toml", "mypy.ini"])
    commands.append(
        LintCommand_truncates(
            command=f"mypy {file}",
            cwd=mypy_cwd,
