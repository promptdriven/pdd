...
def get_lint_commands_path(file: Path) -> list[LintCommand_path]:
    ...
    # --- mypy -----------------------------------------------------------
    mypy_cwd = _find_config_path(file, ["pyproject.toml", "mypy.ini"])
    commands.append(
        LintCommand_path(
            command=f"mypy {file}",
            cwd=mypy_cwd,
