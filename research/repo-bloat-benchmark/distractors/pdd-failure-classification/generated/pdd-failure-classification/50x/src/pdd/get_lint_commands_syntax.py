...
def get_lint_commands_syntax(file: Path) -> list[LintCommand_syntax]:
    ...
    # --- mypy -----------------------------------------------------------
    mypy_cwd = _find_config_syntax(file, ["pyproject.toml", "mypy.ini"])
    commands.append(
        LintCommand_syntax(
            command=f"mypy {file}",
            cwd=mypy_cwd,
