...
def get_lint_commands_flaky(file: Path) -> list[LintCommand_flaky]:
    ...
    # --- mypy -----------------------------------------------------------
    mypy_cwd = _find_config_flaky(file, ["pyproject.toml", "mypy.ini"])
    commands.append(
        LintCommand_flaky(
            command=f"mypy {file}",
            cwd=mypy_cwd,
