...
def get_lint_commands_pytest(file: Path) -> list[LintCommand_pytest]:
    ...
    # --- mypy -----------------------------------------------------------
    mypy_cwd = _find_config_pytest(file, ["pyproject.toml", "mypy.ini"])
    commands.append(
        LintCommand_pytest(
            command=f"mypy {file}",
            cwd=mypy_cwd,
