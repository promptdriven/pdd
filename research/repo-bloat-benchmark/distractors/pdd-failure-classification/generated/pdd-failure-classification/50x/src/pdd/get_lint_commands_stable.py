...
def get_lint_commands_stable(file: Path) -> list[LintCommand_stable]:
    ...
    # --- mypy -----------------------------------------------------------
    mypy_cwd = _find_config_stable(file, ["pyproject.toml", "mypy.ini"])
    commands.append(
        LintCommand_stable(
            command=f"mypy {file}",
            cwd=mypy_cwd,
