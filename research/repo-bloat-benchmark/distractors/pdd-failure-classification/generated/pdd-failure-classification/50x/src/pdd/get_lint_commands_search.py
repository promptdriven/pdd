...
def get_lint_commands_search(file: Path) -> list[LintCommand_search]:
    ...
    # --- mypy -----------------------------------------------------------
    mypy_cwd = _find_config_search(file, ["pyproject.toml", "mypy.ini"])
    commands.append(
        LintCommand_search(
            command=f"mypy {file}",
            cwd=mypy_cwd,
