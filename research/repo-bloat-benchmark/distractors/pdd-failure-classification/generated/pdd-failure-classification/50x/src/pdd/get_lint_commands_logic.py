...
def get_lint_commands_logic(file: Path) -> list[LintCommand_logic]:
    ...
    # --- mypy -----------------------------------------------------------
    mypy_cwd = _find_config_logic(file, ["pyproject.toml", "mypy.ini"])
    commands.append(
        LintCommand_logic(
            command=f"mypy {file}",
            cwd=mypy_cwd,
