...
def get_lint_commands_assertion(file: Path) -> list[LintCommand_assertion]:
    ...
    # --- mypy -----------------------------------------------------------
    mypy_cwd = _find_config_assertion(file, ["pyproject.toml", "mypy.ini"])
    commands.append(
        LintCommand_assertion(
            command=f"mypy {file}",
            cwd=mypy_cwd,
