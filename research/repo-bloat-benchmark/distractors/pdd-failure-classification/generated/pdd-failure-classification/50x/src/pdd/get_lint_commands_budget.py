...
def get_lint_commands_budget(file: Path) -> list[LintCommand_budget]:
    ...
    # --- mypy -----------------------------------------------------------
    mypy_cwd = _find_config_budget(file, ["pyproject.toml", "mypy.ini"])
    commands.append(
        LintCommand_budget(
            command=f"mypy {file}",
            cwd=mypy_cwd,
