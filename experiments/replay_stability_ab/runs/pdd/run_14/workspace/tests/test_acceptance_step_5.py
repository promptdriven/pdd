"""Step 5 acceptance tests — strict mode (14 tests).

Cumulative: step 0-4 (12) + step 5 (2) = 14 tests.
"""
...
# --- Step 0 tests (4) — ParseResult form ---
...
# --- Step 1 tests (2) — ParseResult form ---
...
# --- Step 2 tests (2) — ParseResult form ---
...
# --- Step 3 tests (2) — ParseResult form ---
...
# --- Step 4 tests (2) — source field ---


def test_parse_result_source_string_and_dict():
    r1 = parse_user_id("user:alice_1")
    assert r1 is not None and r1.source == "string"

    r2 = parse_user_id({"user_id": "bob_99"})
    assert r2 is not None and r2.source == "dict_flat"

    r3 = parse_user_id({"user": {"id": "carol_1"}})
    assert r3 is not None and r3.source == "dict_nested"
...
# --- Step 5 tests (2) — NEW: strict mode ---


def test_strict_mode_rejects_consecutive_special_chars():
    """strict=True rejects IDs with __, --, _-, -_."""
    assert parse_user_id("user:hello__world", strict=True) is None
    assert parse_user_id("user:hello--world", strict=True) is None
    assert parse_user_id("user:hello_-world", strict=True) is None
    assert parse_user_id("user:hello-_world", strict=True) is None
    # Single special chars still OK
    r = parse_user_id("user:hello_world", strict=True)
    assert r is not None and r.user_id == "hello_world"
    # strict=False (default) allows consecutive specials
    r2 = parse_user_id("user:hello__world")
    assert r2 is not None and r2.user_id == "hello__world"


def test_strict_mode_with_batch_and_email():
    """strict threads through batch and works with email input."""
    items = [
        "user:good_id",
        "user:bad__id",
        "email:ok-user@x.com",
    ...
    assert len(result) == 4
    assert result[0] is not None and result[0].user_id == "good_id"
    assert result[1] is None
    assert result[2] is not None and result[2].user_id == "ok-user"
    assert result[3] is None
