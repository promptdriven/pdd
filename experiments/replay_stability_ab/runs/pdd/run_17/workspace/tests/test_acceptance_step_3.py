"""Step 3 acceptance tests — configurable reserved IDs (10 tests).

Cumulative: step 0 (4) + step 1 (2) + step 2 (2) + step 3 (2) = 10 tests.
"""
...
def test_configurable_reserved_ids():
    """reserved_ids=None uses defaults; provided set replaces defaults."""
    # Default reserved still works
    assert parse_user_id("user:admin") is None
    assert parse_user_id("user:root") is None
    # Custom reserved replaces defaults entirely
    assert parse_user_id("user:admin", reserved_ids={"blocked"}) == "admin"
    assert parse_user_id("user:blocked", reserved_ids={"blocked"}) is None
    # Empty set means nothing is reserved
    assert parse_user_id("user:admin", reserved_ids=set()) == "admin"


def test_batch_with_custom_reserved_ids():
    """parse_user_ids passes reserved_ids through to parse_user_id."""
    items = ["user:admin", "user:hello", "user:banned"]
    result = parse_user_ids(items, reserved_ids={"banned"})
    assert result == ["admin", "hello", None]
