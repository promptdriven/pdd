...
def _save_run_report_atomic(report: Dict[str, Any], basename: str, language: str,
    ...
    """Save a run report, supporting atomic updates."""
    if atomic_state:
...
def _parse_test_output(output: str, language: str) -> tuple:
    ...
    if lang == 'python':
        ...
        if not cov:
            cov = re.search(r'(\d+)%\s*$', output, re.MULTILINE)
        if not cov:
            cov = re.search(r'(\d+(?:\.\d+)?)%', output)
        if cov:
...
def _try_auto_fix_import_error(error_output: str, code_file: Path, example_file: Path) -> tuple:
    """Try to automatically fix common import errors."""
...
def _run_example_with_error_detection(cmd_parts: list, env: dict, cwd: Optional[str] = None, timeout: int = 60) -> tuple:
    ...
    t_out = _threading.Thread(target=read_pipe, args=(proc.stdout, stdout_chunks), daemon=True)
    t_err = _threading.Thread(target=read_pipe, args=(proc.stderr, stderr_chunks), daemon=True)
    t_out.start()
    t_err.start()
...
def _create_synthetic_run_report_for_agentic_success(
    test_file: Path, basename: str, language: str,
    *, atomic_state: Optional['AtomicStateUpdate'] = None,
) -> RunReport:
    ...
    report = RunReport(
        timestamp=timestamp, exit_code=0, tests_passed=1,
        tests_failed=0, coverage=0.0, test_hash=test_hash,
    )
    report_file = META_DIR / f"{_safe_basename(basename)}_{language.lower()}_run.json"
    if atomic_state:
...
def _execute_tests_and_create_run_report(
    ...
    *, code_file: Optional[Path] = None, atomic_state: Optional['AtomicStateUpdate'] = None,
    ...
    test_file_hashes = {
        f.name: calculate_sha256(f) for f in all_test_files if f.exists()
    ...
    try:
        ...
        if lang_lower == "python":
            ...
            pytest_args = [python_executable, '-m', 'pytest'] + [str(f) for f in all_test_files] + [
                '-v', '--tb=short', f'--cov={cov_target}', '--cov-report=term-missing'
            ]
            ...
            if test_cmd is None:
                report = RunReport(timestamp=timestamp, exit_code=127, tests_passed=0,
                                   tests_failed=0, coverage=0.0, test_hash=test_hash,
            ...
            result = subprocess.run(
                ...
                env=clean_env, cwd=str(test_file.parent), stdin=subprocess.DEVNULL,
        ...
        report = RunReport(
            timestamp=timestamp, exit_code=exit_code, tests_passed=tests_passed,
            tests_failed=tests_failed, coverage=coverage, test_hash=test_hash,
            test_files=test_file_hashes,
        )
    except Exception:
        report = RunReport(
            timestamp=timestamp, exit_code=1, tests_passed=0, tests_failed=1,
            coverage=0.0, test_hash=test_hash, test_files=test_file_hashes,
        )
    ...
    for entry in log_entries:
        ...
        if verbose:
            ...
            if 'details' in entry and entry['details']:
                ...
                if details_copy:
                    print(f"  Details: {json.dumps(details_copy, indent=2)}")
        else:
            status_icon = "✓" if success else "✗" if success is False else "?"
            cost_info = f" | {status_icon} ${actual_cost:.2f} (est: ${estimated_cost:.2f})" if actual_cost is not None else f" | Est: ${estimated_cost:.2f}"
            duration_info = f" | {duration:.1f}s" if duration is not None else ""
            error_info = f" | Error: {entry['error']}" if entry.get('error') else ""
...
def sync_orchestration(
    ...
    # Defense in depth: replace None values with defaults
    ...
    try:
        ...
        if "test_config.py" in str(e) or "tests/test_" in str(e):
            ...
            return {
                "success": False, "error": f"Failed to construct paths: {str(e)}",
                "operations_completed": [], "errors": [f"Path construction failed: {str(e)}"]
            }
    except Exception as e:
        print(f"Error constructing paths: {e}")
        return {
    ...
    def sync_worker_logic():
        ...
        try:
            log_event(basename, language, "sync_start", {"pid": os.getpid()}, invocation_mode="sync")
        except Exception:
            pass
        ...
        try:
                ...
                while True:
                    ...
                    if current_cost_ref[0] >= budget:
                        ...
                        log_event(basename, language, "budget_exceeded", {
                            "total_cost": current_cost_ref[0], "budget": budget
                    ...
                    decision = sync_determine_operation(
                        basename, language, target_coverage, budget_remaining,
                    ...
                    if no_steer:
                        ...
                        steered_op, should_abort = maybe_steer_operation(
                            operation, decision.reason, app_ref[0], quiet,
                            skip_tests, skip_verify, timeout_s=steer_timeout,
                        )
                    if should_abort:
                    ...
                    log_entry = create_log_entry(
                        ...
                        invocation_mode="sync", estimated_cost=decision.estimated_cost,
                    ...
                    if operation == 'test_extend':
                        if _use_agentic_path(language, agentic_mode):
                            log_event(basename, language, "test_extend_skipped", {
                                "reason": f"test_extend not supported for {language} (or agentic_mode)"
                            }, invocation_mode="sync")
                            success = True
                            break
                        ...
                        if extend_attempts >= MAX_TEST_EXTEND_ATTEMPTS:
                            log_event(basename, language, "test_extend_limit", {
                                "attempts": extend_attempts, "max_attempts": MAX_TEST_EXTEND_ATTEMPTS,
                    ...
                    ctx = _create_mock_context(
                        ...
                        confirm_callback=get_confirm_callback(), context=context_override,
                    ...
                    with AtomicStateUpdate(basename, language) as atomic_state:
                        try:
                            if operation == 'auto-deps':
                                ...
                                result = auto_deps_main(
                                    ...
                                    output=temp_output, force_scan=False,
                                ...
                                result = code_generator_main(
                                    ctx, prompt_file=str(pdd_files['prompt'].resolve()),
                                    output=str(pdd_files['code'].resolve()),
                                    original_prompt_file_path=None, force_incremental_flag=False
                                )
                                ...
                                result = context_generator_main(
                                    ctx, prompt_file=str(pdd_files['prompt'].resolve()),
                                    code_file=str(pdd_files['code'].resolve()),
                                    output=str(pdd_files['example'].resolve())
                                )
                                ...
                                if current_run_report and current_run_report.exit_code != 0:
                                    ...
                                    if returncode != 0:
                                        has_crash = True
                                        crash_log_content = f"Example failed exit code: {returncode}\nSTDOUT:\n{stdout}\nSTDERR:\n{stderr}\n"
                                ...
                                if has_crash:
                                    ...
                                    if auto_fixed:
                                        ...
                                        if retry_returncode == 0:
                                            ...
                                            # Fix for issue #430: Save fingerprint and track operation completion
                                    ...
                                    try:
                                        ...
                                        result = crash_main(
                                            ...
                                            loop=True, max_attempts=effective_max_attempts,
                                ...
                                result = fix_verification_main(
                                    ...
                                    code_file=str(pdd_files['code']),
                                    program_file=str(pdd_files['example']),
                                    output_results=f"{basename.replace('/', '_')}_verify_results.log",
                                    output_code=str(pdd_files['code']),
                                    output_program=str(pdd_files['example']),
                                    loop=True, verification_program=str(pdd_files['example']),
                                ...
                                result = cmd_test_main(
                                    ...
                                    existing_tests=[str(pdd_files['test'])] if test_file_exists else None,
                                    target_coverage=target_coverage,
                                    merge=test_file_exists,
                                    strength=strength, temperature=temperature
                                )
                                ...
                                if pdd_files['test'].exists():
                                    ...
                                    result = cmd_test_main(
                                        ...
                                        code_file=str(pdd_files['code']),
                                        output=str(pdd_files['test']), language=language,
                                        coverage_report=None, existing_tests=[existing_test_path],
                                    ...
                                    result = cmd_test_main(
                                        ...
                                        code_file=str(pdd_files['code']),
                                        output=str(pdd_files['test']), language=language,
                                        coverage_report=None, existing_tests=None,
                                        target_coverage=target_coverage, merge=False,
                                        strength=strength, temperature=temperature
                                    )
                                    agentic_success = None
                                    if isinstance(result, tuple) and len(result) >= 4:
                                    ...
                                    if test_cmd:
                                        if language.lower() == 'python' and not agentic_mode:
                                            ...
                                            test_result = subprocess.run(
                                                ...
                                                timeout=300, stdin=subprocess.DEVNULL, env=clean_env,
                                                cwd=str(pdd_files['test'].parent), start_new_session=True
                                            )
                                ...
                                # Bug #156: Find actual failing files
                                ...
                                result = update_main(
                                    ctx, input_prompt_file=str(pdd_files['prompt']),
                                    modified_code_file=str(pdd_files['code']),
                                    input_code_file=None, output=str(pdd_files['prompt']),
                                    use_git=True, strength=strength, temperature=temperature
                                )
                            else:
                        ...
                        update_log_entry(log_entry, success=success, cost=actual_cost, model=model_name,
                                         duration=duration, error=errors[-1] if errors and not success else None)
                        append_log_entry(basename, language, log_entry)
                            ...
                            if _use_agentic_path(language, agentic_mode):
                                ...
                                try:
                                    ...
                                    report = RunReport(
                                        ...
                                        100.0 if returncode == 0 else 0.0, test_hash=test_hash
                                    )
                                    _save_run_report_atomic(asdict(report), basename, language)
                                except Exception as e:
                                    error_msg = f"Post-crash verification failed: {e}"
                                    errors.append(error_msg)
                                    log_event(basename, language, "post_crash_verification_failed",
                                              {"error": str(e)}, invocation_mode="sync")
            ...
            try:
                log_event(basename, language, "lock_released",
                          {"pid": os.getpid(), "total_cost": current_cost_ref[0]}, invocation_mode="sync")
            except Exception:
                pass
    ...
    if headless:
        ...
        app = SyncApp(
            basename=basename, budget=budget, worker_func=sync_worker_logic,
            function_name_ref=current_function_name_ref, cost_ref=current_cost_ref,
            prompt_path_ref=prompt_path_ref, code_path_ref=code_path_ref,
            example_path_ref=example_path_ref, tests_path_ref=tests_path_ref,
            prompt_color_ref=prompt_box_color_ref, code_color_ref=code_box_color_ref,
            example_color_ref=example_box_color_ref, tests_color_ref=tests_box_color_ref,
            stop_event=stop_event, progress_callback_ref=progress_callback_ref
        )
        app_ref[0] = app
        result = app.run()
    ...
    if not headless and worker_exception is not None:
        ...
        if hasattr(worker_exception, '__traceback__'):
            traceback.print_exception(type(worker_exception), worker_exception,
                                      worker_exception.__traceback__, file=sys.stderr)

    if result is None:
        return {
            "success": False, "total_cost": current_cost_ref[0], "model_name": "",
            "error": "Sync process interrupted or returned no result.",
            "operations_completed": [], "errors": ["App exited without result"]
        }
