...
from .sync_tui import SyncApp, maybe_steer_operation, DEFAULT_STEER_TIMEOUT_S
...
def _run_example_with_error_detection(
    ...
    # Issue #xxx fix: Ensure cmd_parts and cwd are absolute
    # This prevents 'double-relative' paths when cwd is set to a relative path
    if cwd and not os.path.isabs(cwd):
        cwd = os.path.abspath(cwd)
    
    # Ensure command executable is absolute if it's a file path
    executable = cmd_parts[0]
    if os.path.sep in executable and not os.path.isabs(executable):
        if cwd:
            cmd_parts[0] = os.path.join(cwd, executable)
        else:
            cmd_parts[0] = os.path.abspath(executable)
