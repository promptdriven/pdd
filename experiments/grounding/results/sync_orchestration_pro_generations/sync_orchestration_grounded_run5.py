...
def _save_run_report_atomic(report: Dict[str, Any], basename: str, language: str,
    ...
    if atomic_state:
        ...
        # Case-insensitive path construction for consistent paths
...
def _save_fingerprint_atomic(basename: str, language: str, operation: str,
    ...
    if atomic_state:
        ...
        # Case-insensitive path construction
        ...
        try:
            with SyncLock(basename, language):
                ...
                while True:
                    ...
                    # Detect crash-verify cycle pattern
                    ...
                    if operation == 'fix':
                        consecutive_fixes = sum(1 for op in reversed(operation_history) if op == 'fix')
                        if consecutive_fixes >= 5:
                            errors.append(f"Detected {consecutive_fixes} consecutive fix operations. Breaking infinite fix loop.")
                            break

                    if operation == 'test':
                        consecutive_tests = sum(1 for op in reversed(operation_history) if op == 'test')
                    ...
                    if operation == 'crash':
                        consecutive_crashes = sum(1 for op in reversed(operation_history) if op == 'crash')
