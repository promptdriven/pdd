...
def _parse_test_output(output: str, language: str) -> tuple[int, int, float]:
    ...
    if lang == 'python':
        ...
        if not coverage_match:
            coverage_match = re.search(r'(\d+(?:\.\d+)?)%')
        if coverage_match:
                        ...
                        try:
                            if operation == 'auto-deps':
                                ...
                                try:
                                    ...
                                    existing_test_files = [f for f in test_files_list if f.exists()]

                                    if not existing_test_files:
                                        error_content = "Test file(s) missing; skipping pytest execution."
                                    else:
                                        test_cmd = get_test_command_for_file(str(existing_test_files[0]), language)
                                        ...
                                        if test_cmd:
                                            ...
                                            if language.lower() == 'python' and not agentic_mode:
                                                ...
                                                pytest_args = [python_executable, '-m', 'pytest'] + [str(f) for f in existing_test_files] + ['-v', '--tb=short']

                                                # Bug fix: Find project root for proper pytest configuration
                                                # This matches the fix in _execute_tests_and_create_run_report()
                                                project_root = _find_project_root(existing_test_files[0])
                                                ...
                                                test_result = subprocess.run(
                                                    ...
                                                    cwd=str(existing_test_files[0].parent),
                                                    start_new_session=True
                                                )
