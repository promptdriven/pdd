...
def _save_fingerprint_atomic(
    ...
    """Save fingerprint state, supporting atomic updates."""
    if atomic_state:
        from datetime import datetime, timezone
        from .sync_determine_operation import Fingerprint
...
def _run_example_with_error_detection(
    ...
    has_errors, _ = _detect_example_errors(combined)
...
def sync_orchestration(
    ...
    try:
        ...
        if "test_config.py" in str(e) or "tests/test_" in str(e):
            ...
            if not quiet:
                print("Note: Test file missing, continuing with sync workflow to generate it")
        else:
        ...
        try:
                ...
                while True:
                    ...
                    if budget_remaining < budget * 0.2 and budget_remaining > 0:
                        log_event(
                            ...
                            language,
                            "budget_warning",
                            {"remaining": budget_remaining, "percentage": (budget_remaining / budget) * 100},
                            invocation_mode="sync",
                        )
                    ...
                    if len(operation_history) >= 4:
                        ...
                        if recent_ops == ["crash", "verify", "crash", "verify"] or recent_ops == ["verify", "crash", "verify", "crash"]:
                            errors.append(f"Detected crash-verify cycle repeated {MAX_CYCLE_REPEATS} times. Breaking cycle.")
                            log_event(
                                basename,
                                language,
                                "cycle_detected",
                                {"cycle_type": "crash-verify", "count": MAX_CYCLE_REPEATS},
                                invocation_mode="sync",
                            )
                            break
                    ...
                    if len(operation_history) >= 4:
                        recent_ops = operation_history[-4:]
                        if recent_ops == ["test", "fix", "test", "fix"] or recent_ops == ["fix", "test", "fix", "test"]:
                            errors.append(f"Detected test-fix cycle repeated {MAX_CYCLE_REPEATS} times. Breaking cycle.")
                            log_event(
                                basename,
                                language,
                                "cycle_detected",
                                {"cycle_type": "test-fix", "count": MAX_CYCLE_REPEATS},
                                invocation_mode="sync",
                            )
                            break
                            ...
                            if operation == "auto-deps":
                                ...
                                if current_run_report and current_run_report.exit_code != 0:
                                    ...
                                    if ex_res.returncode != 0:
                                        has_crash = True
                                        crash_log_content = (
                                            f"Example failed exit code: {ex_res.returncode}\nSTDOUT:\n{ex_res.stdout}\nSTDERR:\n{ex_res.stderr}\n"
                                        )
                                    ...
                                    if auto_fixed:
                                        ...
                                        if retry_returncode == 0:
                                            ...
                                            _save_fingerprint_atomic(
                                                basename, language, "crash", pdd_files, 0.0, "auto-fix", atomic_state=atomic_state
                                            )
                                            continue
                                        else:
                                            crash_log_content = (
                                                f"Auto-fix attempted ({auto_fix_msg}) but still failing:\nRETRY STDOUT:\n{retry_stdout}\nRETRY STDERR:\n{retry_stderr}\n"
                                            )
                                ...
                                if failing_files:
                                    ...
                                    tracked_in_failures = any(Path(ff).name == tracked_file_name for ff in failing_files)
                            ...
                            if _use_agentic_path(language, agentic_mode):
                                ...
                                try:
                                    ...
                                    report = RunReport(
                                        ...
                                        100.0 if returncode == 0 else 0.0,
                                        test_hash=test_hash,
                                    )
                                    _save_run_report_atomic(asdict(report), basename, language)
                                except Exception as e:
                                    ...
                                    log_event(
                                        basename,
                                        language,
                                        "post_crash_verification_failed",
                                        {"error": str(e)},
                                        invocation_mode="sync",
                                    )
            ...
            try:
                log_event(
                    ...
                    language,
                    "lock_released",
                    {"pid": os.getpid(), "total_cost": current_cost_ref[0]},
                    invocation_mode="sync",
                )
            except Exception:
                pass
    ...
    if headless:
        ...
        if not quiet:
            print("Running sync in headless mode (CI/non-TTY environment)...")
