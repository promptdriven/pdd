...
to run a synchronization analysis and execution workflow.
"""
...
# Setup directories relative to this file's directory
...
def setup_mock_project() -> tuple[Path, Path]:
    ...
    Returns:
        A tuple of (prompts_dir, code_dir) paths.
    """
...
def main() -> None:
    ...
    def mock_sync_orchestration(*args, **kwargs):
        return {
            "success": True,
            "total_cost": 0.0425,  # Cost of LLM calls in USD
    ...
    with patch("pdd.sync_main.construct_paths", side_effect=mock_construct_paths), \
        ...
        # - max_attempts (int | None): Max allowed iterative fixes. 0 skips loop.
        ...
        results, total_cost, primary_model = sync_main(
            ...
            budget=10.00,            # Cap spending at $10.00 USD
            skip_verify=False,       # Run semantic LLM verification
            skip_tests=False,        # Generate and run unit tests
            target_coverage=90.0,    # 90.0% coverage threshold
            dry_run=False,           # Execute actual writes and LLM loops
            one_session=False,       # Run step-by-step classic pipeline
            compress=False,          # Disable source AST-compression
            evidence=False,          # Skip exporting execution JSON evidence
            snapshot_context=False,  # Skip saving replayable context snapshots
            compressed_context=False # Disable phase-aware compressed context packages
        )

    console.rule("[bold blue]5. Sync Output Analysis[/bold blue]")
    console.print(f"Overall Status: [bold green]Success[/bold green]" if results.get("overall_success") else "[bold red]Failure[/bold red]")
