...
def copy_from_sync_config(
    ...
    def add_existing_dest_path(paths: set[str], path: str) -> None:
        rel_path = os.path.relpath(path, dest)
        if rel_path == ".":
            return
    ...
    def git_tracked_paths(pathspec: str) -> set[str]:
        result = subprocess.run(
            ["git", "-C", dest, "ls-files", "-z", "--", pathspec],
            capture_output=True,
            check=False,
        )
    ...
    def remove_stale_dest_path(rel_path: str, git_repo: bool) -> bool:
        removed = False
        if git_repo:
            result = subprocess.run(
                [
    ...
    dest_is_git_repo = sync_deletions and is_git_work_tree(dest)
    if sync_deletions:
        for pattern in patterns:
            ...
            for match in matches:
                if os.path.islink(match) or os.path.isfile(match):
                    add_existing_dest_path(existing_dest_files, match)
                    continue
            ...
            if dest_is_git_repo:
                existing_dest_files.update(git_tracked_paths(pattern))
    ...
    for pattern in patterns:
        ...
        for src in sorted(matches):
            rel_path = normalize_rel_path(os.path.relpath(src, project_root))
            ...
            if os.path.isdir(src):
                ...
                for root, dirs, files in os.walk(src, followlinks=True):
                    ...
                    for fname in files:
                        ...
                        rel_file = normalize_rel_path(
                            os.path.relpath(src_file, project_root)
                        )
    ...
    if sync_deletions:
        ...
        for rel_path in sorted(stale_files):
            if remove_stale_dest_path(rel_path, dest_is_git_repo):
