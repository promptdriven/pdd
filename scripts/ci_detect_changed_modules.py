...
def _basename_from_path(path: str) -> str | None:
    ...
    if normalized.startswith("pdd/") and normalized.endswith(".py"):
        ...
        # Skip only the root package marker; nested __init__.py files map to
        # real prompt basenames like commands/__init__ and server/routes/__init__.
        return None if basename == "__init__" else basename

    context_basename = _context_basename_from_path(path)
    if context_basename:
        return context_basename
